...
Lorsqu’un joueur rejoint le vocal d’un bâtiment, KingdomCore lui donne l’accès temporaire au salon textuel correspondant et crée dans ce salon un **fil privé** contenant son menu interactif. Ce fil reste dans l’espace Discord du bâtiment, n’est accessible qu’au joueur concerné (et aux administrateurs Discord), puis est supprimé automatiquement lorsque son propriétaire quitte le vocal ou change de bâtiment. Aucun message privé Discord n’est utilisé. Si le salon, le rôle ou les permissions de fils privés manquent, relancer la synchronisation Discord depuis KingdomWeb ; sur une installation historique, régénérer au besoin le lien avec `python run.py invite-url` pour accorder les nouvelles permissions.
