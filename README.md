...
La même page affiche les **Journaux système** de KingdomWeb, KingdomCore et KingdomVoice. Sur Debian, elle privilégie directement `journald` et conserve les fichiers `var/logs/*.out.log` et `*.err.log` comme repli. Les lignes peuvent être sélectionnées ou copiées en un clic ; les tokens, mots de passe et en-têtes d’autorisation sont masqués avant leur affichage. KingdomCore journalise notamment la reprise des anciens boutons Discord après redémarrage et le détail des interactions de bâtiment en échec.
...
**Présences vocales** propose un parcours direct : ajouter les Voice Workers au serveur depuis **Connexion Discord**, cliquer sur **Affecter un bot audio**, choisir explicitement **Ambiance sonore** ou **Personnage / PNJ**, puis sélectionner le bâtiment et la scène. Une ambiance diffuse un décor sonore sans incarner quelqu’un ; un personnage applique au worker une identité, une photo et éventuellement un profil vocal. Le mode automatique est sélectionné par défaut : KingdomVoice retrouve le salon vocal provisionné du bâtiment, choisit un worker libre réellement installé sur ce serveur et le connecte dès qu’un joueur entre. Les réglages de source, profil vocal, priorité et libération restent disponibles dans **Options avancées**. Sur mobile, ces informations restent consultables mais leur édition structurelle est réservée à l’ordinateur.

Les cartes des catalogues s’ouvrent uniquement avec leur bouton **Ouvrir** ou **Modifier** : un survol ou un clic ailleurs sur la carte ne déclenche aucune navigation. Un éditeur ne se ferme pas lorsque l’on clique accidentellement sur son arrière-plan. Pour les bâtiments, **Enregistrer** ferme l’éditeur et conserve un brouillon ; **Publier sur Discord** se trouve ensuite sur la carte et déclenche la synchronisation de la définition publiée.

KingdomEngine fournit cinq emplacements **Voice Worker** de base. Ils restent visibles même avant la configuration de leurs tokens et ne peuvent pas être supprimés. Des workers supplémentaires peuvent être ajoutés puis supprimés par l’administrateur. Un worker est uniquement une capacité technique : son nom et sa photo Discord se configurent dans la fiche **Voix et Présences** du bâtiment. Chaque worker utilisé doit aussi avoir été invité au serveur concerné depuis **Connexion Discord**. Un worker correspond à une connexion vocale simultanée : deux bâtiments occupés au même moment nécessitent donc deux workers installés. Lorsqu’un joueur quitte le salon, la capacité est libérée ; sa prochaine affectation réapplique automatiquement le nom et la photo de la nouvelle présence.

Les dix emplacements fournis utilisent `VOICE_WORKER_1_TOKEN` à `VOICE_WORKER_10_TOKEN` et leurs `VOICE_WORKER_1_APPLICATION_ID` à `VOICE_WORKER_10_APPLICATION_ID`. Les anciennes variables `EDGAR_BOT_TOKEN`, `EDOUARD_BOT_TOKEN`, `ROLAND_BOT_TOKEN`, `SYLVAIN_BOT_TOKEN` et `WAGNER_BOT_TOKEN` restent acceptées uniquement comme solution de compatibilité pour les cinq premiers Workers. Au démarrage, les anciennes fiches qui dupliquent un worker système sont archivées automatiquement. Les bots textuels ajoutés par d’anciens presets, comme **Intendant du Royaume**, sont également retirés : la connexion textuelle est portée uniquement par KingdomCore.
...
KingdomVoice reste actif et supervisable même lorsqu'aucun token n'est encore configuré. Si un token est révoqué, invalide ou si Discord est momentanément inaccessible, seul le Voice Worker concerné est placé en attente puis retenté automatiquement ; les autres workers continuent de fonctionner et le service systemd ne tombe plus avec lui. Le détail non sensible de l'erreur apparaît dans `journalctl -u kingdom-voice` et `var/logs/voice.err.log`/`voice.out.log`.

Une présence vocale publiée en mode **automatique** est affectée par KingdomVoice à un worker disponible. L’affectation enregistre directement le bâtiment choisi : elle reste donc fiable même si plusieurs bâtiments appartiennent au même lieu. KingdomVoice surveille tous les mondes actifs déclarés dans `managed_servers`, chacun avec sa propre base et son propre serveur Discord. Les diagnostics distinguent serveur introuvable, salon absent ou obsolète, permissions Discord `Connect`/`Speak` manquantes et capacité indisponible. Discord autorise un surnom propre au serveur pour représenter la présence ; l’avatar et la description d’un bot restent globaux à son application et ne sont donc pas modifiés à chaque affectation.
...
Comme dans KingdomEngine V1, le salon textuel du bâtiment contient un panneau d’entrée commun. Lorsqu’un joueur rejoint le vocal, KingdomCore lui donne temporairement accès à ce salon. Le clic sur **Entrer dans le bâtiment** affiche ensuite une véritable interface Discord éphémère (`ephemeral`) : elle apparaît dans le salon mais reste strictement invisible aux autres joueurs. Aucun message privé et aucun fil Discord ne sont utilisés. Le bouton refuse également l’ouverture si le joueur n’est plus dans le vocal correspondant. Si le salon ou le rôle du bâtiment manque, relancer la synchronisation Discord depuis KingdomWeb.

L’éditeur de bâtiments utilise toute la surface disponible sur les grands écrans : la liste des bâtiments et l’inspecteur restent accessibles sur les côtés tandis que le formulaire et le builder Discord occupent automatiquement toute la largeur centrale restante. Les dispositions tablette et mobile restent simplifiées.
...
Dans KingdomWeb, le parcours quotidien se fait depuis **Bâtiments → ouvrir un bâtiment → Audio & Présences**. Cet écran réunit l'ambiance, les personnages résidents, les sons ponctuels, les routines et les scènes liées. L'assistant **Ajouter un personnage** crée et associe automatiquement le profil vocal et la présence technique. Les catalogues historiques Voice Presence/Profile sont conservés pour l'administration avancée et la compatibilité, mais ne figurent plus dans la navigation normale.
