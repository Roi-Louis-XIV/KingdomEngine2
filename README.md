...
Les cinq variables actuelles sont `VOICE_WORKER_1_TOKEN` à `VOICE_WORKER_5_TOKEN` et leurs Application IDs correspondants. Les anciennes variables `EDGAR_BOT_TOKEN`, `EDOUARD_BOT_TOKEN`, `ROLAND_BOT_TOKEN`, `SYLVAIN_BOT_TOKEN` et `WAGNER_BOT_TOKEN` restent acceptées uniquement comme solution de compatibilité. Au démarrage, les anciennes fiches qui dupliquent un worker système sont archivées automatiquement. Les bots textuels ajoutés par d’anciens presets, comme **Intendant du Royaume**, sont également retirés : la connexion textuelle est portée uniquement par KingdomCore.
