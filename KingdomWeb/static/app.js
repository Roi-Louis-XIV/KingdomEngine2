...
function buttonPropertyFields(component) {
  ...
  let fields=propertyInput("Libellé","label",props.label)+propertyInput("Emoji","emoji",props.emoji)+propertySelect("Style","style",props.style||"primary",[["primary","Principal"],["secondary","Secondaire"],["success","Succès"],["danger","Danger"]])+propertySelect("Au clic","interaction_type",interaction.type,[["navigate","Ouvrir une page"],["action","Lancer une action"],["refresh","Actualiser la page"],["close","Quitter / fermer"]]);
...
function restoreContentToolbar(){const toolbar=$("#content-workspace .toolbar");if(!$("#search"))toolbar.innerHTML='<input id="search" placeholder="Rechercher…"><span>Les changements publiés sont chargés par tous les modules.</span>';$("#search").oninput=renderCards;}
...
async function loadDashboard() {
  ...
  const core=serviceMetric("core"),voice=serviceMetric("voice");
  $("#admin-view").innerHTML=`<div class="royal-dashboard"><section class="royal-metrics">${metricCard("HABITANTS INSCRITS",metrics.players,"participants au jeu")}${metricCard("ACTIFS AUJOURD’HUI",activeToday,"activité dans les dernières 24 h")}${metricCard("KINGDOMCORE",core?.running?"En ligne":"Arrêté","moteur du Royaume")}${metricCard("KINGDOMVOICE",voice?.running?"En ligne":"Arrêté","voix et ambiances")}</section><div class="royal-rankings"><section class="royal-panel"><div class="royal-panel-title"><small>FORTUNES</small><h2>Les plus riches du Royaume</h2></div><ol>${rank(data.rankings?.wealth||[],"wealth")}</ol></section><section class="royal-panel"><div class="royal-panel-title"><small>MÉTIERS</small><h2>Les plus expérimentés</h2></div><ol>${rank(data.rankings?.experience||[],"experience")}</ol></section></div>${projects||`<section class="royal-panel empty-admin">Aucun chantier collectif actif.</section>`}<section class="royal-panel"><div class="royal-panel-title"><small>DERNIERS ÉCHOS</small><h2>Activité récente</h2><button data-go="supervision">Voir le direct →</button></div><ol class="royal-activity">${activity}</ol></section></div>`;
