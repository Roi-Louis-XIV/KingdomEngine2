...
const state = {
  ...
  playerFilters: { search: "", profession: "", status: "", sort: "recent" },
  expandedPlayers: [],
  audioFilters: { search: "", type: "", bot: "", sort: "name_asc" },
  audioMode: "library",
  worldMapZoom: 1,
  worldMapDragged: false,
  worldMapBackgroundUrl: "",
  catalogs: {
    item: [],
    itemEnriched: [],
    event: [],
...
function technicalKey(value, fallback = "element") {
  ...
  return (slug.length >= 3 ? slug : `${fallback}_${slug || "nouveau"}`).slice(
    0,
    64,
  );
}
const headers = { "Content-Type": "application/json" };
function syncServerHeaders() {
  if (state.server) headers["X-Kingdom-Server"] = state.server;
  else delete headers["X-Kingdom-Server"];
}
function multipartHeaders() {
  const result = {};
  if (state.server) result["X-Kingdom-Server"] = state.server;
...
const pageDescriptions = {
  ...
  live_world:
    "Observez l’heure, la météo, les événements, les chemins et les positions réelles.",
  calendar:
    "Créez vos mois, cycles, jours et saisons indépendamment de l’environnement.",
  players:
    "Consultez l’état des joueurs, leurs inventaires et leurs activités en direct.",
  building:
    "Construisez les espaces, mécaniques et interfaces Discord de votre monde.",
  profession:
    "Configurez les activités, progressions, outils et productions de votre univers.",
  item: "Gérez le catalogue utilisé par les inventaires, productions et commerces.",
  npc: "Créez des personnages indépendants de Discord et leurs réactions au monde.",
  event: "Configurez les déclencheurs et les effets qui font évoluer le monde.",
  environment:
    "Pilotez l’heure, le jour, la météo et leurs influences temporaires.",
  location:
    "Reliez les territoires, stations, pièces ou destinations de votre monde persistant.",
  bot: "Configurez les applications Discord qui fournissent les capacités du monde.",
  voice_presence:
    "Donnez une identité sonore temporaire à un personnage, une ambiance ou une scène.",
  audio: "Centralisez les voix, musiques, ambiances, scènes et effets sonores.",
  supervision:
    "Surveillez la disponibilité du monde, l’activité et les alertes utiles.",
  settings:
    "Définissez les rôles, salons et règles générales du serveur Discord.",
  profile:
    "Gérez votre compte, vos serveurs Discord et les droits de votre équipe.",
...
function applyTheme(theme, persist = false) {
  const preference = ["light", "dark", "system"].includes(theme)
    ? theme
    : "system";
  const selected =
    preference === "system"
  ...
  const sidebarButton = $("#sidebar-theme");
  if (sidebarButton)
    sidebarButton.textContent = `${selected === "dark" ? "☀" : "☾"} Mode ${selected === "dark" ? "clair" : "sombre"}`;
...
function installProductShell() {
  ...
  $('[data-account-destination="profile"]').onclick = () => {
    $("#account-menu").hidden = true;
    navigateTo("profile");
  };
  $("[data-account-logout]").onclick = logoutAccount;
  $$("[data-theme-choice]").forEach(
    (button) =>
      (button.onclick = () => applyTheme(button.dataset.themeChoice, true)),
  );
  document.addEventListener("click", (event) => {
    if (!event.target.closest("#account-menu,#account-button")) {
      const menu = $("#account-menu");
...
const HELP = {
  preset: [
    "Choisir un modèle",
    "Le modèle prépare une structure complète. Tout reste modifiable ensuite.",
    [
      "Récolte pour obtenir des ressources",
      "Production pour transformer",
      "Commerce pour vendre",
    ],
  ],
  name: [
    "Nom du lieu",
    "Choisis un nom court et évocateur. Il sera affiché dans Discord et utilisé pour nommer les salons.",
    ["Ferme du Royaume", "Atelier des alchimistes"],
  ],
  emoji: [
    "Symbole",
    "Un emoji aide les joueurs à reconnaître instantanément le lieu.",
    ["🌾 pour une ferme", "⚒️ pour une forge"],
  ],
  description: [
    "Description",
    "Explique simplement ce que le joueur peut faire ici, en une seule phrase.",
    ["Récoltez des céréales et nourrissez le village."],
  ],
  actions: [
    "Actions des joueurs",
    "Chaque action devient un bouton Discord. Commence par une action claire, puis ajoute ses conséquences.",
    ["Récolter", "Acheter", "Discuter"],
  ],
  action_name: [
    "Nom du bouton",
    "Utilise un verbe qui annonce clairement ce qui va se passer.",
    ["Couper du bois", "Commander un repas"],
  ],
  effects: [
    "Résultat de l’action",
    "Les résultats sont exécutés dans l’ordre. Pour attribuer un métier, choisis « Gérer un métier », puis « Attribuer / rejoindre le métier » et sélectionne le métier concerné.",
    [
      "Attribuer le métier de Cuisinier",
      "Donner 25 XP de Bûcheron",
      "Retirer 5 énergie",
      "Donner 2 bois",
    ],
  ],
  effect_resource: [
    "Ressource ou objet",
    "Utilise l’identifiant d’un objet existant, ou money / energy pour la monnaie et l’énergie.",
    ["wood", "iron_ore", "money", "energy"],
  ],
  condition_type: [
    "Condition d’accès",
    "Choisis ce que le moteur doit vérifier avant d’autoriser l’action. Les conditions peuvent être combinées ou inversées.",
    ["Métier actif", "Durabilité minimale", "Rôle Discord"],
  ],
  condition_operator: [
    "Comparaison",
    "Compare la valeur du joueur avec le seuil configuré.",
    ["≥ 5 : au moins cinq", "= 1 : exactement un"],
  ],
  condition_value: [
    "Valeur attendue",
    "Seuil numérique utilisé par la condition. Les conditions oui/non utilisent généralement 1.",
    ["10 énergie", "2 niveaux"],
  ],
  condition_group: [
    "Combiner les conditions",
    "Toutes exige chaque condition. Au moins une accepte l’action dès qu’une condition est vraie.",
    [],
  ],
  module_activity_scope: [
    "Portée de la limite",
    "Détermine quelles activités en cours sont comptées pour bloquer un nouveau lancement.",
    ["Joueur + bâtiment", "Joueur + action", "Catégorie"],
  ],
  module_activity_min_durability: [
    "Durabilité requise",
    "L’activité est refusée si l’outil possède moins de durabilité que ce seuil.",
    [],
  ],
  module_hook_claim: [
    "Événement de récupération",
    "Cet événement KingdomEvent est émis lorsque le joueur récupère le résultat. Le résultat aléatoire sélectionné est inclus.",
    [],
  ],
  npc_name: [
    "Personnage associé",
    "Facultatif. Donne un visage au bâtiment et prépare les futures interactions narratives.",
    ["Roland le mineur"],
  ],
  color: [
    "Couleur Discord",
    "Couleur hexadécimale des encarts Discord, sans le caractère #.",
    ["22c55e", "8b5cf6"],
  ],
  technical_key: [
    "Identifiant technique",
    "Il relie les données au moteur. Il est généré automatiquement et ne doit contenir que des lettres minuscules, chiffres et underscores.",
    ["ferme_du_royaume"],
  ],
  modules_json: [
    "Configuration intégrale",
    "Tous les paramètres du bâtiment sont conservés ici : PNJ, métiers, niveaux, durées, énergie, stocks, recettes, butins, livraisons, réparations, améliorations et jeux.",
    [
      "Modifie une valeur puis publie le brouillon",
      "Le moteur régénère les actions depuis ces modules",
    ],
  ],
  building_mechanics: [
    "Comprendre le fonctionnement",
    "Un métier définit qui peut agir. Une activité décrit où agir, pendant combien de temps et avec quel outil. Les résultats indiquent ensuite ce que le joueur reçoit.",
    [
      "1. Crée le métier",
      "2. Ajoute ses zones d’activité",
      "3. Ajoute plusieurs résultats et leurs effets",
      "4. Enregistre puis publie",
    ],
  ],
};
...
const EMOJI_LIBRARY = {
  Tous: [],
  Bâtiments: [
    ["🏰", "château royaume administration"],
    ["🏠", "maison foyer"],
    ["🏘️", "village maisons"],
    ["🏛️", "administration mairie temple"],
    ["⛪", "église culte"],
    ["🏥", "hôpital soin"],
    ["🏦", "banque argent"],
    ["🏪", "boutique commerce"],
  ...
  Métiers: [
    ["⚒️", "forge forgeron artisan"],
    ["🪓", "hache bûcheron bois"],
    ["⛏️", "mine pioche minerai"],
    ["🏹", "chasseur arc"],
    ["🛡️", "garde défense"],
    ["⚔️", "combat épée"],
    ["🧙", "mage magie"],
...
EMOJI_LIBRARY["Tous"] = Object.entries(EMOJI_LIBRARY)
  .filter(([name]) => name !== "Tous")
  .flatMap(([, items]) => items);
let emojiCategory = "Tous";
const normaliseEmojiSearch = (value) =>
  String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
function renderEmojiLibrary() {
  const query = normaliseEmojiSearch($("#emoji-search")?.value),
    source = EMOJI_LIBRARY[emojiCategory] || EMOJI_LIBRARY["Tous"];
  const rows = source.filter(
    ([emoji, keywords]) =>
      !query || normaliseEmojiSearch(`${emoji} ${keywords}`).includes(query),
  );
  $("#emoji-categories").innerHTML = Object.keys(EMOJI_LIBRARY)
    .map(
      (name) =>
        `<button type="button" class="${name === emojiCategory ? "active" : ""}" data-emoji-category="${escapeHtml(name)}">${escapeHtml(name)}</button>`,
    )
    .join("");
  $("#emoji-grid").innerHTML = rows
    .map(
      ([emoji, keywords]) =>
        `<button type="button" data-emoji-choice="${emoji}" title="${escapeHtml(keywords)}" aria-label="${escapeHtml(keywords)}">${emoji}</button>`,
    )
    .join("");
  $("#emoji-empty").hidden = rows.length > 0;
  $$("[data-emoji-category]").forEach(
    (button) =>
      (button.onclick = () => {
  ...
  $$("[data-emoji-choice]").forEach(
    (button) =>
      (button.onclick = () => {
        $("#emoji").value = button.dataset.emojiChoice;
...
function closeEmojiLibrary() {
  const library = $("#emoji-library"),
    button = $("#open-emoji-library");
  if (library) library.hidden = true;
  if (button) button.setAttribute("aria-expanded", "false");
}
function bindEmojiLibrary() {
  const allowed = ["building", "item"].includes(state.type),
    button = $("#open-emoji-library");
  if (!button) return;
  ...
  button.onclick = () => {
    const library = $("#emoji-library"),
      opening = library.hidden;
    library.hidden = !opening;
  ...
  $("#close-emoji-library").onclick = closeEmojiLibrary;
  $("#emoji-search").oninput = renderEmojiLibrary;
}
...
const input = (label, name, value = "", type = "text", extra = "") =>
  `<label>${label}<input data-field="${name}" data-help="${name}" type="${type}" value="${escapeHtml(value)}" ${extra}></label>`;
const select = (label, name, value, options) =>
  `<label>${label}<select data-field="${name}" data-help="${name}">${options.map(([key, text]) => `<option value="${key}" ${key === value ? "selected" : ""}>${text}</option>`).join("")}</select></label>`;
const check = (label, name, checked) =>
  `<label class="check"><input data-field="${name}" type="checkbox" ${checked ? "checked" : ""}><span>${label}</span></label>`;
...
function setHelp(key) {
  const [title, text, examples = []] = HELP[key] || [
    "Option modifiable",
    "Cette option peut être ajustée maintenant ou plus tard.",
    [],
  ];
  $("#help-title").textContent = title;
  $("#help-text").textContent = text;
  $("#help-list").innerHTML = examples
...
async function load() {
  ...
  if (state.type !== "live_world" && state.liveAbort) {
  ...
  if (state.type === "academy") {
    await KingdomTutorials.renderAcademy();
    installReferenceAcademyCard();
    return;
  }
  $("#content-stats").hidden = false;
  ...
  if (state.type === "voice_presence") {
    await loadVoicePresenceStudio();
    return;
  }
  if (state.type === "bot") {
    await loadDiscordConnections();
    return;
  }
  if (state.type === "location") {
    await loadLocationWorld();
    return;
  }
  restoreContentToolbar();
  const response = await fetch(`/api/content?entity_type=${state.type}`, {
    headers,
  });
  if (!response.ok) {
    alert("Accès refusé ou API indisponible.");
    return;
  }
...
async function loadCatalogs() {
  const [
    ...
    buildingsResponse,
    interfacesResponse,
    enrichedItemsResponse,
    audioResponse,
    audioGroupsResponse,
    audioStoriesResponse,
    voicePresencesResponse,
    ...
    fetch("/api/content?entity_type=audio_group", { headers }),
    fetch("/api/content?entity_type=audio_story", { headers }),
    fetch("/api/content?entity_type=voice_presence", { headers }),
    fetch("/api/content?entity_type=voice_profile", { headers }),
    fetch("/api/content?entity_type=bot", { headers }),
    fetch("/api/content?entity_type=location", { headers }),
    fetch("/api/content?entity_type=npc", { headers }),
    fetch("/api/content?entity_type=profession", { headers }),
    fetch("/api/content?entity_type=environment", { headers }),
  ]);
  ...
  if (buildingsResponse.ok)
    state.catalogs.building = await buildingsResponse.json();
  if (interfacesResponse.ok)
    state.catalogs.interface = await interfacesResponse.json();
  if (enrichedItemsResponse.ok)
    state.catalogs.itemEnriched = (await enrichedItemsResponse.json()).items;
  if (audioResponse.ok) state.catalogs.audio = await audioResponse.json();
  if (audioGroupsResponse.ok)
    state.catalogs.audio_group = await audioGroupsResponse.json();
  if (audioStoriesResponse.ok)
    state.catalogs.audio_story = await audioStoriesResponse.json();
  if (voicePresencesResponse.ok)
    state.catalogs.voice_presence = await voicePresencesResponse.json();
  if (voiceProfilesResponse.ok)
    state.catalogs.voice_profile = await voiceProfilesResponse.json();
  if (botsResponse.ok) state.catalogs.bot = await botsResponse.json();
  if (locationsResponse.ok)
    state.catalogs.location = await locationsResponse.json();
  if (npcsResponse.ok) state.catalogs.npc = await npcsResponse.json();
  if (professionsResponse.ok)
    state.catalogs.profession = await professionsResponse.json();
  if (environmentsResponse.ok)
    state.catalogs.environment = await environmentsResponse.json();
}

/* ==============================
   CARTE, LIEUX ET MONDE EN DIRECT
   ============================== */
function worldMapMarkup(geography, options = {}) {
  const nodes = geography.nodes || [],
    settings = options.settings || {},
    width = Math.max(800, Number(settings.width) || 1600),
    height = Math.max(500, Number(settings.height) || 900);
  const positions = new Map(
    nodes.map((node, index) => {
      const savedX = Number(node.map?.x),
        savedY = Number(node.map?.y),
        placed =
      ...
      return [
        node.key,
        {
          x: placed ? savedX : 100 + (index % 5) * 260,
          y: placed ? savedY : 80 + Math.floor(index / 5) * 170,
        },
      ];
    }),
  );
  const hierarchyLinks = (geography.hierarchy || []).length
    ? geography.hierarchy
    : nodes
        .filter((node) => node.parent_key && positions.has(node.parent_key))
        .map((node) => ({
          origin: node.key,
          target: node.parent_key,
          name: "Appartient à",
        }));
  const hierarchy = hierarchyLinks
    .map((link) => {
      const a = positions.get(link.origin),
        b = positions.get(link.target);
      if (!a || !b) return "";
      return `<g class="world-hierarchy-link" data-map-route data-map-relation="parent" data-origin="${escapeHtml(link.origin)}" data-target="${escapeHtml(link.target)}"><line x1="${a.x + 90}" y1="${a.y + 42}" x2="${b.x + 90}" y2="${b.y + 42}"/><text x="${(a.x + b.x) / 2 + 90}" y="${(a.y + b.y) / 2 + 34}">${escapeHtml(link.name || "Appartient à")}</text></g>`;
    })
    .join("");
  const routes = (geography.connections || [])
    .map((route) => {
      const a = positions.get(route.origin),
        b = positions.get(route.target);
      if (!a || !b) return "";
      return `<g class="world-route-link" data-map-route data-map-relation="route" data-origin="${escapeHtml(route.origin)}" data-target="${escapeHtml(route.target)}"><line x1="${a.x + 90}" y1="${a.y + 42}" x2="${b.x + 90}" y2="${b.y + 42}"/><text x="${(a.x + b.x) / 2 + 90}" y="${(a.y + b.y) / 2 + 34}">${escapeHtml(route.name)}</text></g>`;
    })
    .join("");
  ...
  const map = `<div class="world-map ${options.interactive ? "is-editable" : ""}" data-map-width="${width}" data-map-height="${height}" style="width:${width}px;height:${height}px;${background}"><svg viewBox="0 0 ${width} ${height}" aria-label="Relations et chemins du monde">${hierarchy}${routes}</svg>${nodes
    .map((node) => {
      ...
      return `<button type="button" class="world-node" data-world-node="${escapeHtml(node.key)}" style="left:${point.x}px;top:${point.y}px" ${options.interactive ? 'aria-label="Déplacer ' + escapeHtml(node.name || node.key) + '"' : ""}><span>${escapeHtml(node.emoji || "📍")}</span><b>${escapeHtml(node.name || node.key)}</b><small>${escapeHtml(node.location_type || "lieu")}</small>${
        buildings.length
          ? `<i class="world-node-buildings">${buildings
              .slice(0, 4)
              .map(
                (building) =>
                  `<em title="${escapeHtml(building.name)}">${escapeHtml(building.emoji || "🏰")}</em>`,
              )
              .join(
                "",
              )}${buildings.length > 4 ? `<em>+${buildings.length - 4}</em>` : ""}</i>`
          : ""
      }</button>`;
    })
...
async function saveMappedEntity(type, entity, payload) {
  const response = await fetch(
    `/api/content/${type}/${encodeURIComponent(entity.entity_key)}`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({
        ...
        author: "world-map-editor",
      }),
    },
  );
  const saved = await response.json();
  if (!response.ok) throw Error(saved.detail || "Enregistrement impossible.");
  if (entity.status === "published") {
    const publication = await fetch(
      `/api/content/${type}/${encodeURIComponent(entity.entity_key)}/${saved.version}/publish`,
      { method: "POST", headers, body: "{}" },
    );
    if (!publication.ok)
      throw Error(
        (await publication.json()).detail || "Publication impossible.",
      );
    saved.status = "published";
  }
  Object.assign(entity, saved, { payload });
  return saved;
}

function updateMapConnections() {
  $$("[data-map-route]").forEach((route) => {
    const a = $(`[data-world-node="${CSS.escape(route.dataset.origin)}"]`),
      b = $(`[data-world-node="${CSS.escape(route.dataset.target)}"]`);
    if (!a || !b) return;
    const x1 = parseFloat(a.style.left) + a.offsetWidth / 2,
      y1 = parseFloat(a.style.top) + a.offsetHeight / 2,
      x2 = parseFloat(b.style.left) + b.offsetWidth / 2,
      y2 = parseFloat(b.style.top) + b.offsetHeight / 2;
    const line = route.querySelector("line"),
      label = route.querySelector("text");
...
async function updateWorldMapSettings(configuration) {
  const response = await fetch("/api/world/map/settings", {
      method: "POST",
      headers,
...
async function applyWorldMapBackground(
  settings = {},
  selector = ".interactive-map-viewport .world-map",
) {
  const map = $(selector);
  if (!map) return;
  if (state.worldMapBackgroundUrl) {
    URL.revokeObjectURL(state.worldMapBackgroundUrl);
    state.worldMapBackgroundUrl = "";
  }
  ...
  const response = await fetch(
    `/api/world/map/background?v=${encodeURIComponent(settings.background_version || Date.now())}`,
    { headers, cache: "no-store" },
  );
  if (!response.ok)
    throw Error("Le paysage enregistré ne peut pas être chargé.");
  state.worldMapBackgroundUrl = URL.createObjectURL(await response.blob());
  map.style.backgroundImage = `linear-gradient(#06100b44,#06100b44),url("${state.worldMapBackgroundUrl}")`;
  map.style.backgroundSize = "cover";
  map.style.backgroundPosition = "center";
  map.style.backgroundRepeat = "no-repeat";
}

function bindWorldMapEditor(geography, mapSettings) {
  const viewport = $(".world-map-scroll"),
    map = $(".world-map"),
    shell = $(".world-map-zoom-shell"),
    zoomLabel = $("#map-zoom-label"),
    status = $("#map-editor-status");
  const setStatus = (text, kind = "") => {
    status.textContent = text;
    status.dataset.kind = kind;
  };
  const applyZoom = (value) => {
    state.worldMapZoom = Math.min(2, Math.max(0.35, Number(value.toFixed(2))));
    $(".world-map-transform").style.transform = `scale(${state.worldMapZoom})`;
    shell.style.width = `${Number(map.dataset.mapWidth) * state.worldMapZoom}px`;
    shell.style.height = `${Number(map.dataset.mapHeight) * state.worldMapZoom}px`;
    zoomLabel.textContent = `${Math.round(state.worldMapZoom * 100)} %`;
  };
  $("#map-zoom-out").onclick = () => applyZoom(state.worldMapZoom - 0.15);
  $("#map-zoom-in").onclick = () => applyZoom(state.worldMapZoom + 0.15);
  $("#map-zoom-reset").onclick = () => applyZoom(1);
  $("#map-zoom-fit").onclick = () =>
    applyZoom(
      Math.min(1, (viewport.clientWidth - 24) / Number(map.dataset.mapWidth)),
    );
  viewport.addEventListener(
    ...
    (event) => {
      ...
      applyZoom(state.worldMapZoom + (event.deltaY < 0 ? 0.1 : -0.1));
    },
    { passive: false },
  );
  $$("[data-world-node]").forEach((node) => {
    let drag = null,
      moved = false;
    node.onpointerdown = (event) => {
      ...
      drag = {
        dx:
          (event.clientX - rect.left) / state.worldMapZoom -
          parseFloat(node.style.left),
        dy:
          (event.clientY - rect.top) / state.worldMapZoom -
    ...
    node.onpointermove = (event) => {
      ...
      const rect = map.getBoundingClientRect(),
        x = Math.max(
          20,
          Math.min(
            Number(map.dataset.mapWidth) - node.offsetWidth - 20,
            (event.clientX - rect.left) / state.worldMapZoom - drag.dx,
          ),
        ),
        y = Math.max(
          20,
          Math.min(
            Number(map.dataset.mapHeight) - node.offsetHeight - 20,
            (event.clientY - rect.top) / state.worldMapZoom - drag.dy,
          ),
        );
    ...
    node.onpointerup = async (event) => {
      ...
      state.worldMapDragged = moved;
      if (!moved) return;
      const entity = state.items.find(
        (item) => item.entity_key === node.dataset.worldNode,
      );
      if (!entity) return;
      setStatus(`Enregistrement de ${entity.payload.name}…`);
      try {
        ...
        await saveMappedEntity("location", entity, payload);
        setStatus(`✓ ${entity.payload.name} repositionné`, "success");
      } catch (error) {
        setStatus(error.message, "error");
        await loadLocationWorld();
      }
      setTimeout(() => (state.worldMapDragged = false), 50);
    };
    node.onclick = () => {
      if (state.worldMapDragged) return;
      const entity = state.items.find(
        (item) => item.entity_key === node.dataset.worldNode,
      );
    ...
    node.ondragover = (event) => {
      event.preventDefault();
      node.classList.add("building-drop-target");
    };
    node.ondragleave = () => node.classList.remove("building-drop-target");
    node.ondrop = async (event) => {
      event.preventDefault();
      node.classList.remove("building-drop-target");
      const key = event.dataTransfer.getData("application/x-kingdom-building"),
        entity = state.catalogs.building.find(
          (item) => item.entity_key === key,
        );
      if (!entity) return;
      setStatus(`Ancrage de ${entity.payload.name}…`);
      try {
        const payload = clone(entity.payload);
        payload.location_key = node.dataset.worldNode;
        await saveMappedEntity("building", entity, payload);
        setStatus(
          `✓ ${entity.payload.name} ancré à ${node.querySelector("b").textContent}`,
          "success",
        );
        await loadLocationWorld();
      } catch (error) {
  ...
  $$("[data-map-building]").forEach((card) => {
    card.ondragstart = (event) => {
      event.dataTransfer.setData(
        "application/x-kingdom-building",
        card.dataset.mapBuilding,
      );
  ...
  $("#map-background-input").onchange = async (event) => {
    const file = event.target.files[0];
    if (!file) return;
    setStatus("Import du paysage…");
    const form = new FormData();
    form.append("file", file);
    try {
      const response = await fetch("/api/world/map/background", {
          method: "POST",
          headers: multipartHeaders(),
          body: form,
        }),
        result = await response.json();
      if (!response.ok) throw Error(result.detail || "Import impossible.");
      setStatus("✓ Paysage enregistré", "success");
      await loadLocationWorld();
    } catch (error) {
      setStatus(error.message, "error");
    }
  };
  $("#remove-map-background").onclick = async () => {
    try {
      await updateWorldMapSettings({ ...mapSettings, background_path: "" });
      await loadLocationWorld();
    } catch (error) {
      setStatus(error.message, "error");
    }
  };
  $("#save-map-size").onclick = async () => {
    try {
      await updateWorldMapSettings({
        ...mapSettings,
...
function worldImpactsMarkup(impactData = { impacts: [] }) {
  const impacts = impactData?.impacts || [];
  return `<section class="royal-panel world-impacts"><div class="royal-panel-title"><small>IMPACTS ACTUELS</small><span>${impacts.length} valeur(s) modifiée(s)</span></div>${impacts.map((impact) => `<article class="impact-row"><div><b>${escapeHtml(impact.building_name)} · ${escapeHtml(impact.subject_name)}</b><small>${escapeHtml(impact.label)}</small></div><span>Base ${impact.base} → <strong>${impact.effective}</strong></span><small>${impact.modifiers.map((modifier) => `${escapeHtml(modifier.source)} ${escapeHtml(modifier.operator)} ${modifier.value}`).join(" · ")}</small></article>`).join("") || '<p class="empty-admin">Aucune valeur gameplay n’est actuellement modifiée.</p>'}</section>`;
}

async function loadLocationWorld() {
  restoreContentToolbar();
  const [contentResponse, geoResponse, settingsResponse] = await Promise.all([
    fetch("/api/content?entity_type=location", { headers }),
    fetch("/api/world/geography", { headers }),
    fetch("/api/server/settings", { headers }),
  ]);
  if (!contentResponse.ok || !geoResponse.ok || !settingsResponse.ok) {
    alert("La géographie est indisponible.");
    return;
  }
  ...
  const geography = await geoResponse.json(),
    ...
    mapSettings = {
      ...(settingsEntity.payload.world_map || {}),
      background_version: settingsEntity.version,
    };
  $("#count").textContent = state.items.length;
  $("#published").textContent = state.items.filter(
    (item) => item.status === "published",
  ).length;
  $("#drafts").textContent = state.items.filter(
    (item) => item.status === "draft",
  ).length;
  const locationNames = new Map(
    state.items.map((item) => [item.entity_key, item.payload.name]),
  );
  ...
  $$("[data-open]").forEach(
    (button) =>
      (button.onclick = () => {
        const entity = state.items.find(
          (item) => item.entity_key === button.dataset.open,
        );
        if (entity) openEditor(entity);
      }),
  );
  $("#map-create-location").onclick = () => $("#new").click();
  $$("[data-publish-location]").forEach(
    (button) =>
      (button.onclick = () =>
        publishItem(
          button.dataset.publishLocation,
          Number(button.dataset.version),
        )),
  );
  $$("[data-delete-location]").forEach(
    (button) =>
      (button.onclick = async () => {
        if (!confirm("Supprimer ce lieu et archiver son historique ?")) return;
        const response = await fetch(
          `/api/content/location/${encodeURIComponent(button.dataset.deleteLocation)}`,
  ...
  bindWorldMapEditor(geography, mapSettings);
  applyWorldMapBackground(mapSettings).catch((error) => {
    $("#map-editor-status").textContent = error.message;
    $("#map-editor-status").dataset.kind = "error";
  });
}

function weatherVisualType(weather = {}) {
  const label =
    `${weather.key || ""} ${weather.name || ""} ${weather.emoji || ""}`
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .toLowerCase();
  if (/orage|storm|thunder|⛈/.test(label)) return "storm";
  if (/neige|snow|blizzard|❄|🌨/.test(label)) return "snow";
  if (/pluie|rain|bruine|averse|drizzle|🌧|🌦/.test(label)) return "rain";
  if (/brouillard|brume|fog|mist|🌫/.test(label)) return "fog";
  if (/nuag|cloud|couvert|☁|⛅/.test(label)) return "cloud";
  return "sun";
}

function weatherScene(weather = {}, compact = false) {
  const type = weatherVisualType(weather),
    particles =
      type === "rain" || type === "storm" ? 8 : type === "snow" ? 10 : 0;
  return `<span class="weather-scene weather-type-${type} ${compact ? "is-compact" : ""}" aria-hidden="true"><i class="weather-sun"></i><i class="weather-cloud cloud-one"></i><i class="weather-cloud cloud-two"></i><i class="weather-lightning"></i><i class="weather-fog fog-one"></i><i class="weather-fog fog-two"></i>${Array.from({ length: particles }, (_, index) => `<i class="weather-particle particle-${index + 1}"></i>`).join("")}</span>`;
}

async function loadLiveWorld(background = false) {
  ...
  $("#admin-view").innerHTML =
    `<div class="live-world"><section class="live-world-hero"><div><small>ROYAUME EN DIRECT</small><h2>Jour ${world.day} · ${String(world.hour).padStart(2, "0")}:${String(world.minute).padStart(2, "0")}</h2><p>${world.time_of_day === "night" ? "🌙" : world.time_of_day === "evening" ? "🌆" : "☀️"} ${periods[world.time_of_day] || escapeHtml(world.time_of_day)} · vitesse ×${world.speed}</p></div><div class="live-weather" data-weather="${weatherVisualType(weather)}">${weatherScene(weather)}<b>${escapeHtml(weather.name || weather.key || "Beau")}</b></div></section><section class="royal-metrics">${metricCard("LIEUX", world.world.locations)}${metricCard("CHEMINS", world.world.connections)}${metricCard("EN VOYAGE", (world.travels || []).length)}${metricCard(
      "JOUEURS POSITIONNÉS",
      (world.player_positions || []).reduce(
  ...
  const metrics = $("#admin-view .royal-metrics");
  const liveTitle = $("#admin-view .live-world-hero h2");
  if (liveTitle && world.date)
    liveTitle.textContent = `${world.date.day} ${world.date.month_name} — An ${world.date.year} · ${String(world.hour).padStart(2, "0")}:${String(world.minute).padStart(2, "0")}`;
  const livePeriod = $("#admin-view .live-world-hero p");
  if (livePeriod && world.season)
    livePeriod.insertAdjacentHTML(
      "beforeend",
      ` · ${escapeHtml(world.season.emoji || "")} ${escapeHtml(world.season.name)} · jour ${world.season.day}/${world.season.duration_days}`,
    );
  const livePresences = state.catalogs.voice_presence.filter(
    (item) =>
      item.status === "published" && item.payload.current_state === "active",
  );
  const mapPanel = $("#admin-view .world-map-panel");
  if (mapPanel) {
    mapPanel.insertAdjacentHTML(
      "beforebegin",
      `<section class="royal-panel live-voice-presences"><div class="royal-panel-title"><div><small>PRÉSENCES VOCALES</small><h2>Identités audibles maintenant</h2></div><button data-go="voice_presence">Gérer</button></div><div>${livePresences.map((item) => `<article><span>${item.payload.presence_type === "npc" ? "♙" : item.payload.presence_type === "ambience" ? "◖" : "◉"}</span><div><b>${escapeHtml(item.payload.name)}</b><small>${escapeHtml(state.catalogs.location.find((location) => location.entity_key === item.payload.location_key)?.payload.name || "Lieu dynamique")}</small></div></article>`).join("") || '<p class="empty-admin">Aucune présence vocale active. Le monde reste jouable sans audio.</p>'}</div></section>`,
    );
    bindNavigationShortcuts();
  }
  if (metrics && (world.forecasts || []).length) {
    const strip = document.createElement("section");
    strip.className = "weather-forecast-strip";
    strip.innerHTML = (world.forecasts || [])
      .map(
        (forecast, index) =>
          `<article data-weather="${weatherVisualType(forecast)}"><small>${index === 0 ? "AUJOURD’HUI" : index === 1 ? "DEMAIN" : `J+${index}`}</small>${weatherScene(forecast, true)}<b>${escapeHtml(forecast.name || forecast.key)}</b>${forecast.temperature_min !== undefined ? `<em>${forecast.temperature_min}° / ${forecast.temperature_max}°</em>` : ""}${forecast.rain_probability !== undefined ? `<small>Pluie ${forecast.rain_probability}%</small>` : ""}</article>`,
      )
      .join("");
  ...
  impactPromise
    .then(async (response) => {
      if (
        ...
        return;
      const panel = $("#admin-view .world-impacts");
      if (panel) panel.outerHTML = worldImpactsMarkup(await response.json());
    })
    .catch((error) => {
      if (!controller.signal.aborted)
        console.warn("Impacts du monde indisponibles", error);
    });
  state.adminTimer = setInterval(() => {
    if (state.type === "live_world" && state.viewRequest === requestId)
      loadLiveWorld(true);
  }, 5000);
}

async function loadCalendar(year = "", monthKey = "") {
  const requestId = state.viewRequest;
  showSystemView();
  const query = new URLSearchParams();
  if (year) query.set("year", year);
  if (monthKey) query.set("month_key", monthKey);
  const response = await fetch(`/api/world/calendar?${query}`, {
    headers,
    cache: "no-store",
  });
  if (state.type !== "calendar" || requestId !== state.viewRequest) return;
  if (!response.ok) {
    $("#admin-view").innerHTML =
      '<p class="empty-admin">Calendrier indisponible.</p>';
    return;
  }
  const data = await response.json(),
    definition = data.definition,
    month = definition.months.find((item) => item.key === data.month_key),
    firstOffset = data.days[0]?.weekday_index || 0;
  $("#admin-view").innerHTML =
    `<div class="calendar-page"><div class="calendar-module-tabs"><button type="button" class="active">Vue Calendrier</button><button type="button" data-calendar-config>Configuration du calendrier</button></div><section class="calendar-hero"><div><small>${escapeHtml(definition.name)}</small><h2>${escapeHtml(month?.name || data.month_key)} — AN ${data.year}</h2><p>${data.season ? `${escapeHtml(data.season.emoji || "")} ${escapeHtml(data.season.name)} · jour ${data.season.day}/${data.season.duration_days}` : "Aucune saison configurée"}</p></div><div class="calendar-nav"><select data-calendar-month>${definition.months.map((item) => `<option value="${escapeHtml(item.key)}" ${item.key === data.month_key ? "selected" : ""}>${escapeHtml(item.name)}</option>`).join("")}</select><input data-calendar-year type="number" value="${data.year}"><button type="button" data-calendar-go>Afficher</button></div></section><section class="calendar-grid" style="--week-days:${definition.weekdays.length}">${definition.weekdays.map((day) => `<b class="calendar-weekday">${escapeHtml(day)}</b>`).join("")}${Array.from({ length: firstOffset }, () => '<span class="calendar-empty"></span>').join("")}${data.days.map((day) => `<button type="button" class="calendar-day ${day.absolute_day === data.current.absolute_day ? "today" : ""}" data-calendar-day="${day.day}"><strong>${day.day}</strong>${day.forecast ? `<span>${escapeHtml(day.forecast.emoji || "☀️")} ${escapeHtml(day.forecast.name || day.forecast.key)}</span>` : ""}${day.events.map((event) => `<small>✦ ${escapeHtml(event.name)}</small>`).join("")}</button>`).join("")}</section><aside class="calendar-detail"><p>Sélectionnez une journée pour voir son contexte.</p></aside></div>`;
  $("[data-calendar-config]").onclick = loadCalendarConfiguration;
  $("[data-calendar-go]").onclick = () =>
    loadCalendar(
      Number($("[data-calendar-year]").value),
      $("[data-calendar-month]").value,
    );
  $$("[data-calendar-day]").forEach(
    (button) =>
      (button.onclick = () => {
        const day = data.days.find(
            (item) => item.day === Number(button.dataset.calendarDay),
          ),
          season = day
            ? calendarSeasonForDay(definition, data.year, day)
            : null;
        $(".calendar-detail").innerHTML =
          `<button type="button" class="dialog-close" aria-label="Fermer">×</button><small>${escapeHtml(day.weekday)}</small><h3>${day.day} ${escapeHtml(month.name)} — An ${data.year}</h3><p><b>Météo</b><br>${day.forecast ? `${escapeHtml(day.forecast.emoji || "")} ${escapeHtml(day.forecast.name || day.forecast.key)}` : "Au-delà des prévisions disponibles"}</p><p><b>Saison</b><br>${season ? `${escapeHtml(season.emoji || "")} ${escapeHtml(season.name)}` : "Non configurée"}</p><p><b>Événements</b><br>${day.events.map((event) => escapeHtml(event.name)).join("<br>") || "Aucun événement"}</p><button type="button" class="primary" data-day-event>Programmer un événement</button>`;
        $(".calendar-detail .dialog-close").onclick = () =>
          ($(".calendar-detail").innerHTML =
            "<p>Sélectionnez une journée pour voir son contexte.</p>");
        $(".calendar-detail [data-day-event]").onclick = () => {
          state.type = "event";
          activateNavigation($(`#nav [data-type="event"]`));
          load();
        };
      }),
  );
...
async function loadCalendarConfiguration() {
  if (mobileCreationBlocked())
    return showDesktopRequired("La configuration complète du calendrier");
  const requestId = state.viewRequest;
  showSystemView();
  await loadCatalogs();
  if (state.type !== "calendar" || requestId !== state.viewRequest) return;
  const entity =
      [...state.catalogs.environment].sort(
        (a, b) => Number(b.version) - Number(a.version),
      )[0] || null,
    payload = clone(entity?.payload || {});
  state.calendarConfigContext = { entity, payload };
  $("#admin-view").innerHTML =
    '<div class="calendar-page calendar-configuration"><div class="calendar-module-tabs"><button type="button" data-calendar-view>Vue Calendrier</button><button type="button" class="active">Configuration du calendrier</button></div><section class="royal-panel"><div class="section-head"><div><h2>Configuration du calendrier</h2><p>Le calendrier est autonome. Les événements, la météo ou l’horloge peuvent l’utiliser, mais aucune météo n’est nécessaire pour le créer.</p></div></div><form id="calendar-config-form"><div class="calendar-template-actions"><span>Point de départ facultatif :</span><button type="button" data-calendar-template="cycles">Cycles génériques</button><button type="button" data-calendar-template="terrestrial">Calendrier terrestre</button></div><div id="calendar-type-fields"></div><div class="calendar-config-actions"><span id="calendar-config-status"></span><button type="button" id="calendar-config-save" class="primary">Enregistrer le calendrier</button></div></form></section></div>';
  $("[data-calendar-view]").onclick = () => loadCalendar();
  try {
    renderCalendarConfig(payload, $("#calendar-type-fields"));
  } catch (error) {
    $("#calendar-config-status").textContent =
      `Éditeur indisponible : ${error.message}`;
  }
  $$("[data-calendar-template]").forEach(
    (button) =>
      (button.onclick = () => {
        if (
          !confirm(
            "Remplacer la configuration affichée par ce modèle ? La modification ne sera enregistrée qu’après votre confirmation.",
          )
        )
          return;
        const cycles = {
            name: "Cycles du monde",
            start_year: 1,
            start_month_key: "cycle_1",
            start_day: 1,
            weekdays: ["Phase 1", "Phase 2", "Phase 3", "Phase 4", "Phase 5"],
            months: [
              { key: "cycle_1", name: "Cycle 1", days: 30 },
              { key: "cycle_2", name: "Cycle 2", days: 30 },
              { key: "cycle_3", name: "Cycle 3", days: 30 },
            ],
            seasons: [],
          },
          terrestrial = {
            name: "Calendrier terrestre",
            start_year: 1,
            start_month_key: "month_01",
            start_day: 1,
            weekdays: [
              "Lundi",
              "Mardi",
            ...
            months: [
              ...
              key: `month_${String(index + 1).padStart(2, "0")}`,
              name,
              days,
            })),
            seasons: [],
        ...
        payload.calendar =
          button.dataset.calendarTemplate === "cycles" ? cycles : terrestrial;
        state.calendarConfigContext.payload = payload;
        const root = $("#calendar-type-fields");
        root.innerHTML = "";
        renderCalendarConfig(payload, root);
      }),
  );
}
async function saveCalendarConfiguration(button) {
  const { entity, payload } = state.calendarConfigContext || {};
  const status = $("#calendar-config-status");
  button.disabled = true;
  status.textContent = "Enregistrement…";
  try {
    const calendarPayload = clone(payload || {});
    calendarPayload.name = calendarPayload.name || "Environnement du monde";
    calendarPayload.emoji = calendarPayload.emoji || "🌐";
    calendarPayload.description =
      calendarPayload.description ||
      "Horloge, calendrier et conditions environnementales configurables.";
    calendarPayload.calendar = readCalendar(payload?.calendar || {});
    const key = entity?.entity_key || "world_environment";
    const savedResponse = await fetch(
      `/api/content/environment/${encodeURIComponent(key)}`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    if (!savedResponse.ok) throw Error((await savedResponse.json()).detail);
    const saved = await savedResponse.json();
    const publishResponse = await fetch(
      `/api/content/environment/${encodeURIComponent(key)}/${saved.version}/publish`,
      { method: "POST", headers, body: "{}" },
    );
    if (!publishResponse.ok) throw Error((await publishResponse.json()).detail);
    status.textContent = "Calendrier enregistré.";
    await loadCatalogs();
    await loadCalendar();
  } catch (error) {
...
function calendarSeasonForDay(definition, year, day) {
  const monthOffsets = new Map();
  let cursor = 0;
  definition.months.forEach((month) => {
    monthOffsets.set(month.key, cursor);
    cursor += Number(month.days);
  });
  const current = monthOffsets.get(day.month_key) + day.day - 1,
    starts = (definition.seasons || [])
      .map((season) => ({
        season,
        offset:
          (monthOffsets.get(season.start_month_key) || 0) +
          Number(season.start_day || 1) -
          1,
      }))
      .sort((a, b) => a.offset - b.offset);
  return (
    (
      [...starts].reverse().find((item) => item.offset <= current) ||
      starts.at(-1)
    )?.season || null
  );
}
...
/* ==============================
   COMPTES, MONDES ET SERVEURS
   ============================== */
async function initializeAccount() {
  let response = await fetch("/api/profile", { headers, cache: "no-store" });
  if (response.status === 403 && state.server) {
    state.server = "";
    localStorage.removeItem("kingdomServer");
    syncServerHeaders();
    response = await fetch("/api/profile", { headers, cache: "no-store" });
  }
  if (!response.ok) {
  ...
  if (!state.profile.servers.length) {
    state.server = "";
    localStorage.removeItem("kingdomServer");
    syncServerHeaders();
    renderAccountShell();
    ...
    state.type = "profile";
    $("#title").textContent = labels.profile;
    $("#crumb").textContent = labels.profile.toUpperCase();
    await loadProfile();
    return true;
  }
  const accessible = state.profile.servers.some(
...
function renderAccountShell() {
  ...
  const unassigned = !state.profile.servers.length;
  document.body.classList.toggle("account-unassigned", unassigned);
  selector.innerHTML = state.profile.servers
    .map(
      (server) =>
        `<option value="${escapeHtml(server.slug)}" ${server.slug === state.server ? "selected" : ""}>${escapeHtml(server.name)}</option>`,
    )
    .join("");
  selector.disabled = unassigned;
  const profileNavigation = $('#nav [data-type="profile"] span:last-child');
  ...
  $("#account-avatar").textContent = (
    ...
    .charAt(0)
    .toUpperCase();
  if ($("#menu-account-name"))
    $("#menu-account-name").textContent =
      account.display_name || account.username;
  if ($("#menu-account-avatar"))
    $("#menu-account-avatar").textContent = (
      account.display_name ||
      account.username ||
      "A"
    )
      ...
      .charAt(0)
      .toUpperCase();
  if ($("#platform-admin-entry"))
    $("#platform-admin-entry").hidden =
...
async function selectServer(slug, navigate = true) {
  ...
  state.server = slug;
  localStorage.setItem("kingdomServer", slug);
  syncServerHeaders();
  renderAccountShell();
  state.catalogs = {
    item: [],
  ...
  setSaveState("saving", "Changement de serveur…");
  await loadCatalogs();
  if (navigate) navigateTo("dashboard");
  else await load();
...
function serverProfileCard(server) {
  ...
  const canManage =
    state.profile.account.is_admin ||
    ["gestionnaire", "proprietaire", "administrateur"].includes(server.role);
  return `<article class="server-profile-card ${current ? "current" : ""}"><div class="server-profile-head"><div><h3>${escapeHtml(server.name)}</h3><p>${server.guild_id ? `Discord · ${escapeHtml(server.guild_id)}` : "Identifiant Discord à renseigner"}</p></div><span class="server-role">${escapeHtml(server.role)}</span></div><div class="server-meta"><span>Base indépendante · ${escapeHtml(server.slug)}</span><span>${server.bot_installed ? "KingdomCore présent sur Discord" : "KingdomEngine n’est pas encore installé"}</span></div><div class="bot-list">${server.bots.map((bot) => `<div class="bot-row"><span><b>${escapeHtml(bot.name)}</b><small>${escapeHtml(bot.type)}</small></span><i class="bot-state ${bot.installed && bot.available ? "" : "offline"}">${bot.installed && bot.available ? "Disponible" : bot.available ? "À installer" : "Application non configurée"}</i></div>`).join("") || "<p>Aucun bot publié.</p>"}</div><div class="profile-actions">${current ? "<button disabled>Serveur sélectionné</button>" : `<button class="secondary" data-select-server="${escapeHtml(server.slug)}">Gérer ce serveur</button>`}${canManage ? `<button class="primary" data-install-server="${escapeHtml(server.slug)}">${server.bot_installed ? "Mettre à jour l’installation" : "Installer KingdomEngine sur ce serveur"}</button>` : ""}${
    server.bot_installed
      ? server.bots
          .filter((bot) => bot.available && bot.type === "voice")
          .map(
            (bot) =>
              `<button class="secondary" data-profile-invite="${escapeHtml(bot.key)}" data-server="${escapeHtml(server.slug)}">Ajouter ${escapeHtml(bot.name)}</button>`,
          )
          .join("")
      : ""
  }${canManage ? `<button class="danger server-remove" data-remove-server="${escapeHtml(server.slug)}" data-server-name="${escapeHtml(server.name)}">Supprimer ce serveur</button>` : ""}</div><small class="server-operation-status" data-server-operation="${escapeHtml(server.slug)}"></small></article>`;
}

function worldPresetPicker() {
  const presets = state.worldPresets || [
    {
      key: "blank",
      name: "Monde vierge",
      emoji: "◇",
      description: "Commencer sans lieu ni mécanique imposée.",
      tone: "neutral",
    },
    {
      key: "medieval_kingdom",
      name: "Royaume médiéval",
      emoji: "🏰",
      description:
        "Village, forêt, mine, métiers, économie, météo et événement.",
      tone: "emerald",
    },
    {
      key: "royal_festival",
    ...
    },
    {
      ...
      emoji: "🛰️",
      description:
        "Station, exploration, ingénierie, crédits et phénomènes spatiaux.",
      tone: "violet",
    },
  ];
...
async function loadProfile() {
  clearInterval(state.adminTimer);
  $("#content-stats").hidden = true;
  $("#content-workspace").hidden = true;
  ...
  if (account.platform_role === "platform_admin") {
    const result = await fetch("/api/accounts", { headers });
    if (result.ok) accounts = (await result.json()).accounts;
  }
  const [foundationsResponse, supportResponse, presetsResponse] =
    await Promise.all([
      fetch("/api/product/foundations", { headers }),
      fetch("/api/support/grants", { headers }),
      fetch("/api/world-presets", { headers }),
    ]);
  const foundations = foundationsResponse.ok
...
function supportModeMarkup(worlds, grants) {
  return `<section class="accounts-panel support-mode-panel"><div class="accounts-title"><div><small>ASSISTANCE RESPECTUEUSE DE VOS DONNÉES</small><h3>Support Mode</h3><p>Autorisez temporairement Payen Studio à consulter uniquement les diagnostics choisis. Aucun token, conversation privée ou secret n'est inclus.</p></div></div><form id="support-mode-form" class="support-mode-form"><label>Monde<select name="world_slug">${worlds.map((world) => `<option value="${escapeHtml(world.slug)}">${escapeHtml(world.name)}</option>`).join("")}</select></label><label>Durée<select name="duration_minutes"><option value="30">30 minutes</option><option value="60" selected>1 heure</option><option value="240">4 heures</option><option value="1440">24 heures</option></select></label><fieldset><legend>Diagnostics autorisés</legend>${[
    ...
    .map(
      ([key, label]) =>
        `<label class="check"><input type="checkbox" name="scope" value="${key}" ${key === "diagnostics" || key === "service_health" ? "checked" : ""}><span>${label}</span></label>`,
    )
    .join(
      "",
    )}</fieldset><button class="primary" ${worlds.length ? "" : "disabled"}>Autoriser temporairement</button><small data-support-status></small></form><div class="support-grants">${grants.map((grant) => `<article><div><b>${escapeHtml(grant.world_slug)}</b><small>${grant.status === "active" ? `Expire le ${formatDate(grant.expires_at)}` : escapeHtml(grant.status)} · ${grant.scopes.map(escapeHtml).join(", ")}</small></div>${grant.status === "active" ? `<button type="button" class="danger-link" data-revoke-support="${escapeHtml(grant.grant_id)}">Révoquer maintenant</button>` : ""}</article>`).join("") || '<p class="field-note">Aucune autorisation active.</p>'}</div></section>`;
}
function bindSupportMode() {
  ...
  form.onsubmit = async (event) => {
    ...
    status.textContent = response.ok
      ? "Assistance autorisée et auditée."
      : result.detail;
    if (response.ok) await loadProfile();
  };
  $$("[data-revoke-support]").forEach(
    (button) =>
      (button.onclick = async () => {
        if (!confirm("Révoquer immédiatement cet accès d'assistance ?")) return;
        const response = await fetch(
          `/api/support/grants/${button.dataset.revokeSupport}`,
          { method: "DELETE", headers },
        );
        if (!response.ok) {
...
async function createManagedServer(event) {
  ...
  try {
    ...
    const result = await response
      .json()
      .catch(() => ({
        detail: `Le serveur a répondu avec l’erreur ${response.status}.`,
      }));
    if (!response.ok)
      throw new Error(result.detail || "Impossible de créer ce serveur.");
    status.textContent = result.reactivated
      ? "Ancien monde restauré avec ses données. Vous pouvez maintenant réinstaller KingdomEngine sur Discord."
      : `${result.seeded_entities || 0} éléments préparés. Ouverture du Studio…`;
    state.profile = null;
    state.server = result.slug;
    localStorage.setItem("kingdomServer", state.server);
    syncServerHeaders();
    ...
    status.textContent =
      error?.message ||
      "La création a échoué. Vérifiez la connexion au serveur.";
    if (button) button.disabled = false;
  }
}

function adminAccountsPanel(accounts) {
  const serverOptions = state.profile.servers
    .map(
      (server) =>
        `<option value="${escapeHtml(server.slug)}">${escapeHtml(server.name)}</option>`,
    )
    .join("");
  const administrators = accounts.filter((item) => item.is_admin).length,
    managed = accounts.reduce(
      (sum, item) => sum + Number(item.administered_server_count || 0),
      0,
    );
  return `<section class="accounts-panel"><div class="accounts-title"><div><small>ADMINISTRATION GLOBALE</small><h3>Comptes KingdomWeb</h3><p>Suivez les inscriptions, attribuez les serveurs et supprimez proprement les profils inactifs. Leurs bases KingdomData sont préservées.</p></div></div><div class="account-metrics"><article><small>COMPTES CRÉÉS</small><b>${accounts.length}</b></article><article><small>ADMINISTRATEURS GLOBAUX</small><b>${administrators}</b></article><article><small>SERVEURS ADMINISTRÉS</small><b>${managed}</b><span>attributions de gestion</span></article></div><div class="admin-forms"><form id="create-account-form"><b>Nouveau profil</b><label>Identifiant<input name="username" required minlength="3"></label><label>Nom affiché<input name="display_name" required></label><label>E-mail<input name="email" type="email"></label><label>Mot de passe temporaire<input name="password" type="password" minlength="8" required></label><label>Serveur<select name="server_slug">${serverOptions}</select></label><label>Accès<select name="role"><option value="lecture">Lecture</option><option value="editeur">Éditeur</option><option value="gestionnaire">Gestionnaire</option><option value="proprietaire">Propriétaire</option></select></label><button class="primary">Créer le profil</button><small data-form-status></small></form></div><div class="account-table-scroll"><table class="account-table"><thead><tr><th>Profil</th><th>Création</th><th>Type</th><th>Serveurs administrés</th><th>Accès actuels</th><th>Administration</th></tr></thead><tbody>${accounts.map((item) => `<tr><td><b>${escapeHtml(item.display_name)}</b><small>@${escapeHtml(item.username)}${item.email ? ` · ${escapeHtml(item.email)}` : ""}</small></td><td>${formatDate(item.created_at)}</td><td>${item.is_admin ? "Administrateur global" : "Utilisateur"}</td><td><strong class="server-count">${item.administered_server_count || 0}</strong><small>${item.server_count || 0} serveur(s) accessible(s)</small></td><td><div class="access-list">${(item.access || []).map((access) => `<span>${escapeHtml(access.name)} · ${escapeHtml(access.role)} ${item.is_admin ? "" : `<button type="button" title="Retirer cet accès" data-revoke-access="${item.id}|${escapeHtml(access.slug)}">×</button>`}</span>`).join("") || "Aucun accès attribué"}</div></td><td>${item.is_admin ? "Utilisez sa fiche de sécurité" : `<div class="account-admin-actions"><div class="access-editor"><select data-access-server>${serverOptions}</select><select data-access-role><option value="lecture">Lecture</option><option value="editeur">Éditeur</option><option value="gestionnaire">Gestionnaire</option><option value="proprietaire">Propriétaire</option></select><button type="button" data-grant-access="${item.id}">Attribuer</button></div><details><summary>Réinitialiser le mot de passe</summary><div class="password-reset-editor"><input type="password" minlength="8" autocomplete="new-password" placeholder="Nouveau mot de passe temporaire" data-reset-password="${item.id}"><button type="button" class="secondary" data-reset-account="${item.id}">Réinitialiser</button><small data-reset-status="${item.id}"></small></div></details><button type="button" class="danger-link" data-delete-account="${item.id}" data-account-username="${escapeHtml(item.username)}">Supprimer ce compte</button></div>`}</td></tr>`).join("")}</tbody></table></div></section>`;
}

function bindAccountAdministration() {
  $("#create-account-form").onsubmit = async (event) => {
    event.preventDefault();
    const form = event.currentTarget,
      data = Object.fromEntries(new FormData(form));
    data.access = [{ server_slug: data.server_slug, role: data.role }];
    delete data.server_slug;
    delete data.role;
    ...
    form.querySelector("[data-form-status]").textContent = response.ok
      ? "Profil créé."
      : (await response.json()).detail;
    if (response.ok) await loadProfile();
  };
  $$("[data-grant-access]").forEach(
    (button) =>
      (button.onclick = async () => {
        const editor = button.closest(".access-editor"),
          body = {
            server_slug: editor.querySelector("[data-access-server]").value,
            role: editor.querySelector("[data-access-role]").value,
          };
        const response = await fetch(
          `/api/accounts/${button.dataset.grantAccess}/access`,
  ...
  $$("[data-revoke-access]").forEach(
    (button) =>
      (button.onclick = async () => {
        const [accountId, serverSlug] = button.dataset.revokeAccess.split("|");
        if (!confirm("Retirer l’accès de ce profil à ce serveur ?")) return;
        const response = await fetch(
          `/api/accounts/${accountId}/access/${encodeURIComponent(serverSlug)}`,
          { method: "DELETE", headers },
        );
        if (!response.ok) {
          alert((await response.json()).detail);
  ...
  $$("[data-reset-account]").forEach(
    (button) =>
      (button.onclick = async () => {
        const accountId = button.dataset.resetAccount,
          input = $(`[data-reset-password="${accountId}"]`),
          status = $(`[data-reset-status="${accountId}"]`),
          newPassword = input.value;
        if (newPassword.length < 8) {
          status.textContent = "8 caractères minimum.";
          return;
        }
        if (
          !confirm(
            "Réinitialiser ce mot de passe et fermer toutes les sessions de ce compte ?",
          )
        )
          return;
        button.disabled = true;
        const response = await fetch(`/api/accounts/${accountId}/password`, {
          ...
          result = await response.json();
        status.textContent = response.ok ? result.message : result.detail;
  ...
  $$("[data-delete-account]").forEach(
    (button) =>
      (button.onclick = async () => {
        ...
        if (
          prompt(
            `Supprimer définitivement le compte @${username} ?\n\nSes sessions et accès seront retirés. Ses bases KingdomData seront conservées afin de pouvoir réinstaller ses anciens serveurs.\n\nSaisissez « ${username} » pour confirmer.`,
          ) !== username
        )
          return;
        button.disabled = true;
...
async function inviteBotForServer(botKey, serverSlug) {
  const popup = window.open("about:blank", "_blank");
  const response = await fetch(
      `/api/bots/${encodeURIComponent(botKey)}/invite`,
      { headers: { ...headers, "X-Kingdom-Server": serverSlug } },
    ),
...
async function removeManagedServer(slug, name) {
  if (
    prompt(
      `Cette action va retirer les bots vocaux, les salons et rôles KingdomEngine, puis KingdomCore du serveur Discord.\n\nLe monde, ses joueurs, ses contenus et sa base KingdomData seront supprimés définitivement. Une nouvelle installation avec ce serveur repartira de zéro.\n\nSaisissez « ${name} » pour confirmer.`,
    ) !== name
  )
    return;
  const status = $(`[data-server-operation="${slug}"]`),
    buttons = $$(
      `[data-remove-server="${slug}"],[data-install-server="${slug}"]`,
    );
  buttons.forEach((button) => (button.disabled = true));
  status.textContent = "Désinstallation Discord demandée à KingdomCore…";
  let response = await fetch(
      `/api/servers/${encodeURIComponent(slug)}/uninstall`,
      { method: "POST", headers, body: "{}" },
    ),
    result = await response.json();
  if (!response.ok) {
  ...
  if (result.status !== "not_installed") {
    ...
    while (Date.now() - started < 120000) {
      ...
      response = await fetch(
        `/api/servers/${encodeURIComponent(slug)}/operation`,
        { headers, cache: "no-store" },
      );
      result = await response.json();
      if (!response.ok || result.status === "failed") {
        status.textContent =
          result.detail ||
          `Désinstallation interrompue : ${result.error || "erreur Discord"}`;
        buttons.forEach((button) => (button.disabled = false));
        return;
      }
      if (result.scope === "uninstall" && result.status === "done") break;
      status.textContent = "KingdomCore retire les composants Discord…";
    }
    if (result.status !== "done") {
      status.textContent =
        "Délai dépassé. Vérifiez KingdomCore avant de réessayer.";
      buttons.forEach((button) => (button.disabled = false));
      return;
    }
  }
  status.textContent = "Discord nettoyé. Suppression définitive des données…";
  response = await fetch(`/api/servers/${encodeURIComponent(slug)}`, {
    method: "DELETE",
    headers,
  });
  result = await response.json();
...
function catalogOptions(type, currentValue = "") {
  ...
  return [
    [
      "",
      {
        item: "Choisir une ressource…",
        event: "Choisir un événement…",
        building: "Choisir un bâtiment…",
        interface: "Choisir une interface…",
      }[type] || "Choisir…",
    ],
    ...options,
  ];
...
function renderCards() {
  ...
  const items = state.items.filter((item) =>
    `${item.entity_key} ${item.payload.name}`.toLowerCase().includes(query),
  );
  $("#count").textContent = state.items.length;
  $("#published").textContent = state.items.filter(
    (item) => item.status === "published",
  ...
  $$("#cards [data-edit]").forEach(
    (button) =>
      (button.onclick = async (event) => {
        ...
        event.stopPropagation();
        const entity = state.items.find(
          (item) => item.entity_key === button.dataset.edit,
        );
        if (!entity)
          return alert(
            "Ce bâtiment n'est plus présent dans le catalogue. Actualisez la page.",
          );
        try {
          await openEditor(entity);
        } catch (error) {
          ...
          alert(
            `Impossible d'ouvrir l'éditeur : ${error.message || "erreur inconnue"}`,
          );
        }
      }),
  );
...
async function loadDiscordConnections() {
  ...
  if (!contentResponse.ok) {
    alert("Les connexions Discord sont indisponibles.");
    return;
  }
  state.items = await contentResponse.json();
...
function discordConnectionCard(entity) {
  ...
  return `<article class="discord-connection-card ${voice ? "audio-connection" : "core-connection"} ${locked ? "is-locked" : ""}" data-open="${escapeHtml(entity.entity_key)}" data-access-tier="${locked ? "locked" : "included"}" tabindex="0"><div class="discord-connection-icon">${escapeHtml(payload.emoji || (voice ? "🎙️" : "🛡️"))}</div><div class="discord-connection-copy"><small>${voice ? protectedWorker ? "VOICE WORKER DE BASE" : "VOICE WORKER PERSONNEL" : "CONNEXION PRINCIPALE"}</small><h3>${escapeHtml(payload.name || entity.entity_key)}</h3><p>${escapeHtml(payload.description || (voice ? "Capacité technique réutilisée par les identités de Voix et Présences." : "Pilote les interfaces textuelles du monde."))}</p><div class="discord-connection-meta"><span class="${applicationReady ? "ready" : "missing"}">${applicationReady ? "✓ Application configurée" : "⚠ Application ID à renseigner"}</span><span class="${tokenReady ? "ready" : "missing"}">${tokenReady ? "✓ Token configuré" : "⚠ Token à renseigner"}</span>${voice ? `<span>${locked ? `🔒 Offre ${escapeHtml(status.voice_plan || "Basic")}` : protectedWorker ? "🔒 Géré par KingdomEngine · non modifiable" : "Modifiable et supprimable"}</span>` : ""}</div></div><div class="discord-connection-actions"><span class="connection-readiness ${locked ? "missing" : applicationReady && tokenReady ? "ready" : "missing"}">${locked ? "Verrouillé par l’offre" : readiness}</span><button type="button" class="primary" data-invite="${escapeHtml(entity.entity_key)}" ${applicationReady && !locked ? "" : "disabled"}>${voice ? "Ajouter à Discord" : "Installer sur Discord"}</button>${!locked && !protectedWorker ? `<button type="button" data-edit="${escapeHtml(entity.entity_key)}">Configurer le Worker</button><button type="button" data-worker-credentials="${escapeHtml(entity.entity_key)}">Token et Application ID</button>` : ""}${!protectedWorker && voice && !locked ? `<button type="button" class="danger" data-delete="${escapeHtml(entity.entity_key)}">Supprimer</button>` : ""}${entity.status === "draft" ? `<button type="button" data-publish="${escapeHtml(entity.entity_key)}" data-version="${entity.version}">Publier sur Discord</button>` : ""}</div></article>`;
}

function openWorkerCredentials(key) {
  const dialog = document.createElement("dialog");
  dialog.className = "worker-credentials-dialog";
  dialog.innerHTML = `<form method="dialog"><header><div><small>VOICE WORKER PERSONNEL</small><h2>Connexion Discord</h2><p>Ces identifiants sont enregistrés côté serveur et ne seront jamais réaffichés.</p></div><button value="cancel" aria-label="Fermer">×</button></header><label>Application ID<input name="application_id" inputmode="numeric" pattern="[0-9]+" required autocomplete="off"></label><label>Token du bot<input name="token" type="password" minlength="20" required autocomplete="new-password"></label><p class="field-note">Après l’enregistrement, KingdomVoice devra être redémarré.</p><small data-credentials-status></small><footer><button value="cancel">Annuler</button><button type="submit" class="primary" value="save">Enregistrer les identifiants</button></footer></form>`;
  document.body.append(dialog);
  const form = dialog.querySelector("form");
  form.addEventListener("submit", async (event) => {
    if (event.submitter?.value !== "save") return;
    event.preventDefault();
    const response = await fetch(`/api/bots/${encodeURIComponent(key)}/credentials`, {
      method: "POST", headers: { ...headers, "Content-Type": "application/json" },
      body: JSON.stringify(Object.fromEntries(new FormData(form))),
    });
    const result = await response.json();
    const status = dialog.querySelector("[data-credentials-status]");
    status.textContent = result.message || result.detail;
    if (response.ok) setTimeout(() => { dialog.close(); dialog.remove(); loadDiscordConnections(); }, 900);
  });
...
function renderDiscordConnections() {
  const query = $("#search").value.toLowerCase(),
    items = state.items.filter((item) =>
      `${item.entity_key} ${item.payload.name || ""}`
        .toLowerCase()
        .includes(query),
    );
  const core = items.filter((item) => item.payload.bot_type !== "voice"),
    audio = items.filter((item) => item.payload.bot_type === "voice"),
    audioTotal = state.items.filter(
      (item) => item.payload.bot_type === "voice",
    ).length;
  $("#count").textContent = state.items.length;
  $("#published").textContent = state.items.filter(
    (item) => item.status === "published",
...
async function loadProfessionCatalog() {
  const response = await fetch("/api/world/professions", { headers });
  if (!response.ok) {
    alert("Catalogue des métiers indisponible.");
    return;
  }
  const professions = await response.json();
  state.items = professions;
  $("#count").textContent = professions.length;
  $("#published").textContent = professions.filter(
    (item) => item.buildings.length,
  ).length;
  $("#drafts").textContent = professions.filter(
    (item) => !item.buildings.length,
  ).length;
  $("#cards").innerHTML =
    professions
...
function buildingCardMarkup(item) {
  const modules = item.payload.modules || {},
    professions = modules.professions || [],
    bot = state.catalogs.bot.find(
      (entity) => entity.payload.building_key === item.entity_key,
    ),
    productionCount =
...
function resetEditor() {
  const definitionStep = $("#definition-step"),
    typeFields = $("#type-fields");
  [
    definitionStep?.querySelector(".common-fields.form-section"),
    definitionStep?.querySelector("details.common-fields"),
  ]
    .filter(Boolean)
  ...
  state.buildingBase = null;
  state.editorDirty = false;
  state.interfaceDraft = null;
  state.selectedPage = null;
  state.selectedComponent = null;
  $(".wizard-panel").classList.remove(
    "visual-mode",
    "building-mode",
    "event-mode",
    "item-mode",
...
function installReferenceAcademyCard() {
  ...
  if (!academy || academy.querySelector("[data-open-reference-building]"))
    return;
  academy
    .querySelector(".academy-hero")
    ?.insertAdjacentHTML(
      "afterend",
      `<section class="academy-reference-building"><div class="academy-reference-heading"><div class="academy-reference-icon">🎓</div><div><small>LABORATOIRE PÉDAGOGIQUE · HORS DE VOS SERVEURS</small><h3>Atelier-école no-code</h3><p>Suivez le parcours d'un joueur, puis inspectez exactement les réglages qui le rendent possible dans le véritable éditeur KingdomWeb.</p></div></div><div class="academy-reference-feature-grid"><article><b>⚒️ 2 métiers</b><span>Niveaux, outil offert et exclusivité royaume</span></article><article><b>🗺️ 2 zones</b><span>Verrouillage, durée, énergie, usure et cooldown</span></article><article><b>🎲 Résultats riches</b><span>Objets, stock, XP, message et Event en un tirage</span></article><article><b>🛒 Économie</b><span>Recette, vente, livraison, réparation et amélioration</span></article><article><b>🧩 Discord</b><span>Pages, menu, navigation, inventaires et conditions</span></article><article><b>🔊 Audio</b><span>Ambiance, SFX et changement lors d'un événement</span></article></div><ol class="academy-reference-journey"><li>Choisir un métier</li><li>Débloquer une zone</li><li>Lancer l'activité</li><li>Recevoir plusieurs effets</li><li>Transformer ou livrer</li></ol><div class="academy-reference-actions"><span>🔒 Lecture seule · aucune donnée créée · aucune publication Discord</span><div><button type="button" data-open-reference-building>Explorer librement</button><button type="button" class="primary" data-guide-reference-building>Visite guidée</button></div></div></section>`,
    );
  academy.querySelector("[data-open-reference-building]").onclick =
    openReferenceBuilding;
  academy.querySelector("[data-guide-reference-building]").onclick =
    async () => {
      await openReferenceBuilding();
      KingdomTutorials.guide([
        {
          target: ".reference-demo-banner",
          title: "Votre bâtiment-école",
          content:
            "Cette copie est indépendante de votre serveur. Les onglets et formulaires sont les vrais outils du Studio, verrouillés en lecture seule.",
        },
        {
          target: "[data-reference-tab='mechanics']",
          title: "Métiers et activités",
          content:
            "Vous y trouverez deux métiers, leurs outils, niveaux, zones, limites, durées, cooldowns et résultats pondérés.",
        },
        {
          target: "[data-reference-tab='visual']",
          title: "Parcours Discord",
          content:
            "Observez comment les pages, menus, inventaires et boutons composent l'expérience visible par le joueur.",
        },
        {
          target: "[data-reference-tab='relations']",
          title: "Relations calculées",
          content:
            "Le Studio explique ici quels objets sont produits, consommés et livrés, sans vous demander de recopier les liens.",
        },
        {
          target: "[data-reference-tab='sound']",
          title: "Atmosphère vivante",
          content:
            "Ambiance globale, effet sonore et changement déclenché par Event sont réunis dans ce chapitre.",
        },
        {
          target: "[data-reference-tab='advanced']",
          title: "Pour aller plus loin",
          content:
            "Réparation, amélioration et primitives détaillées restent disponibles lorsque le mode Simple ne suffit plus.",
        },
      ]);
    };
}
...
async function openReferenceBuilding() {
  const response = await fetch("/api/tutorials/reference-building", {
    headers,
    cache: "no-store",
  });
  if (!response.ok) {
    alert((await response.json()).detail || "Démonstration indisponible.");
    return;
  }
...
function configureCommonFields() {
  ...
  $("#common-title").textContent = visual
    ? "Identité de l’interface"
    : "L’essentiel";
  $("#common-description").textContent = visual
    ? "Le nom utilisé dans le Studio et le thème général."
    : "Ce que les joueurs verront dans Discord.";
  $("#name-label").textContent = visual
    ? "Nom de l’interface"
    : state.type === "building"
      ? "Nom du lieu"
      : "Nom";
...
function renderPresetPicker() {
  ...
  root.hidden = false;
  root.innerHTML = `<div class="preset-intro"><h3>Choisis le fonctionnement le plus proche</h3><p>Pas d’inquiétude : chaque détail pourra être changé à l’étape suivante.</p></div><div class="preset-grid">${KingdomBuildingPresets.map(
    (preset) => `
    <button type="button" class="preset-card" data-preset="${preset.key}"><span class="preset-icon">${preset.icon}</span><strong>${preset.name}</strong><p>${preset.description}</p><small>Ex. ${preset.example}</small></button>`,
  ).join("")}</div>`;
...
async function openEditor(entity, duplicate = false) {
  if (mobileCreationBlocked()) return showDesktopRequired("L’édition avancée");
  ...
  if (!duplicate) {
    const fresh = await fetch(
      `/api/content/${state.type}/${encodeURIComponent(entity.entity_key)}`,
      { headers, cache: "no-store" },
    );
  ...
  $("#emoji").value = payload.emoji || "";
  $("#description").value = payload.description || "";
  $("#editor-kicker").textContent = duplicate
    ? "DUPLICATION GUIDÉE"
    : "ÉDITEUR NO-CODE";
  $("#editor-title").textContent = duplicate
    ? "Crée une variante"
  ...
  showModal();
  if (state.type === "event" && !duplicate)
    renderEventLifecycle(entity.entity_key);
  KingdomTutorials.notify("building_editor_opened", entity.entity_key);
}

function mobileCreationBlocked() {
...
function showDesktopRequired(feature) {
  ...
  showSystemView();
  $("#admin-view").innerHTML =
    `<section class="mobile-desktop-required"><span>🖥️</span><h2>${escapeHtml(feature)} se fait sur ordinateur</h2><p>Sur mobile, KingdomWeb privilégie la surveillance du monde, les joueurs, les événements et les alertes. Utilise la version ordinateur pour créer ou restructurer du contenu en toute sécurité.</p></section>`;
}

async function renderEventLifecycle(eventKey) {
  const response = await fetch("/api/events/occurrences", {
    headers,
    cache: "no-store",
  });
  if (!response.ok) return;
  const occurrences = (await response.json()).occurrences.filter(
      (item) => item.event_key === eventKey,
    ),
    current = occurrences.find((item) =>
  ...
  root.insertAdjacentHTML(
    "afterbegin",
    `<section class="form-section event-lifecycle"><div class="section-head"><div><small>OCCURRENCE</small><h3>${current ? `État : ${current.status.toUpperCase()}` : "Aucune occurrence en cours"}</h3><p>${remaining !== null && remaining !== undefined ? `Temps restant : <b data-event-countdown="${remaining}">${formatDuration(remaining)}</b>` : "La définition peut être activée quand vous le souhaitez."}</p></div></div><div class="event-lifecycle-actions">${!current ? '<button type="button" class="primary" data-event-command="activate">Activer maintenant</button>' : ""}${current?.status === "active" ? '<button type="button" data-event-command="pause">Mettre en pause</button>' : ""}${current?.status === "paused" ? '<button type="button" class="primary" data-event-command="resume">Reprendre</button>' : ""}${["active", "paused"].includes(current?.status) ? '<button type="button" data-event-command="extend">+ 1 heure</button><button type="button" class="danger-link" data-event-command="stop">Arrêter maintenant</button>' : ""}</div></section>`,
  );
  $$("[data-event-command]", root).forEach(
    (button) =>
      (button.onclick = async () => {
        let url,
          body = {};
        if (button.dataset.eventCommand === "activate") {
          ...
          if (button.dataset.eventCommand === "extend")
            body = { seconds: 3600 };
          if (
            button.dataset.eventCommand === "stop" &&
            !confirm(
              "Arrêter cet événement maintenant ? Ses impacts seront retirés immédiatement.",
            )
          )
            return;
        }
        ...
        if (!result.ok) {
          alert((await result.json()).detail);
          return;
        }
        KingdomTutorials.notify(
          `event_${button.dataset.eventCommand}`,
          eventKey,
        );
...
function formatDuration(seconds) {
  ...
  return [days && `${days} j`, hours && `${hours} h`, `${minutes} min`]
    .filter(Boolean)
    .join(" ");
}
...
async function loadAudioBank() {
  closeAudioPreview();
  restoreContentToolbar();
  if (state.audioMode !== "library") {
    renderAudioComposer(state.audioMode);
    return;
  }
  ...
  const items = state.items
    .filter((entity) => {
      ...
      return (
        (!query ||
          `${entity.entity_key} ${payload.name} ${(payload.tags || []).join(" ")}`
            .toLowerCase()
            .includes(query)) &&
        (!filters.type ||
          (payload.audio_type || payload.channel) === filters.type) &&
        (!filters.bot || payload.speaker_bot_key === filters.bot)
      );
    })
    .sort((a, b) =>
      filters.sort === "recent"
        ...
        : filters.sort === "type"
          ? String(a.payload.audio_type || a.payload.channel).localeCompare(
              String(b.payload.audio_type || b.payload.channel),
            )
          : String(a.payload.name).localeCompare(String(b.payload.name), "fr"),
    );
  $("#count").textContent = state.items.length;
  $("#published").textContent = state.items.filter(
  ...
  $("#content-workspace .toolbar").innerHTML =
    `${audioModeTabs()}<input id="audio-search" value="${escapeHtml(filters.search)}" placeholder="Rechercher un son ou un mot-clé…"><select id="audio-type-filter"><option value="">Tous les types</option><option value="voice">Voix</option><option value="music">Musiques</option><option value="ambience">Ambiances</option><option value="sfx">SFX</option></select><select id="audio-bot-filter"><option value="">Tous les bots</option>${voiceBotOptions(
      "",
    )
      .slice(1)
      .map(
      ...
      .join(
        "",
      )}</select><select id="audio-sort"><option value="name_asc">Nom A–Z</option><option value="type">Type audio</option><option value="recent">Plus récents</option></select>`;
  $("#audio-type-filter").value = filters.type;
  $("#audio-bot-filter").value = filters.bot;
  $("#audio-sort").value = filters.sort;
  $("#cards").innerHTML =
    `<section class="audio-upload-card"><div><small>BANQUE SONORE KINGDOMDATA</small><h3>Importer un nouveau son</h3><p>Le fichier sera copié, référencé et publié automatiquement.</p></div><form id="audio-upload-form"><input name="file" type="file" accept="audio/*,.mp3,.wav,.ogg,.flac,.m4a,.aac,.opus" required><input name="name" placeholder="Nom du son" required><select name="audio_type"><option value="sfx">Effet sonore</option><option value="ambience">Ambiance</option><option value="music">Musique</option><option value="voice">Voix / phrase</option></select><select name="speaker_bot_key">${voiceBotOptions(
      "",
    )
      .map(
        ([key, label]) =>
      ...
      .join(
        "",
      )}</select><input name="tags" placeholder="Mots-clés : forêt, bois, hache"><button class="primary">Télécharger dans KingdomData</button><span id="audio-upload-status"></span></form></section><section id="audio-preview-panel" class="audio-preview-panel" hidden><div><small>ÉCOUTE AVANT ATTRIBUTION</small><b id="audio-preview-title">Aperçu du son</b></div><audio id="audio-preview-player" controls preload="metadata"></audio><button type="button" id="audio-preview-close" aria-label="Fermer le lecteur">×</button></section>${
      items
        .map((entity) => {
          const p = entity.payload,
            type = p.audio_type || p.channel || "sfx",
            bot = state.catalogs.bot.find(
  ...
  $("#audio-type-filter").onchange = (e) => {
    filters.type = e.target.value;
    loadAudioBank();
  };
  $("#audio-bot-filter").onchange = (e) => {
    filters.bot = e.target.value;
    loadAudioBank();
  };
  $("#audio-sort").onchange = (e) => {
...
function audioModeTabs() {
  return `<nav class="audio-mode-tabs"><button type="button" data-audio-mode="library" class="${state.audioMode === "library" ? "active" : ""}">🔊 Banque sonore</button><button type="button" data-audio-mode="groups" class="${state.audioMode === "groups" ? "active" : ""}">🌲 Groupes d’ambiance</button><button type="button" data-audio-mode="stories" class="${state.audioMode === "stories" ? "active" : ""}">📖 Histoires auditives</button></nav>`;
}
function bindAudioModeTabs() {
  $$("[data-audio-mode]").forEach(
    (button) =>
      (button.onclick = () => {
...
function renderAudioComposer(mode) {
  const groups = state.catalogs.audio_group || [],
    stories = state.catalogs.audio_story || [],
    entities = mode === "groups" ? groups : stories;
  state.items = entities;
  $("#count").textContent = entities.length;
  $("#published").textContent = entities.filter(
    (x) => x.status === "published",
  ).length;
  $("#drafts").textContent = entities.filter(
    (x) => x.status === "draft",
  ).length;
  $("#content-workspace .toolbar").innerHTML =
    `${audioModeTabs()}<button type="button" class="primary" id="create-audio-composition">＋ ${mode === "groups" ? "Nouveau groupe" : "Nouvelle histoire"}</button>`;
  $("#cards").innerHTML =
    `<section class="audio-composer-intro"><span>${mode === "groups" ? "🌲" : "📖"}</span><div><small>${mode === "groups" ? "AMBIANCES RÉUTILISABLES" : "NARRATION AUDIO NO-CODE"}</small><h2>${mode === "groups" ? "Composez une atmosphère une fois, utilisez-la partout" : "Construisez une histoire étape par étape"}</h2><p>${mode === "groups" ? "Superposez ambiance, musique, voix et effets. Un Event peut appliquer ce groupe à un ou plusieurs bâtiments." : "Ordonnez sons, silences et textes. Ces séquences sont prêtes pour les expéditions et scénarios futurs."}</p></div></section>${entities.map((entity) => audioCompositionCard(entity, mode)).join("") || `<p class="empty">Aucun ${mode === "groups" ? "groupe d’ambiance" : "récit audio"} pour le moment.</p>`}`;
  bindAudioModeTabs();
  $("#create-audio-composition").onclick = () => openAudioComposition(mode);
  $$("[data-edit-audio-composition]").forEach(
    (button) =>
      (button.onclick = () =>
        openAudioComposition(
          mode,
          entities.find(
            (item) => item.entity_key === button.dataset.editAudioComposition,
          ),
        )),
  );
  $$("[data-delete-audio-composition]").forEach(
    (button) =>
      (button.onclick = async () => {
        const entity = entities.find(
          (item) => item.entity_key === button.dataset.deleteAudioComposition,
        );
        if (!entity || !confirm(`Supprimer « ${entity.payload.name} » ?`))
          return;
        const response = await fetch(
          `/api/content/${mode === "groups" ? "audio_group" : "audio_story"}/${encodeURIComponent(entity.entity_key)}`,
          { method: "DELETE", headers },
        );
        if (!response.ok) {
          alert((await response.json()).detail);
...
function audioSelectMarkup(name, current = "", type = "") {
  return `<select name="${name}">${audioOptions(current, type)
    .map(
      ([key, label]) =>
...
function addAudioLayerRow(root, layer = {}) {
  const row = document.createElement("article");
  row.className = "audio-layer-row";
  row.innerHTML = `<span class="audio-drag">⠿</span><label>Son${audioSelectMarkup("audio_key", layer.audio_key || "")}</label><label>Rôle<select name="role"><option value="ambience">Ambiance</option><option value="music">Musique</option><option value="sfx">Effet</option><option value="voice">Voix</option></select></label><label>Volume<input name="volume" type="range" min="0" max="1" step=".05" value="${Number(layer.volume ?? 1)}"><output>${Math.round(Number(layer.volume ?? 1) * 100)} %</output></label><label class="check"><input name="loop" type="checkbox" ${layer.loop !== false ? "checked" : ""}><span>Boucle</span></label><button type="button" class="danger-link">×</button>`;
  root.append(row);
  row.querySelector('[name="role"]').value = layer.role || "ambience";
  row.querySelector('[name="volume"]').oninput = (e) =>
    (row.querySelector("output").textContent =
      `${Math.round(e.target.value * 100)} %`);
  row.querySelector(".danger-link").onclick = () => row.remove();
}
function addAudioStoryStep(root, step = {}) {
  const row = document.createElement("article");
  row.className = "audio-story-step";
  row.innerHTML = `<span class="story-index">${root.children.length + 1}</span><div class="form-grid"><label>Nom de l’étape<input name="name" value="${escapeHtml(step.name || "")}"></label><label>Son${audioSelectMarkup("audio_key", step.audio_key || "")}</label><label>Délai avant lecture<input name="delay_seconds" type="number" min="0" value="${Number(step.delay_seconds || 0)}"></label><label>Comportement<select name="wait_for_end"><option value="true">Attendre la fin du son</option><option value="false">Continuer sans attendre</option></select></label></div><label>Texte / indication narrative<textarea name="text" rows="2">${escapeHtml(step.text || "")}</textarea></label><button type="button" class="danger-link">Supprimer l’étape</button>`;
  root.append(row);
  row.querySelector('[name="wait_for_end"]').value = String(
    step.wait_for_end !== false,
  );
  row.querySelector(".danger-link").onclick = () => {
    row.remove();
    [...root.children].forEach(
      (item, index) =>
        (item.querySelector(".story-index").textContent = index + 1),
    );
  };
}
async function saveAudioComposition(type, key, payload, expectedVersion) {
  const entityType = type === "groups" ? "audio_group" : "audio_story",
    draftResponse = await fetch(
      `/api/content/${entityType}/${encodeURIComponent(key)}`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
  ...
  if (!draftResponse.ok)
    throw Error(draft.detail || "Enregistrement impossible.");
  const publishResponse = await fetch(
    `/api/content/${entityType}/${encodeURIComponent(key)}/${draft.version}/publish`,
    { method: "POST", headers, body: "{}" },
  );
...
function openAudioComposition(mode, entity = null) {
  ...
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "audio-composition-dialog";
    document.body.append(dialog);
  }
  ...
  if (isGroup)
    (p.layers || []).forEach((layer) => addAudioLayerRow(rows, layer));
  else (p.steps || []).forEach((step) => addAudioStoryStep(rows, step));
  if (!rows.children.length)
    isGroup ? addAudioLayerRow(rows, {}) : addAudioStoryStep(rows, {});
  dialog
    .querySelectorAll("[data-close]")
    .forEach((button) => (button.onclick = () => dialog.close()));
  dialog.querySelector(
    isGroup ? "[data-add-layer]" : "[data-add-step]",
  ).onclick = () =>
    isGroup ? addAudioLayerRow(rows, {}) : addAudioStoryStep(rows, {});
  dialog.querySelector("form").onsubmit = async (event) => {
    ...
    const form = event.currentTarget,
      key = technicalKey(
        form.elements.key.value || form.elements.title.value,
        isGroup ? "ambiance" : "histoire",
      );
    form.elements.key.value = key;
    const payload = {
      name: form.elements.title.value.trim(),
      description: form.elements.description.value.trim(),
      emoji: isGroup ? "🌲" : "📖",
    };
    if (isGroup) {
      payload.volume = Number(form.elements.master_volume.value);
      payload.transitions = {
        fade_in_seconds: Number(form.elements.fade_in.value),
        fade_out_seconds: 0,
        crossfade_seconds: 0,
      };
      ...
      payload.layers = [...rows.children]
        .map((row) => ({
          ...
          volume: Number(row.querySelector('[name="volume"]').value),
          loop: row.querySelector('[name="loop"]').checked,
        }))
        .filter((layer) => layer.audio_key);
    } else
      payload.steps = [...rows.children].map((row, index) => ({
        key: `etape_${index + 1}`,
        name: row.querySelector('[name="name"]').value.trim(),
        audio_key: row.querySelector('[name="audio_key"]').value,
        delay_seconds: Number(
          row.querySelector('[name="delay_seconds"]').value,
        ),
        wait_for_end:
          row.querySelector('[name="wait_for_end"]').value === "true",
...
async function loadVoicePresenceStudio() {
  ...
  const profileName = (key) =>
    profiles.find((item) => item.entity_key === key)?.payload.name ||
    "Aucun profil";
  ...
  const cards = presences
    .map((entity) => {
      const p = entity.payload,
        isActive = activePresenceKeys.has(entity.entity_key),
        stateLabel =
          isActive
            ? "Active"
...
function bindVoiceStudio() {
  $$("[data-voice-tab]").forEach(
    (button) =>
      (button.onclick = () => {
        $$("[data-voice-tab]").forEach((item) =>
          item.classList.toggle("active", item === button),
        );
        $$("[data-voice-panel]").forEach(
          (panel) =>
            (panel.hidden =
              panel.dataset.voicePanel !== button.dataset.voiceTab),
        );
      }),
  );
  $$("[data-new-presence]").forEach(
    (button) => (button.onclick = () => openVoicePresenceDialog()),
  );
  $("[data-new-profile]")?.addEventListener("click", () =>
    openVoiceProfileDialog(),
  );
  $$("[data-edit-presence]").forEach(
    (button) =>
      (button.onclick = () =>
        openVoicePresenceDialog(
          state.catalogs.voice_presence.find(
            (item) => item.entity_key === button.dataset.editPresence,
          ),
        )),
  );
  $$("[data-edit-profile]").forEach(
    (button) =>
      (button.onclick = () =>
        openVoiceProfileDialog(
          state.catalogs.voice_profile.find(
            (item) => item.entity_key === button.dataset.editProfile,
          ),
        )),
  );
  bindNavigationShortcuts();
...
function voiceDialog() {
  let dialog = $("#voice-editor-dialog");
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "voice-editor-dialog";
    dialog.className = "voice-editor-dialog";
    document.body.append(dialog);
  }
  return dialog;
}
function entityOptions(items, value, empty = "Aucun") {
...
async function ensureNpcVoiceResources(npcKey, payload, { createPresence = true } = {}) {
  const currentProfile = state.catalogs.voice_profile.find(
    ...
    currentPresence = state.catalogs.voice_presence.find(
      (item) => item.entity_key === payload.voice_presence_key,
    ),
    profileKey = currentProfile?.entity_key || `voice_${npcKey}`,
    presenceKey = currentPresence?.entity_key || `presence_${npcKey}`,
    building = state.catalogs.building.find(
      (item) => item.entity_key === payload.building_key,
    ),
    voiceAudio = fieldValue("npc_voice_audio") || "";
  await saveAndPublishEntity(
    "voice_profile",
    profileKey,
    {
      ...(currentProfile?.payload || {}),
      name: currentProfile?.payload.name || `Voix de ${payload.name}`,
      provider: currentProfile?.payload.provider || "files",
      language: currentProfile?.payload.language || "fr",
      volume: Number(fieldValue("npc_voice_volume") ?? 1),
      tags: [...new Set([...(currentProfile?.payload.tags || []), "personnage", npcKey])],
      clips: voiceAudio
        ? [{ audio_key: voiceAudio, category: "default" }]
        : currentProfile?.payload.clips || [],
      metadata: {
        ...(currentProfile?.payload.metadata || {}),
        missing_audio: voiceAudio || currentProfile?.payload.clips?.length ? "" : "À fournir",
      },
    },
...
function openVoicePresenceDialog(entity = null) {
  ...
  dialog
    ...
    ?.addEventListener("click", () =>
      deleteVoiceEntity("voice_presence", entity, dialog),
    );
}

function openVoiceProfileDialog(entity = null) {
  if (mobileCreationBlocked())
    return showDesktopRequired("La création d’un profil vocal");
  const dialog = voiceDialog(),
    p = clone(entity?.payload || {}),
    clipKeys = (p.clips || []).map((item) => item.audio_key).filter(Boolean);
  dialog.innerHTML = `<form><div class="dialog-head"><div><small>PROFIL VOCAL</small><h2>${entity ? "Modifier" : "Créer"} une bibliothèque de voix</h2></div><button type="button" data-close>×</button></div><div class="voice-editor-body"><div class="form-grid"><label>Nom<input name="name" value="${escapeHtml(p.name || "")}" required></label><label>Langue<input name="language" value="${escapeHtml(p.language || "fr")}" placeholder="fr, en, langue fictive…"></label><label>Source<select name="provider"><option value="files">Fichiers audio</option><option value="future" ${p.provider === "future" ? "selected" : ""}>Provider futur</option></select></label><label>Volume<input name="volume" type="number" min="0" max="2" step=".05" value="${Number(p.volume ?? 1)}"></label><label>Profil de secours<select name="fallback_profile_key">${entityOptions(
    state.catalogs.voice_profile.filter(
      (item) => item.entity_key !== entity?.entity_key,
    ),
  ...
  dialog.querySelector("form").onsubmit = async (event) => {
    ...
    const form = event.currentTarget,
      payload = {
        ...
        fallback_profile_key: form.elements.fallback_profile_key.value,
        tags: form.elements.tags.value
          .split(",")
          .map((tag) => tag.trim())
          .filter(Boolean),
        clips: [...form.querySelectorAll('[name="clip"]:checked')].map(
          (input) => ({ audio_key: input.value, category: "default" }),
        ),
      };
    await saveAndPublishEntity(
      "voice_profile",
      entity?.entity_key ||
        technicalKey(form.elements.key.value || payload.name, "profil"),
      payload,
      entity?.version,
    );
    dialog.close();
  ...
  dialog
    .querySelector("[data-delete]")
    ?.addEventListener("click", () =>
      deleteVoiceEntity("voice_profile", entity, dialog),
    );
}

async function saveAndPublishEntity(type, key, payload, expectedVersion) {
  const response = await fetch(
      `/api/content/${type}/${encodeURIComponent(key)}`,
      {
        method: "POST",
        headers,
    ...
    data = await response.json();
  if (!response.ok) throw Error(data.detail || "Enregistrement impossible.");
  const published = await fetch(
    `/api/content/${type}/${encodeURIComponent(key)}/${data.version}/publish`,
    { method: "POST", headers, body: "{}" },
  );
  if (!published.ok)
    throw Error((await published.json()).detail || "Publication impossible.");
  return published.json();
}
async function deleteVoiceEntity(type, entity, dialog) {
  if (
    !confirm(
      `Supprimer « ${entity.payload.name} » ? L’historique restera conservé.`,
    )
  )
    return;
  const response = await fetch(
...
async function previewAudio(key) {
  const response = await fetch(`/api/audio/${encodeURIComponent(key)}/file`, {
    headers,
  });
  if (!response.ok) {
    alert((await response.json()).detail);
  ...
  const url = URL.createObjectURL(await response.blob()),
    player = $("#audio-preview-player"),
    panel = $("#audio-preview-panel"),
    entity = state.catalogs.audio.find((item) => item.entity_key === key);
  player.src = url;
  player.dataset.objectUrl = url;
  $("#audio-preview-title").textContent = entity?.payload?.name || key;
  panel.hidden = false;
  panel.scrollIntoView({ behavior: "smooth", block: "nearest" });
  try {
    await player.play();
  } catch (_) {
    /* Le navigateur laisse alors l'utilisateur cliquer sur les contrôles visibles. */
  }
}
...
function closeAudioPreview() {
  ...
  const panel = $("#audio-preview-panel");
  if (panel) panel.hidden = true;
}

function voiceBotOptions(current = "") {
  const entries = state.catalogs.bot
    .filter((entity) => entity.payload.bot_type === "voice")
    .map((entity) => [entity.entity_key, `🤖 ${entity.payload.name}`]);
  if (current && !entries.some(([key]) => key === current))
    entries.push([current, `⚠ ${current} (introuvable)`]);
  return [["", "Choisir un bot vocal…"], ...entries];
}

function audioOptions(current = "", type = "") {
  const entries = state.catalogs.audio
    .filter(
      (entity) =>
        !type || (entity.payload.audio_type || entity.payload.channel) == type,
    )
    .map((entity) => [entity.entity_key, `🔊 ${entity.payload.name}`]);
  if (current && !entries.some(([key]) => key === current))
    entries.push([current, `⚠ ${current} (introuvable)`]);
...
async function uploadBotAvatar(key) {
  const file = $("#bot-avatar-file")?.files?.[0];
  if (!file) return;
  const form = new FormData();
  form.append("file", file);
  const response = await fetch(`/api/bots/${encodeURIComponent(key)}/avatar`, {
      method: "POST",
      headers: multipartHeaders(),
      body: form,
    }),
    result = await response.json();
  if (!response.ok)
    throw Error(result.detail || "Import de la photo impossible.");
  const path = $('[data-field="avatar_path"]');
  if (path) path.value = result.avatar_path;
}

function renderAudioFields(payload) {
  const root = $("#type-fields");
  root.innerHTML = `<section class="form-section"><h3>Classement du son</h3><p class="field-note">Le fichier est conservé dans KingdomData. Modifie ici uniquement sa fiche et son comportement.</p><div class="form-grid">${select(
    "Type d’audio",
    "audio_type",
    payload.audio_type || payload.channel || "sfx",
    [
      ["voice", "Voix / phrase"],
      ["music", "Musique"],
      ["ambience", "Ambiance"],
  ...
  )}${select("Bot qui parle (facultatif)", "speaker_bot_key", payload.speaker_bot_key || "", voiceBotOptions(payload.speaker_bot_key || ""))}${input("Mots-clés (séparés par des virgules)", "audio_tags", (payload.tags || []).join(", "))}${input("Volume", "volume", payload.volume ?? 0.5, "number", 'min="0" max="1" step="0.05"')}</div><div class="checks">${check("Lecture en boucle", "loop", !!payload.loop)}</div><details class="advanced"><summary>Informations du fichier</summary><div class="advanced-content"><p><b>${escapeHtml(payload.file_name || "Ancien fichier")}</b> · ${Math.round(Number(payload.size_bytes || 0) / 1024)} Ko</p><code>${escapeHtml(payload.storage_path || payload.source || "")}</code></div></details></section>`;
}

function modifierTargetOptions(type, current = "") {
  let rows = [];
  if (type === "building") rows = catalogOptions("building", current);
  else if (type === "item") rows = catalogOptions("item", current);
  else if (type === "location") rows = locationOptions(current);
  else if (type === "profession") rows = professionOptions(current);
  else if (["activity", "recipe", "action"].includes(type)) {
    rows = [["", "Choisir…"]];
    for (const building of state.catalogs.building) {
      const entries =
        type === "action"
          ? building.payload.actions || []
          : building.payload.modules?.[`${type}s`] || [];
      entries.forEach((entry) =>
        rows.push([
          entry.key,
          `${building.payload.emoji || "🏰"} ${building.payload.name} · ${entry.name || entry.key}`,
        ]),
      );
    }
  } else rows = [["", "Tout le royaume"]];
  return rows;
}
function addWorldModifier(root, modifier = {}) {
  const row = document.createElement("div");
  row.className = "builder world-modifier";
  const type = modifier.target?.type || "kingdom";
  row.innerHTML = `<button type="button" class="remove">×</button><div class="modifier-support">✓ Pris en charge par le gameplay</div><div class="form-grid">${select(
    "Cible",
    "modifier_target_type",
    type,
    [
      ["kingdom", "Tout le royaume"],
      ["building", "Bâtiment"],
      ["profession", "Métier"],
      ["activity", "Activité"],
      ["recipe", "Recette"],
  ...
  )}<label data-modifier-target>Élément ciblé<select data-field="modifier_target_key">${modifierTargetOptions(
    type,
    modifier.target?.key || "",
  )
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === (modifier.target?.key || "") ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join("")}</select></label>${select(
    "Valeur modifiée",
    "modifier_property",
    modifier.property || "production.quantity",
    [
      ["production.quantity", "Quantité produite"],
      ["activity.duration", "Durée activité"],
      ["energy.cost", "Coût énergétique"],
      ["recipe.ingredient_quantity", "Quantité d’ingrédient"],
      ["economy.price", "Prix"],
    ...
    "Opération",
    "modifier_operator",
    modifier.operator || "multiply",
    [
      ["multiply", "Multiplier"],
      ["add", "Ajouter"],
      ["set", "Définir"],
      ["min", "Maximum autorisé"],
      ["max", "Minimum garanti"],
    ],
  )}${input("Valeur", "modifier_value", modifier.value ?? 1, "number", 'step="0.01"')}</div>`;
  root.append(row);
  row.querySelector(".remove").onclick = () => row.remove();
  row.querySelector('[data-field="modifier_target_type"]').onchange = (
    event,
  ) => {
    const target = row.querySelector("[data-modifier-target]"),
      options = modifierTargetOptions(event.target.value);
    target.innerHTML = `Élément ciblé<select data-field="modifier_target_key">${options.map(([key, label]) => `<option value="${escapeHtml(key)}">${escapeHtml(label)}</option>`).join("")}</select>`;
  };
}
function readWorldModifiers() {
  return $$(".world-modifier").map((row) => ({
    target: {
      type: fieldValue("modifier_target_type", row),
      key: fieldValue("modifier_target_key", row) || undefined,
    },
    property: fieldValue("modifier_property", row),
    operator: fieldValue("modifier_operator", row),
    value: fieldValue("modifier_value", row),
  }));
}
...
function locationOptions(current = "", exclude = "") {
  const depth = (key) => {
    let count = 0,
      cursor = state.catalogs.location.find((item) => item.entity_key === key);
    while (cursor?.payload?.parent_key && count < 8) {
      count++;
      cursor = state.catalogs.location.find(
        (item) => item.entity_key === cursor.payload.parent_key,
      );
    }
    return count;
  };
  const rows = state.catalogs.location
    .filter((item) => item.entity_key !== exclude)
    .sort(
      (a, b) =>
        depth(a.entity_key) - depth(b.entity_key) ||
        (a.payload.name || a.entity_key).localeCompare(
          b.payload.name || b.entity_key,
          "fr",
        ),
    )
    .map((item) => [
      item.entity_key,
      `${"　".repeat(depth(item.entity_key))}${item.payload.emoji || "📍"} ${item.payload.name || item.entity_key}`,
    ]);
  if (current && !rows.some(([key]) => key === current))
    rows.push([current, `⚠ Lieu indisponible`]);
  return [["", "Choisir un lieu…"], ...rows];
}
function routeRequirementOptions(type, current = "") {
  return type === "item"
    ? catalogOptions("item", current)
    : type === "profession"
      ? professionOptions(current)
      : [["", "Aucun prérequis"]];
}
function addLocationConnection(root, connection = {}) {
  const condition = (connection.conditions || [])[0] || {},
    conditionType = condition.type || "none";
  const row = document.createElement("article");
  row.className = "location-connection-editor";
  row.innerHTML = `<button type="button" class="remove" aria-label="Supprimer la connexion">×</button><div class="form-grid">${select("Destination", "connection_target", connection.target || "", locationOptions(connection.target || "", state.editing?.entity_key || ""))}${input("Nom du chemin", "connection_name", connection.name || "")}${select(
    "Direction",
    "connection_direction",
    connection.direction || "one_way",
    [
      ["one_way", "Sens unique"],
      ["bidirectional", "Aller-retour"],
    ],
  )}${input("Durée (secondes)", "connection_duration", connection.duration_seconds || 0, "number", 'min="0"')}${input("Coût en écus", "connection_cost", connection.cost || 0, "number", 'min="0"')}${select(
    "Visibilité",
    "connection_visibility",
    connection.visibility || "visible",
    [
      ["visible", "Visible immédiatement"],
      ["discovered", "Après découverte"],
      ["secret", "Passage secret"],
    ],
  )}${select("Prérequis", "connection_condition_type", conditionType, [
    ["none", "Aucun"],
    ["item", "Objet possédé"],
    ["profession", "Métier actif"],
  ])}<label data-route-requirement>Élément requis<select data-field="connection_condition_key">${routeRequirementOptions(
    conditionType,
    condition.key || "",
  )
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === (condition.key || "") ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join(
      "",
    )}</select></label>${input("Quantité / niveau minimum", "connection_condition_minimum", condition.minimum || 1, "number", 'min="1"')}</div>`;
  root.append(row);
  row.querySelector(".remove").onclick = () => row.remove();
  row.querySelector('[data-field="connection_condition_type"]').onchange = (
    event,
  ) => {
    row.querySelector("[data-route-requirement]").innerHTML =
      `Élément requis<select data-field="connection_condition_key">${routeRequirementOptions(
        event.target.value,
      )
        .map(
          ([key, label]) =>
...
function readLocationConnections() {
  return $$(".location-connection-editor")
    .map((row, index) => {
      const type = fieldValue("connection_condition_type", row),
        key = fieldValue("connection_condition_key", row);
      return {
        key: technicalKey(
          `${state.editing?.entity_key || fieldValue("location_type")}_${fieldValue("connection_target", row)}_${index + 1}`,
          "route",
        ),
        target: fieldValue("connection_target", row),
        name: fieldValue("connection_name", row),
        direction: fieldValue("connection_direction", row),
        duration_seconds: fieldValue("connection_duration", row),
        cost: fieldValue("connection_cost", row),
        visibility: fieldValue("connection_visibility", row),
        conditions:
          type !== "none" && key
            ? [
                {
                  type,
                  key,
                  minimum: fieldValue("connection_condition_minimum", row) || 1,
                },
              ]
            : [],
      };
...
function localActivityOptions(current = "") {
  const rows = [["", "Choisir une activité…"]];
  for (const building of state.catalogs.building)
    for (const activity of building.payload.modules?.activities || [])
      rows.push([
        `${building.entity_key}|${activity.key}`,
        `${building.payload.emoji || "🏰"} ${building.payload.name} · ${activity.emoji || "⚙️"} ${activity.name || activity.key}`,
      ]);
  if (current && !rows.some(([key]) => key === current))
    rows.push([current, "⚠ Activité indisponible"]);
  return rows;
}
function addLocationActivity(root, reference = {}) {
  const value =
    reference.building_key && reference.activity_key
      ? `${reference.building_key}|${reference.activity_key}`
      : "";
  const row = document.createElement("div");
  row.className = "location-activity-editor";
  row.innerHTML = `${select("Activité disponible ici", "location_activity_ref", value, localActivityOptions(value))}<button type="button" class="danger-link">Retirer</button>`;
  root.append(row);
  row.querySelector("button").onclick = () => row.remove();
}
function readLocationActivities() {
  return $$(".location-activity-editor")
    .map((row) => fieldValue("location_activity_ref", row))
    .filter(Boolean)
    .map((value) => {
      const [building_key, activity_key] = value.split("|");
      return { building_key, activity_key };
    });
}
function addWeatherOption(root, weather = {}) {
  const row = document.createElement("article");
  row.className = "weather-option-editor";
  row.innerHTML = `<button type="button" class="remove">×</button><div class="form-grid">${input("Nom", "weather_option_name", weather.name || "")}${input("Emoji", "weather_option_emoji", weather.emoji || "☀️")}${input("Poids", "weather_option_weight", weather.weight ?? 1, "number", 'min="0" step="0.1"')}${input("À partir de l’heure", "weather_option_hour", weather.hour ?? 0, "number", 'min="0" max="23"')}</div>`;
  root.append(row);
  row.querySelector(".remove").onclick = () => row.remove();
}
function readWeatherOptions() {
  return $$(".weather-option-editor")
    .map((row) => ({
      key: technicalKey(fieldValue("weather_option_name", row), "meteo"),
      name: fieldValue("weather_option_name", row),
      emoji: fieldValue("weather_option_emoji", row),
      weight: fieldValue("weather_option_weight", row),
      hour: fieldValue("weather_option_hour", row),
      modifiers: [],
    }))
    .filter((item) => item.name);
}

function npcConditionOptions(type, current = "") {
  const environment =
    state.catalogs.environment.find((item) => item.status === "published")
      ?.payload ||
    state.catalogs.environment[0]?.payload ||
    {};
  let rows = [];
  if (type === "period")
    rows = [
    ...
    rows = (environment.weather_options || []).map((item) => [
      item.key,
      `${item.emoji || "🌦️"} ${item.name || item.key}`,
    ]);
  else if (type === "season")
    rows = (environment.calendar?.seasons || []).map((item) => [
      item.key,
      `${item.emoji || "🍂"} ${item.name || item.key}`,
    ]);
    ...
    rows = state.catalogs.profession.map((item) => [
      item.entity_key,
      `${item.payload.emoji || "📜"} ${item.payload.name || item.entity_key}`,
    ]);
  else return [["", "Aucune valeur nécessaire"]];
  if (current && !rows.some(([key]) => key === current))
    rows.push([current, `⚠ ${current}`]);
  return [["", "Choisir…"], ...rows];
}
function addNpcCondition(root, condition = {}) {
  const type = condition.type || "period",
    current = condition.value || condition.key || "";
  const row = document.createElement("div");
  row.className = "npc-condition form-grid";
  row.innerHTML = `${select("Condition", "npc_condition_type", type, [
    ["period", "Période du jour"],
    ["weather", "Météo"],
    ["season", "Saison"],
    ...
    ["item", "Objet possédé"],
    ["profession", "Métier actif"],
    ["location_discovered", "Lieu découvert"],
  ])}<label>Valeur<select data-field="npc_condition_value">${npcConditionOptions(
    type,
    current,
  )
    .map(
    ...
    .join(
      "",
    )}</select></label><button type="button" class="danger-link">Retirer</button>`;
  root.append(row);
  row.querySelector(".danger-link").onclick = () => row.remove();
  row.querySelector('[data-field="npc_condition_type"]').onchange = (event) => {
    row.querySelector('[data-field="npc_condition_value"]').innerHTML =
      npcConditionOptions(event.target.value)
        .map(
          ([key, label]) =>
            `<option value="${escapeHtml(key)}">${escapeHtml(label)}</option>`,
        )
...
function addNpcVariant(root, variant = {}) {
  const row = document.createElement("div");
  row.className = "npc-variant";
  row.innerHTML = `<button type="button" class="remove">×</button>${input("Texte Discord", "npc_variant_text", variant.text || "")}${select("Voix associée", "npc_variant_audio", variant.audio_key || "", audioOptions(variant.audio_key || "", "voice"))}`;
  root.append(row);
  row.querySelector(".remove").onclick = () => row.remove();
}
function addNpcReaction(root, reaction = {}) {
  const row = document.createElement("details");
  row.className = "module-card npc-reaction";
  row.open = !reaction.key;
  row.innerHTML = `<summary><strong>💬 ${escapeHtml(reaction.name || reaction.key || "Nouvelle réaction")}</strong><small>priorité ${Number(reaction.priority || 0)}</small></summary><div class="module-content"><button type="button" class="remove">×</button><div class="form-grid">${input("Nom", "npc_reaction_name", reaction.name || "")}${select(
    "Déclencheur",
    "npc_reaction_trigger",
    reaction.trigger || "talk",
    [
      ["talk", "Parler"],
      ["greeting", "Salutation"],
      ["purchase", "Achat"],
      ["sale", "Vente"],
      ["activity_success", "Activité réussie"],
      ["activity_failure", "Activité échouée"],
      ["location_entered", "Entrée dans un lieu"],
      ["event_started", "Event démarré"],
    ],
  )}${input("Priorité", "npc_reaction_priority", reaction.priority || 0, "number")}${select(
    "Choix des variantes",
    "npc_variant_mode",
    reaction.variant_mode || "random",
    [
      ["random", "Aléatoire sans répétition immédiate"],
      ["sequential", "Dans l’ordre"],
    ],
  )}</div><div class="section-head"><div><b>Conditions cumulatives</b><p class="field-note">Toutes les conditions ajoutées doivent être vraies.</p></div><button type="button" class="secondary add-npc-condition">＋ Condition</button></div><div class="npc-conditions"></div><div class="section-head"><b>Variantes texte + voix</b><button type="button" class="secondary add-npc-variant">＋ Variante</button></div><div class="npc-variants"></div></div>`;
  root.append(row);
  (reaction.conditions || []).forEach((item) =>
    addNpcCondition(row.querySelector(".npc-conditions"), item),
  );
  (reaction.variants || []).forEach((item) =>
    addNpcVariant(row.querySelector(".npc-variants"), item),
  );
  row.querySelector(".add-npc-condition").onclick = () =>
    addNpcCondition(row.querySelector(".npc-conditions"), {});
  row.querySelector(".add-npc-variant").onclick = () =>
    addNpcVariant(row.querySelector(".npc-variants"), {});
  row.querySelector(":scope > .module-content > .remove").onclick = () =>
    row.remove();
}
function readNpcReactions() {
  return $$(".npc-reaction").map((row, index) => ({
    key: technicalKey(
      fieldValue("npc_reaction_name", row),
      `reaction_${index + 1}`,
    ),
    name: fieldValue("npc_reaction_name", row),
    trigger: fieldValue("npc_reaction_trigger", row),
    priority: fieldValue("npc_reaction_priority", row),
    variant_mode: fieldValue("npc_variant_mode", row),
    conditions: [...row.querySelectorAll(".npc-condition")].map(
      (condition) => ({
        type: fieldValue("npc_condition_type", condition),
        value: fieldValue("npc_condition_value", condition),
      }),
    ),
    variants: [...row.querySelectorAll(".npc-variant")]
      .map((variant, i) => ({
        key: `variant_${i + 1}`,
        text: fieldValue("npc_variant_text", variant),
        audio_key: fieldValue("npc_variant_audio", variant) || "",
      }))
      .filter((item) => item.text),
  }));
}
function moveCalendarRow(row, direction) {
  const sibling =
...
function calendarRowActions(row) {
  row.querySelector("[data-move-up]").onclick = () => moveCalendarRow(row, -1);
  row.querySelector("[data-move-down]").onclick = () => moveCalendarRow(row, 1);
  row.querySelector("[data-remove-calendar]").onclick = () => row.remove();
}
function addCalendarWeekday(root, name = "") {
  const row = document.createElement("div");
  row.className = "calendar-config-row weekday-row";
  row.innerHTML = `${input("Nom du jour", "calendar_weekday", name)}<div class="calendar-row-actions"><button type="button" data-move-up aria-label="Monter">↑</button><button type="button" data-move-down aria-label="Descendre">↓</button><button type="button" class="danger-link" data-remove-calendar>Retirer</button></div>`;
  root.append(row);
  calendarRowActions(row);
}
function addCalendarMonth(root, month = {}) {
  const row = document.createElement("div");
  row.className = "calendar-config-row month-row";
  row.innerHTML = `${input("Nom du mois", "calendar_month_name", month.name || "")}${input("Nombre de jours", "calendar_month_days", month.days || 30, "number", 'min="1"')}<div class="calendar-row-actions"><button type="button" data-move-up aria-label="Monter">↑</button><button type="button" data-move-down aria-label="Descendre">↓</button><button type="button" class="danger-link" data-remove-calendar>Retirer</button></div>`;
  root.append(row);
  calendarRowActions(row);
}
function addCalendarSeason(root, season = {}, months = []) {
  const row = document.createElement("div");
  row.className = "calendar-config-row season-row";
  row.innerHTML = `${input("Nom", "calendar_season_name", season.name || "")}${input("Icône", "calendar_season_emoji", season.emoji || "")}${select(
    "Commence en",
    "calendar_season_month",
    season.start_month_key || months[0]?.key || "",
    months.map((month) => [month.key, month.name]),
  )}${input("Jour", "calendar_season_day", season.start_day || 1, "number", 'min="1"')}<button type="button" class="danger-link">Retirer</button>`;
  root.append(row);
  row.querySelector("button").onclick = () => row.remove();
}
function renderCalendarConfig(payload, root = $("#type-fields")) {
  const calendar = payload.calendar || {};
  root.insertAdjacentHTML(
    "beforeend",
    `<section class="form-section calendar-config"><div class="section-head"><div><h3>Calendrier du royaume</h3><p class="field-note">La semaine, les mois et les saisons sont entièrement personnalisables.</p></div></div><div class="form-grid">${input("Nom du calendrier", "calendar_name", calendar.name || "Calendrier du Royaume")}${input("Année de départ", "calendar_start_year", calendar.start_year || 1, "number")}${input("Jour de départ", "calendar_start_day", calendar.start_day || 1, "number", 'min="1"')}</div><div class="section-head"><b>Jours de semaine</b><button type="button" id="add-calendar-weekday">＋ Jour</button></div><div id="calendar-weekdays"></div><div class="section-head"><b>Mois</b><button type="button" id="add-calendar-month">＋ Mois</button></div><div id="calendar-months"></div><div class="section-head"><b>Saisons</b><button type="button" id="add-calendar-season">＋ Saison</button></div><div id="calendar-seasons"></div></section>`,
  );
  const within = (selector) => root.querySelector(selector),
    ...
    months =
      ...
      Array.from({ length: 12 }, (_, i) => ({
        key: `month_${String(i + 1).padStart(2, "0")}`,
        name: `Mois ${i + 1}`,
        days: 30,
      }));
  weekdays.forEach((day) =>
    addCalendarWeekday(within("#calendar-weekdays"), day),
  );
  months.forEach((month) =>
    addCalendarMonth(within("#calendar-months"), month),
  );
  (calendar.seasons || []).forEach((season) =>
    addCalendarSeason(within("#calendar-seasons"), season, months),
  );
  within("#add-calendar-weekday").onclick = () =>
    addCalendarWeekday(within("#calendar-weekdays"), "");
  within("#add-calendar-month").onclick = () =>
    addCalendarMonth(within("#calendar-months"), {});
  within("#add-calendar-season").onclick = () =>
    addCalendarSeason(within("#calendar-seasons"), {}, readCalendarMonths());
}
function readCalendarMonths() {
  return $$(".month-row")
    .map((row, index) => ({
      key: technicalKey(
        fieldValue("calendar_month_name", row),
        `month_${index + 1}`,
      ),
      name: fieldValue("calendar_month_name", row),
      days: fieldValue("calendar_month_days", row),
    }))
    .filter((month) => month.name);
}
function readCalendar(previousOverride = null) {
  const months = readCalendarMonths(),
    previous = previousOverride || state.editing?.payload?.calendar || {};
  return {
    name: fieldValue("calendar_name") || "Calendrier du Royaume",
    start_year: fieldValue("calendar_start_year") || 1,
    start_month_key:
      previous.start_month_key &&
      months.some((month) => month.key === previous.start_month_key)
        ? previous.start_month_key
        : months[0]?.key,
    start_day: fieldValue("calendar_start_day") || 1,
    weekdays: $$(".weekday-row")
      .map((row) => fieldValue("calendar_weekday", row))
      .filter(Boolean),
    months,
    seasons: $$(".season-row")
      .map((row, index) => ({
        key: technicalKey(
          fieldValue("calendar_season_name", row),
          `season_${index + 1}`,
        ),
        name: fieldValue("calendar_season_name", row),
        emoji: fieldValue("calendar_season_emoji", row),
        start_month_key: fieldValue("calendar_season_month", row),
        start_day: fieldValue("calendar_season_day", row),
        modifiers: [],
      }))
...
function renderFields(payload) {
  ...
  if (state.type === "interface") {
    renderInterfaceFields(payload);
    return;
  }
  if (state.type === "bot") {
    renderBotFields(payload);
    return;
  }
  if (state.type === "audio") {
    renderAudioFields(payload);
    return;
  }
  const root = $("#type-fields");
  if (state.type === "item") {
    $(".wizard-panel").classList.add("item-mode");
    const relations = payload.building_relations || [];
    root.innerHTML = `<section class="form-section"><h3>Propriétés de l’objet</h3><p class="field-note">Le nom est destiné aux joueurs. L’identifiant technique reste stable après la création afin de protéger les recettes et inventaires existants.</p><div class="form-grid">${select(
      "Catégorie",
      "category",
      payload.category || "resources",
      [
        ["drinks", "Boisson / repas"],
    ...
    )}${input("Type", "type", payload.type || "ressource")}${select(
      "Rareté",
      "rarity",
      payload.rarity || "commun",
      [
        ["commun", "Commun"],
  ...
  if (state.type === "event") {
    ...
    (payload.audio_layers || []).forEach((layer) =>
  ...
  if (state.type === "npc") {
    ...
    const linkedProfile = state.catalogs.voice_profile.find(
      ...
      defaultVoice = linkedProfile?.payload.clips?.[0]?.audio_key || "";
    root.innerHTML = `<section class="form-section npc-simple-section"><div class="wizard-section-copy"><span>1</span><div><h3>Identité et rôle</h3><p>Décrivez simplement la place du personnage dans votre monde.</p></div></div><div class="form-grid">${input("Rôle", "npc_role", payload.role || "")}${input("Tags", "npc_tags", (payload.tags || []).join(", "))}</div></section>
      <section class="form-section npc-simple-section"><div class="wizard-section-copy"><span>2</span><div><h3>Voix</h3><p>Choisissez une voix ; KingdomWeb crée et relie automatiquement la bibliothèque vocale nécessaire.</p></div></div><div class="form-grid">${select("Voix ou clip principal", "npc_voice_audio", defaultVoice, [["", "À fournir plus tard"], ...state.catalogs.audio.filter((item) => item.payload.audio_type === "voice" || item.payload.channel === "voice").map((item) => [item.entity_key, `${item.payload.emoji || "🗣️"} ${item.payload.name}`])])}${input("Volume de la voix", "npc_voice_volume", linkedProfile?.payload.volume ?? 1, "number", 'min="0" max="2" step="0.05"')}</div></section>
  ...
  if (state.type === "environment") {
    ...
    root.innerHTML = `<section class="form-section"><h3>Horloge du royaume</h3><div class="form-grid">${select(
      "Horloge",
      "clock_mode",
      payload.clock_mode || "accelerated",
      [
        ["manual", "Heure administrée"],
        ["accelerated", "Temps autonome"],
      ],
    )}${input("Jour de départ", "environment_day", payload.day || 1, "number", 'min="1"')}${input("Heure de départ", "environment_hour", payload.hour ?? 12, "number", 'min="0" max="23"')}${input("Minute de départ", "environment_minute", payload.minute ?? 0, "number", 'min="0" max="59"')}${input("Vitesse du temps", "environment_speed", payload.speed ?? 1, "number", 'min="0" step="0.1"')}</div></section><section class="form-section"><h3>Météo</h3><div class="form-grid">${select(
      "Mode",
      "environment_mode",
      payload.mode || "manual",
      [
        ["manual", "Manuel"],
        ["weighted", "Aléatoire pondéré"],
        ["scheduled", "Selon l’heure"],
      ],
    )}${input("Météo actuelle", "weather_name", weather.name || "Beau")}${input("Emoji", "weather_emoji", weather.emoji || "☀️")}${input("Changer toutes les (secondes)", "weather_interval", payload.weather_interval_seconds || 3600, "number", 'min="1"')}</div><div class="section-head"><h3>Météos possibles</h3><button type="button" class="secondary" id="add-weather-option">＋ Ajouter une météo</button></div><div id="weather-options"></div><div class="section-head"><h3>Influences de la météo actuelle</h3><button type="button" class="secondary" id="add-world-modifier">＋ Ajouter</button></div><div id="world-modifiers"></div></section>`;
    (payload.weather_options || []).forEach((item) =>
      addWeatherOption($("#weather-options"), item),
    );
    $("#add-weather-option").onclick = () =>
      addWeatherOption($("#weather-options"), {});
    (weather.modifiers || []).forEach((item) =>
      addWorldModifier($("#world-modifiers"), item),
    );
    $("#add-world-modifier").onclick = () =>
      addWorldModifier($("#world-modifiers"), {});
  ...
  if (state.type === "location") {
    root.innerHTML = `<section class="form-section"><h3>Place dans le monde</h3><p class="field-note">Choisissez ce que représente ce lieu et l’endroit qui le contient. Aucun identifiant technique n’est nécessaire.</p><div class="form-grid">${select(
      "Type",
      "location_type",
      payload.location_type || "place",
      [
        ["kingdom", "Royaume"],
        ["region", "Région"],
        ["city", "Ville"],
        ...
        ["road", "Route"],
        ["secret", "Passage secret"],
        ["special", "Destination spéciale"],
      ],
    )}${select("Appartient à", "location_parent", payload.parent_key || "", locationOptions(payload.parent_key || "", state.editing?.entity_key || ""))}${input("Tags", "location_tags", (payload.tags || []).join(", "))}${input("Position carte X", "location_map_x", payload.map?.x ?? 0, "number")}${input("Position carte Y", "location_map_y", payload.map?.y ?? 0, "number")}</div><div class="checks">${check("Exploration autorisée", "location_exploration", !!payload.exploration_enabled)}</div></section><section class="form-section"><div class="section-head"><div><h3>Activités disponibles ici</h3><p class="field-note">Associez les activités existantes : leur gameplay reste défini dans leur bâtiment.</p></div><button type="button" class="secondary" id="add-location-activity">＋ Associer une activité</button></div><div id="location-activities"></div></section><section class="form-section"><div class="section-head"><div><h3>Chemins depuis ce lieu</h3><p class="field-note">Les chemins secrets restent invisibles au joueur jusqu’à leur découverte.</p></div><button type="button" class="secondary" id="add-location-connection">＋ Créer une connexion</button></div><div id="location-connections"></div></section>`;
    (payload.activities || []).forEach((reference) =>
      addLocationActivity($("#location-activities"), reference),
    );
    $("#add-location-activity").onclick = () =>
      addLocationActivity($("#location-activities"), {});
    (payload.connections || []).forEach((connection) =>
      addLocationConnection($("#location-connections"), connection),
    );
    $("#add-location-connection").onclick = () =>
      addLocationConnection($("#location-connections"), {});
  }
  if (state.type === "bot")
    root.innerHTML = `<section class="form-section"><h3>Identité et connexion Discord</h3><div class="form-grid">${select(
      "Type de bot",
      "bot_type",
      payload.bot_type || "text",
      [
  ...
  if (state.type === "audio")
    ...
    )}${input("Volume", "volume", payload.volume ?? 0.5, "number", 'min="0" max="1" step="0.05"')}</div>${check("Lecture en boucle", "loop", !!payload.loop)}</section>`;
  if (state.type === "item" && state.editing?.entity_key) {
    root.insertAdjacentHTML(
      "beforeend",
      '<section class="form-section" id="item-world-usage"><h3>Relations calculées</h3><p>Chargement des usages réels…</p></section>',
    );
    fetch(
      `/api/world/items/${encodeURIComponent(state.editing.entity_key)}/usage`,
      { headers },
    )
      .then((response) => response.json())
      .then((usage) => {
        const panel = $("#item-world-usage");
        if (!panel) return;
        const rows = [
          ...usage.tools.map(
            (item) =>
              `Utilisé comme outil · ${item.profession || item.activity} · ${item.building}`,
          ),
          ...usage.produced.map(
            (item) =>
              `Produit par ${item.activity || item.recipe} · ${item.building}`,
          ),
          ...usage.consumed.map(
            (item) => `Consommé par ${item.recipe} · ${item.building}`,
          ),
          ...usage.deliveries.map(
            (item) => `Livré de ${item.from} vers ${item.to}`,
          ),
        ];
        panel.innerHTML = `<h3>Relations calculées</h3>${rows.map((text) => `<article class="world-relation-row"><b>${escapeHtml(text)}</b></article>`).join("") || '<p class="simple-empty">Aucun usage détecté.</p>'}`;
      });
  }
  if (state.type === "npc") {
    ...
    $("#type-fields").insertAdjacentHTML(
      ...
      `<section class="form-section npc-voice-links">
        <div class="section-head">
          ...
          <span class="technical-badge">AUTOMATIQUE</span>
        </div>
        ...
        <p class="field-note"><b>À retenir :</b> le clip principal définit sa voix. Les sons précis joués en réaction se choisissent dans les variantes de « Comportement ».</p>
      </section>`,
    );
  }
}
...
const COMPONENT_LIBRARY = {
  ...
  sequence: {
    ...
    props: {
      steps: [
        { text: "Premier texte affiché", delay_seconds: 0 },
        { text: "Texte suivant", delay_seconds: 2 },
      ],
    },
  },
  card: {
    name: "Carte",
    icon: "▣",
    props: { title: "Titre de la carte", text: "Contenu de la carte" },
  },
  stat: {
    name: "Indicateur",
    icon: "◫",
    props: { label: "Indicateur", value: "42" },
  },
  divider: { name: "Séparateur", icon: "—", props: {} },
  image: { name: "Image", icon: "🖼️", props: { url: "", alt: "Illustration" } },
  player_inventory: {
    name: "Inventaire du joueur",
    icon: "🎒",
    props: { title: "Contenu du sac" },
  },
  building_inventory: {
    name: "Inventaire du bâtiment",
    icon: "📦",
  ...
  button: {
    name: "Bouton",
    icon: "◉",
    props: { label: "Continuer", emoji: "", style: "primary" },
    interaction: { type: "navigate", page: "home" },
  },
  select: {
    name: "Menu déroulant",
    icon: "⌄",
    props: { placeholder: "Choisir une option…" },
    options: [
      {
        key: "option_1",
        label: "Option 1",
...
const PREDEFINED_INTERACTIONS = {
  ...
  close: {
    ...
    component: {
      type: "button",
      props: { label: "Quitter", emoji: "✖️", style: "danger" },
...
function renderInterfaceFields(payload) {
  $(".wizard-panel").classList.add("visual-mode");
  $("#context-help").hidden = true;
...
function blankInterface(
  ...
  return {
    name: `Interface - ${name}`,
    target_building_key: buildingKey,
    start_page: "home",
    theme: { color, density: "comfortable", radius: 12 },
    pages: [
      {
        key: "home",
        name: "Accueil",
        components: [
          {
            id: "hero_accueil",
            type: "hero",
            props: { title: name, subtitle: "Choisissez une action.", emoji },
          },
        ],
      },
    ],
...
function initializeInterfaceDraft(payload = {}, buildingKey = "") {
  state.interfaceDraft = clone(
    Object.keys(payload || {}).length ? payload : blankInterface(buildingKey),
  );
  state.interfaceDraft.pages ||= [
        ...
        if (!Number.isInteger(component.slot)) {
          while (
            page.components.some(
              (other) => other !== component && other.slot === nextSlot,
            )
          )
            nextSlot++;
          component.slot =
...
function componentTiles(types) {
  return types
    .map((key) => {
      const item = COMPONENT_LIBRARY[key];
      return `<button type="button" class="component-tile" draggable="true" data-component-type="${key}"><span>${item.icon}</span><b>${item.name}</b></button>`;
    })
    .join("");
}
...
function currentInterfacePage() {
  return (
    state.interfaceDraft.pages.find(
      (page) => page.key === state.selectedPage,
    ) || state.interfaceDraft.pages[0]
  );
}
function currentInterfaceComponent() {
  return currentInterfacePage().components.find(
    (component) => component.id === state.selectedComponent,
  );
}
function simpleDiscordMarkup() {
  const pages = state.interfaceDraft.pages || [],
        ...
        (page.components || []).flatMap((component) =>
          [
            ...
            ...(component.options || []).map(
              (option) => option.interaction?.action,
            ),
          ].filter(Boolean),
        ),
      ),
    );
  return `<section class="simple-discord-panel"><div class="simple-section-head"><div><small>INTERFACE DISCORD</small><h3>${pages.length} page(s) · ${components} composant(s)</h3><p>${actions.size} action(s) moteur reliée(s), sans modifier leurs identifiants.</p></div><button type="button" data-open-full-builder>Ouvrir le builder complet</button></div><div class="simple-discord-pages">${
    pages
      .map((page, index) => {
        const linked = [
          ...new Set(
        ...
        return `<article><span>${page.key === state.interfaceDraft.start_page ? "🏠" : "📄"}</span><div><small>${page.key === state.interfaceDraft.start_page ? "PAGE D’ACCUEIL" : "PAGE"}</small><h4>${escapeHtml(page.name || page.key)}</h4><p>${page.components.length} composant(s)${
          linked.length
            ? ` · ${linked
                .slice(0, 3)
...
function actionDisplayName(key) {
  const action = $$("#actions > .action-builder").find(
    (element) => fieldValue("action_key", element) === key,
  );
  return action ? fieldValue("action_name", action) : key;
}
function refreshSimpleDiscord() {
  const panel = $('[data-building-panel="visual"]');
  if (!panel) return;
...
function openSimplePageEditor(index, created = false) {
  ...
  if (!page) return;
  const simpleTypes = new Set(["hero", "text", "sequence", "button"]),
    advancedCount = page.components.filter(
      (component) => !simpleTypes.has(component.type),
    ).length,
  ...
  const dialog = simpleDialog(
    "simple-page-dialog",
    "DISCORD › PAGES",
    `📄 ${escapeHtml(page.name || page.key)}`,
    `<div class="simple-zone-form"><label>Nom de la page<input name="name" value="${escapeHtml(page.name || "")}" required></label><label class="check"><input name="start" type="checkbox" ${page.key === state.interfaceDraft.start_page ? "checked" : ""}><span>Page d’accueil</span></label><section class="simple-results"><div><h3>Composants</h3><small>Les composants complexes restent intacts.</small></div>${components || '<p class="simple-empty">Aucun composant.</p>'}<button type="button" class="secondary" data-add-simple-text>＋ Ajouter un texte</button><button type="button" class="secondary" data-add-simple-button>＋ Ajouter un bouton</button></section>${advancedCount ? `<p class="field-note">⚙ ${advancedCount} composant(s) avancé(s) conservé(s).</p>` : ""}<button type="button" class="secondary" data-page-full-builder>Ouvrir dans le builder complet</button></div>`,
    (data, current) => {
      page.name = data.get("name");
      if (data.get("start") === "on")
        state.interfaceDraft.start_page = page.key;
      current.close();
      markEditorDirty();
      refreshSimpleDiscord();
    },
  );
  dialog
    .querySelectorAll("[data-simple-component]")
    .forEach(
      (button) =>
        (button.onclick = () =>
          openSimpleComponentEditor(
            index,
            Number(button.dataset.simpleComponent),
          )),
    );
  dialog.querySelector("[data-add-simple-text]").onclick = () => {
    page.components.push(newComponent("text"));
    openSimplePageEditor(index);
  };
  dialog.querySelector("[data-add-simple-button]").onclick = () => {
    page.components.push(newComponent("button"));
    openSimplePageEditor(index);
  };
  dialog.querySelector("[data-page-full-builder]").onclick = () => {
    dialog.close();
    state.selectedPage = page.key;
    const panel = $('[data-building-panel="visual"]');
    panel.querySelector(".simple-discord-panel").hidden = true;
    panel.querySelector(".visual-studio").hidden = false;
    renderVisualStudio();
  };
  if (created)
    dialog.querySelectorAll("[data-local-cancel]").forEach(
      (button) =>
        (button.onclick = () => {
          state.interfaceDraft.pages.splice(index, 1);
          dialog.close();
          refreshSimpleDiscord();
        }),
    );
}
function openSimpleComponentEditor(pageIndex, componentIndex) {
  const page = state.interfaceDraft.pages[pageIndex],
    component = page?.components[componentIndex];
  if (!component) return;
  ...
  const isButton = component.type === "button",
    ...
    actionOptions = [
      ["", "Aucune action"],
      ...$$("#actions > .action-builder").map((element) => [
        fieldValue("action_key", element),
        fieldValue("action_name", element) || fieldValue("action_key", element),
      ]),
    ];
  const dialog = simpleDialog(
    "simple-component-dialog",
    "DISCORD › COMPOSANT",
    `${COMPONENT_LIBRARY[component.type]?.icon || "📝"} ${COMPONENT_LIBRARY[component.type]?.name || component.type}`,
    `<div class="simple-zone-form"><label>${isButton ? "Texte du bouton" : "Contenu"}<textarea name="content" rows="4">${escapeHtml(value)}</textarea></label>${isButton ? `<label>Action<select name="action">${actionOptions.map(([id, label]) => `<option value="${escapeHtml(id)}" ${component.interaction?.action === id ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}</select></label><label>Style<select name="style"><option value="primary">Principal</option><option value="secondary" ${component.props.style === "secondary" ? "selected" : ""}>Secondaire</option><option value="success" ${component.props.style === "success" ? "selected" : ""}>Succès</option><option value="danger" ${component.props.style === "danger" ? "selected" : ""}>Danger</option></select></label>` : ""}</div>`,
    (data, current) => {
      if (isButton) {
        component.props.label = data.get("content");
        component.props.style = data.get("style");
        const old = component.interaction || {};
        component.interaction = {
          ...old,
          type: "action",
          building: state.interfaceDraft.target_building_key,
          action: data.get("action"),
        };
      } else if (component.type === "hero")
        component.props.title = data.get("content");
      else component.props.text = data.get("content");
      current.close();
      markEditorDirty();
      openSimplePageEditor(pageIndex);
    },
  );
  return dialog;
}
function bindSimpleDiscord() {
  $$("[data-simple-page]").forEach(
    (button) =>
      (button.onclick = () =>
        openSimplePageEditor(Number(button.dataset.simplePage))),
  );
  $("[data-add-simple-page]")?.addEventListener("click", () => {
    const index = state.interfaceDraft.pages.length,
      key = technicalKey(`page_${index + 1}`);
    state.interfaceDraft.pages.push({
      key,
      name: `Page ${index + 1}`,
      components: [],
    });
    openSimplePageEditor(index, true);
  });
  $("[data-open-full-builder]")?.addEventListener("click", () => {
    const panel = $('[data-building-panel="visual"]');
    panel.querySelector(".simple-discord-panel").hidden = true;
    panel.querySelector(".visual-studio").hidden = false;
    renderVisualStudio();
...
function newComponent(type) {
  ...
  return {
    id: technicalKey(
      `component_${Date.now()}_${Math.floor(Math.random() * 9999)}`,
    ),
    type,
    props: clone(template.props),
    ...(template.interaction
      ? { interaction: clone(template.interaction) }
      : {}),
    ...(template.options ? { options: clone(template.options) } : {}),
  };
}

function bindVisualStudio() {
  $$("[data-component-type]").forEach((tile) =>
    tile.addEventListener("dragstart", (event) =>
      event.dataTransfer.setData(
        "text/plain",
        `new:${tile.dataset.componentType}`,
      ),
    ),
  );
  $$("[data-component-preset]").forEach((tile) =>
    tile.addEventListener("dragstart", (event) =>
      event.dataTransfer.setData(
        "text/plain",
        `preset:${tile.dataset.componentPreset}`,
      ),
    ),
  );
  ...
  canvas.ondrop = (event) => {
    ...
    let index = target
      ? Math.max(
          ...
          page.components.findIndex(
            (item) => item.id === target.dataset.componentId,
          ),
        )
    ...
    if (token.startsWith("move:")) {
      ...
      const old = page.components.findIndex((item) => item.id === id);
      if (old >= 0) {
        const [component] = page.components.splice(old, 1);
        if (old < index) index--;
        page.components.splice(index, 0, component);
...
function slotComponent(page, slot) {
  return (
    page.components.find(
      (component) => component.type === "button" && component.slot === slot,
    ) ||
    page.components.find(
      (component) =>
        FULL_ROW_INTERACTION_TYPES.has(component.type) &&
...
function placeInteraction(type, slot) {
  ...
  if (
    ...
    alert("Cette ligne contient déjà une interaction.");
    return;
  }
  if (type === "button" && slotComponent(page, slot)) {
...
function renderInteractionGrid(page) {
  ...
  for (let row = 0; row < 5; row++) {
    ...
    for (let column = 0; column < 5; column++) {
      const slot = row * 5 + column,
        button = page.components.find(
          (component) => component.type === "button" && component.slot === slot,
        );
      html += button
        ? `<button type="button" draggable="true" class="interaction-slot filled ${button.id === state.selectedComponent ? "selected" : ""}" data-component-id="${button.id}" data-slot="${slot}"><small>${slot + 1}</small><b>${escapeHtml(button.props?.emoji || "")} ${escapeHtml(button.props?.label || "Bouton")}</b></button>`
        : `<button type="button" class="interaction-slot" data-slot="${slot}"><small>${slot + 1}</small><span>＋</span></button>`;
    }
  }
  return html;
}
...
function pageLinks(page) {
  ...
  for (const component of page.components || []) {
    if (component.interaction?.type === "navigate")
      links.push(component.interaction.page);
    for (const option of component.options || [])
      if (option.interaction?.type === "navigate")
        links.push(option.interaction.page);
  }
  return [...new Set(links)].filter((key) =>
    state.interfaceDraft.pages.some((page) => page.key === key),
  );
}
function renderPageLinkGraph() {
  const pages = state.interfaceDraft.pages,
    byKey = Object.fromEntries(pages.map((page) => [page.key, page]));
  const depth = { [state.interfaceDraft.start_page]: 0 },
    queue = [state.interfaceDraft.start_page];
  while (queue.length) {
    const source = queue.shift();
    for (const target of pageLinks(byKey[source] || { components: [] })) {
  ...
  pages.forEach((page) => {
    if (depth[page.key] === undefined) depth[page.key] = maxDepth + 1;
  });
  const columns = {};
  pages.forEach((page) => (columns[depth[page.key]] ||= []).push(page));
  const nodeWidth = 180,
    nodeHeight = 54,
    columnGap = 85,
    rowGap = 24,
    padding = 28;
  const columnCount = Math.max(...Object.keys(columns).map(Number)) + 1,
    maxRows = Math.max(...Object.values(columns).map((items) => items.length));
  const width =
      padding * 2 + columnCount * nodeWidth + (columnCount - 1) * columnGap,
    height = padding * 2 + maxRows * nodeHeight + (maxRows - 1) * rowGap;
  const positions = {};
  Object.entries(columns).forEach(([column, items]) =>
    items.forEach((page, index) => {
      positions[page.key] = {
        x: padding + Number(column) * (nodeWidth + columnGap),
  ...
  const edges = pages
    .flatMap((page) =>
      pageLinks(page).map((target) => ({ source: page.key, target })),
    )
  ...
  const lines = edges
    .map((edge) => {
      const from = positions[edge.source],
        ...
        bend = (x1 + x2) / 2;
      return `<path d="M ${x1} ${y1} C ${bend} ${y1}, ${bend} ${y2}, ${x2} ${y2}" class="page-graph-edge" marker-end="url(#page-arrow)"/>`;
    })
    .join("");
  const nodes = pages
    .map((page) => {
      const position = positions[page.key],
        active = page.key === state.selectedPage ? " active" : "",
        label =
          `${page.key === state.interfaceDraft.start_page ? "★ " : ""}${page.name}`.slice(
            0,
            24,
          );
      return `<g class="page-graph-svg-node${active}" data-graph-page="${page.key}" transform="translate(${position.x} ${position.y})"><rect width="${nodeWidth}" height="${nodeHeight}" rx="9"/><text x="12" y="23">${escapeHtml(label)}</text><text class="node-key" x="12" y="41">${escapeHtml(page.key)}</text></g>`;
    })
    .join("");
...
function renderVisualStudio() {
  ...
  $$("[data-graph-page]").forEach(
    (button) =>
      (button.onclick = () => {
        state.selectedPage = button.dataset.graphPage;
        state.selectedComponent = null;
        renderVisualStudio();
      }),
  );
  $("#page-tree").innerHTML = state.interfaceDraft.pages
    .map(
      (item) =>
        `<div class="page-row"><button type="button" class="page-button ${item.key === page.key ? "active" : ""}" data-page="${item.key}">${item.key === state.interfaceDraft.start_page ? "★ " : ""}${escapeHtml(item.name)}</button><div class="mini-actions"><button type="button" title="Dupliquer" data-copy-page="${item.key}">⧉</button></div></div>`,
    )
    .join("");
  $$("[data-page]").forEach(
    (button) =>
  ...
  $$("[data-copy-page]").forEach(
    (button) =>
      (button.onclick = () => {
        const source = state.interfaceDraft.pages.find(
          (item) => item.key === button.dataset.copyPage,
        );
        const copy = clone(source);
        copy.key = technicalKey(`page_copy_${Date.now()}`);
        copy.name = `${source.name} (copie)`;
        copy.components.forEach(
          (component, index) =>
            (component.id = technicalKey(
              `${component.type}_${Date.now()}_${index}`,
            )),
        );
        state.interfaceDraft.pages.push(copy);
        state.selectedPage = copy.key;
        state.selectedComponent = null;
        renderVisualStudio();
      }),
  );
  ...
  $$("[data-component-id]").forEach((element) => {
    ...
    element.ondragstart = (event) =>
      event.dataTransfer.setData(
        "text/plain",
        `move:${element.dataset.componentId}`,
      );
  });
  $$("#interaction-grid [data-slot]").forEach((cell) => {
    ...
    cell.ondrop = (event) => {
      ...
      if (token.startsWith("new:")) {
        ...
        if (component && INTERACTIVE_COMPONENT_TYPES.has(component.type)) {
          ...
          const occupied = slotComponent(page, slot);
          if (occupied) {
            page.components.push(component);
            alert("Cet emplacement est déjà occupé.");
          } else {
...
function renderCanvasComponent(component) {
  ...
  if (component.type === "hero")
    content = `<div class="preview-hero"><small>${escapeHtml(props.emoji || "")}</small><h2>${escapeHtml(props.title || "Sans titre")}</h2><p>${escapeHtml(props.subtitle || "")}</p></div>`;
  if (component.type === "text")
    content = `<div class="preview-text">${escapeHtml(props.text || "")}</div>`;
  if (component.type === "sequence")
    content = `<div class="preview-card sequence-preview"><b>▶ Séquence de texte</b>${(props.steps || []).map((step, index) => `<p><small>Après ${step.delay_seconds || 0} s</small> ${escapeHtml(step.text || `Étape ${index + 1}`)}</p>`).join("")}</div>`;
  if (component.type === "card")
    content = `<div class="preview-card"><b>${escapeHtml(props.title || "Carte")}</b><p>${escapeHtml(props.text || "")}</p></div>`;
  if (component.type === "stat")
    content = `<div class="preview-stat"><small>${escapeHtml(props.label || "Indicateur")}</small><strong>${escapeHtml(props.value || "—")}</strong></div>`;
  if (component.type === "divider")
    content = `<div class="preview-divider"></div>`;
  if (component.type === "image")
    content = `<div class="preview-image">${props.url ? `<img src="${escapeHtml(props.url)}" alt="${escapeHtml(props.alt || "")}">` : "Ajoutez une URL d’image"}</div>`;
...
function propertyInput(label, key, value, type = "text") {
  return `<label>${label}<input data-prop="${key}" type="${type}" value="${escapeHtml(value ?? "")}"></label>`;
}
function propertySelect(label, key, value, options) {
  return `<label>${label}<select data-prop="${key}">${options.map(([option, text]) => `<option value="${option}" ${option === value ? "selected" : ""}>${text}</option>`).join("")}</select></label>`;
}
function renderPropertyPanel() {
  const page = currentInterfacePage(),
    component = currentInterfaceComponent(),
    panel = $("#property-panel");
  const buildingOptions = [
    ["", "Aucun bâtiment"],
    ...state.catalogs.building.map((item) => [
      item.entity_key,
  ...
  const buildingField =
    state.type === "building"
      ? `<p class="field-note">Cette interface appartient au bâtiment courant.</p>`
      : propertySelect(
          "Bâtiment principal",
          "interface_building",
  ...
  const pageFields = `<h4>Interface</h4>${buildingField}${propertyInput("Couleur", "theme_color", state.interfaceDraft.theme?.color || "7a1f1f")}${propertySelect(
    "Densité",
    "theme_density",
    state.interfaceDraft.theme?.density || "comfortable",
    [
      ["compact", "Compacte"],
  ...
  if (component.type === "hero")
    fields =
      propertyInput("Titre", "title", props.title) +
      propertyInput("Sous-titre", "subtitle", props.subtitle) +
      propertyInput("Emoji", "emoji", props.emoji);
  if (component.type === "text")
    fields = `<label>Texte<textarea data-prop="text" rows="5">${escapeHtml(props.text || "")}</textarea></label>`;
  if (component.type === "sequence") fields = sequencePropertyFields(component);
  if (component.type === "card") {
    const access = component.access_when?.profession_level || {};
    fields =
      propertyInput("Titre", "title", props.title) +
      `<label>Contenu<textarea data-prop="text" rows="4">${escapeHtml(props.text || "")}</textarea></label>` +
      propertySelect("Disposition Discord", "inline", String(!!props.inline), [
        ["false", "Une carte par ligne"],
        ["true", "Cartes côte à côte"],
      ]) +
      propertyInput(
        "Texte si verrouillé",
        "locked_label",
        props.locked_label || "Niveau {level} requis",
      ) +
      propertySelect(
        "Condition d'accès",
        "access_type",
        access.profession ? "profession_level" : "none",
        [
          ["none", "Toujours accessible"],
          ["profession_level", "Niveau de métier minimum"],
        ],
      ) +
      (access.profession
        ? propertyInput(
  ...
  if (component.type === "stat")
    fields =
      propertyInput("Libellé", "label", props.label) +
      propertyInput("Valeur", "value", props.value);
  if (component.type === "image")
    fields =
      propertyInput("URL", "url", props.url) +
      propertyInput("Texte alternatif", "alt", props.alt);
  if (component.type === "player_inventory")
    fields = propertyInput("Titre", "title", props.title || "Contenu du sac");
  if (component.type === "building_inventory")
    fields =
      propertyInput("Titre", "title", props.title || "Stock commun") +
      propertySelect(
        "Bâtiment affiché",
        "building",
        props.building || state.interfaceDraft.target_building_key,
        buildingTargetOptions(
          props.building || state.interfaceDraft.target_building_key,
        ),
      );
  if (component.type === "world_weather")
...
function buttonPropertyFields(component) {
  const props = component.props || {},
    interaction = component.interaction || {
      type: "navigate",
      page: state.interfaceDraft.start_page,
    };
  let fields =
    propertyInput("Libellé", "label", props.label) +
    propertyInput("Emoji", "emoji", props.emoji) +
    propertySelect("Style", "style", props.style || "primary", [
      ["primary", "Principal"],
      ["secondary", "Secondaire"],
      ["success", "Succès"],
      ["danger", "Danger"],
    ]) +
    propertySelect("Au clic", "interaction_type", interaction.type, [
      ["navigate", "Ouvrir une page"],
      ["action", "Lancer une action"],
      ["world_state", "Afficher l’état du monde"],
      ["world_travel", "Voyager vers un lieu"],
      ["refresh", "Actualiser la page"],
      ["close", "Quitter / fermer"],
    ]);
  if (interaction.type === "navigate")
    fields += propertySelect(
      "Page cible",
      "target_page",
      interaction.page,
      state.interfaceDraft.pages.map((page) => [page.key, page.name]),
    );
    ...
    const building =
      state.type === "building"
        ...
        : interaction.building ||
          state.interfaceDraft.target_building_key ||
          state.catalogs.building[0]?.entity_key ||
          "";
    ...
    fields += propertySelect(
      ...
      "target_building",
      building,
      buildingTargetOptions(building),
    );
    fields += propertySelect(
      "Action",
      "target_action",
      ...
      [
        ["", "Choisir…"],
        ...actions.map((action) => [action.key, action.name || action.key]),
      ],
    );
  ...
  } else if (interaction.type === "world_travel")
    fields += propertySelect(
      "Destination",
      "world_destination",
      interaction.destination || "",
      locationOptions(interaction.destination || ""),
    );
  return fields;
}

function buildingTargetOptions(current) {
  const options = state.catalogs.building.map((item) => [
    item.entity_key,
    `${item.payload.emoji || "🏰"} ${item.payload.name}`,
  ]);
  if (current && !options.some(([key]) => key === current))
    options.unshift([current, "🏰 Bâtiment courant"]);
  return options;
}

function availableActions(building) {
  const currentKey = $("#key")?.value;
  if (state.type === "building" && building === currentKey && $("#actions"))
    return $$("#actions > .action-builder").map((element, index) => ({
      key:
        fieldValue("action_key", element) ||
        technicalKey(fieldValue("action_name", element), `action_${index + 1}`),
      name: fieldValue("action_name", element) || `Action ${index + 1}`,
    }));
  return (
    state.catalogs.building.find((item) => item.entity_key === building)
      ?.payload?.actions || []
  );
}

function interactionFields(interaction, prefix = "") {
  let fields = propertySelect(
    "Au choix",
    `${prefix}interaction_type`,
    interaction.type || "navigate",
    [
      ["navigate", "Ouvrir une page"],
      ["action", "Lancer une action"],
      ["purchase", "Acheter l’objet sélectionné"],
    ],
  );
  if ((interaction.type || "navigate") === "navigate")
    return (
      fields +
      propertySelect(
        "Page cible",
        `${prefix}target_page`,
        interaction.page || state.interfaceDraft.start_page,
        state.interfaceDraft.pages.map((page) => [page.key, page.name]),
      )
    );
  if (interaction.type === "purchase")
    return (
      ...
      propertySelect(
        ...
        state.catalogs.item.map((item) => [
          item.entity_key,
          `${item.payload.emoji || "📦"} ${item.payload.name}`,
        ]),
      ) +
      `<p class="field-note">Le prix et le stock viennent des Produits du bâtiment.</p>`
    );
  const building =
    state.type === "building"
      ? $("#key").value || state.interfaceDraft.target_building_key
      : interaction.building || state.interfaceDraft.target_building_key || "";
  const actions = availableActions(building);
  return (
    fields +
    propertySelect(
      "Bâtiment",
      `${prefix}target_building`,
      building,
      buildingTargetOptions(building),
    ) +
    propertySelect(
      "Action",
      `${prefix}target_action`,
      interaction.action || "",
      [
        ["", "Choisir…"],
        ...actions.map((action) => [action.key, action.name || action.key]),
    ...
    propertyInput(
      ...
      interaction.cooldown_seconds || 0,
      "number",
    )
  );
}
...
function newPresetComponent(preset) {
  const template = PREDEFINED_INTERACTIONS[preset];
  const component = clone(template.component);
  component.id = technicalKey(
    `component_${preset}_${Date.now()}_${Math.floor(Math.random() * 9999)}`,
  );
  if (preset === "back")
...
function sequencePropertyFields(component) {
  ...
  return `<p class="field-note">Le moteur remplace automatiquement le texte après chaque délai. Une étape conditionnelle est ignorée si le joueur ne remplit pas la condition.</p><div class="sequence-steps">${steps
    .map((step, index) => {
      const rule = step.visible_when?.profession_level || {};
      return `<article class="sequence-step"><button type="button" class="remove-option" data-remove-step="${index}">×</button><b>Étape ${index + 1}</b><label>Texte<textarea data-prop="sequence_${index}_text" rows="4">${escapeHtml(step.text || "")}</textarea></label>${propertyInput("Attendre avant affichage (secondes)", `sequence_${index}_delay_seconds`, step.delay_seconds || 0, "number")}${propertySelect(
        "Condition",
        `sequence_${index}_condition`,
        rule.profession ? "profession_level" : "none",
        [
          ["none", "Toujours afficher"],
          ["profession_level", "Niveau de métier minimum"],
        ],
      )}${rule.profession ? propertyInput("Identifiant du métier", `sequence_${index}_profession`, rule.profession) + propertyInput("Niveau minimum", `sequence_${index}_minimum`, rule.minimum || 1, "number") : ""}</article>`;
    })
    .join(
      "",
    )}</div><button type="button" class="secondary add-sequence-step">＋ Ajouter une étape</button>`;
}
...
function selectPropertyFields(component) {
  ...
  return (
    propertyInput(
      "Texte du menu",
      "placeholder",
      component.props?.placeholder || "Choisir une option…",
    ) +
    itemMenuPicker(component) +
    `<div class="select-options"><div class="section-head"><b>Options du menu (${options.length}/25)</b><button type="button" class="secondary add-select-option">＋ Option libre</button></div>${options
      .map((option, index) => {
        const access = option.access_when?.profession_level || {};
        return `<article class="select-option"><button type="button" class="remove-option" data-remove-option="${index}">×</button>${propertyInput("Libellé", `option_${index}_label`, option.label || "")}${propertyInput("Emoji", `option_${index}_emoji`, option.emoji || "")}${propertyInput("Description", `option_${index}_description`, option.description || "")}${propertySelect(
          "Condition d'accès",
          `option_${index}_access_type`,
          access.profession ? "profession_level" : "none",
          [
            ["none", "Toujours proposée"],
            ["profession_level", "Niveau de métier minimum"],
          ],
        )}${access.profession ? propertyInput("Métier", `option_${index}_access_profession`, access.profession) + propertyInput("Niveau minimum", `option_${index}_access_minimum`, access.minimum || 1, "number") : ""}${interactionFields(option.interaction || { type: "navigate", page: state.interfaceDraft.start_page }, `option_${index}_`)}</article>`;
      })
      .join("")}</div>`
  );
}
...
function professionCatalog() {
  ...
  for (const building of state.catalogs.building) {
    for (const profession of building.payload.modules?.professions || []) {
      ...
      const current = byKey.get(profession.key) || {
        key: profession.key,
        name: profession.name || profession.key,
        emoji: profession.emoji || "🛠️",
        buildings: [],
        definition: profession,
      };
      current.buildings.push({
        key: building.entity_key,
        name: building.payload.name,
      });
      byKey.set(profession.key, current);
    }
  }
...
function buildingRelationMarkup(payload, buildingKey, modules) {
  const localProfessions = modules.professions || [],
    knownProfessions = professionCatalog(),
    primary =
      payload.relations?.primary_profession_key ||
      localProfessions[0]?.key ||
      "";
  const assignedBots = state.catalogs.bot.filter(
      (entity) => entity.payload.building_key === buildingKey,
    ),
    selectedBot = assignedBots[0]?.entity_key || "";
  const ambience = payload.relations?.ambience_audio_key || "";
  const itemKeys = new Set();
  for (const product of modules.products || [])
    if (product.item_key || product.resource)
      itemKeys.add(product.item_key || product.resource);
  for (const recipe of modules.recipes || []) {
    if (recipe.output_item || recipe.output_resource)
      itemKeys.add(recipe.output_item || recipe.output_resource);
    Object.keys(recipe.ingredients || {}).forEach((key) => itemKeys.add(key));
  }
  const itemNames = [...itemKeys]
    .map((key) => itemDisplay(key))
  ...
  <article class="relation-picker"><span>🤖</span><div><b>Bot / PNJ Discord</b><small>${assignedBots.length ? "Association active" : "Aucun bot associé"}</small></div><input data-entity-search="bot" placeholder="Rechercher un bot…"><select data-field="relation_bot_key">${voiceBotOptions(
    selectedBot,
  )
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === selectedBot ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join(
      "",
    )}</select><div class="relation-actions"><button type="button" class="secondary" data-open-related="bot">Ouvrir</button><button type="button" class="danger-link" id="dissociate-building-bot" ${selectedBot ? "" : "disabled"}>Dissocier</button></div></article>
  <article class="relation-picker"><span>🔊</span><div><b>Ambiance globale</b><small>Une ambiance pour le salon vocal du bâtiment</small></div><input data-entity-search="ambience" placeholder="Rechercher une ambiance…"><select data-field="relation_ambience_key">${audioOptions(
    ambience,
    "ambience",
  )
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === ambience ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join(
      "",
    )}</select><div class="relation-actions"><button type="button" class="secondary" data-open-related="audio">Ouvrir Audio</button><button type="button" class="danger-link" id="remove-building-ambience" ${ambience ? "" : "disabled"}>Retirer</button></div></article></div></div>
  <aside class="building-relations-summary"><small>APERÇU DES RELATIONS</small><dl><div><dt>Métiers</dt><dd>${localProfessions.length ? localProfessions.map((item) => escapeHtml(item.name || item.key)).join(", ") : "Non configuré"}</dd></div><div><dt>Bot</dt><dd>${assignedBots.length ? assignedBots.map((item) => escapeHtml(item.payload.name)).join(", ") : "Non associé"}</dd></div><div><dt>Productions / objets</dt><dd>${
    itemNames.length
      ? itemNames
          .slice(0, 4)
          .map((item) => `${escapeHtml(item.emoji)} ${escapeHtml(item.name)}`)
          .join(" · ")
      : "Aucun contenu"
  }</dd></div><div><dt>Ambiance</dt><dd>${ambience ? escapeHtml(state.catalogs.audio.find((item) => item.entity_key === ambience)?.payload.name || ambience) : "Aucune"}</dd></div><div><dt>Interface Discord</dt><dd>${state.interfaceDraft?.pages?.length || 0} page(s)</dd></div></dl></aside></section>`;
}
...
function derivedBuildingRelationsMarkup(modules) {
  ...
  for (const profession of modules.professions || [])
    add(
      used,
      profession.required_item,
      `Outil du métier ${profession.name || profession.key}`,
    );
  for (const activity of modules.activities || []) {
    add(used, activity.tool, `Outil de ${activity.name || activity.key}`);
    for (const outcome of activity.outcomes || [])
      for (const effect of outcome.effects || [])
        if (["reward", "stock_reward"].includes(effect.type))
          add(
            produced,
            effect.resource || effect.item,
            `Produit dans ${activity.name || activity.key}`,
          );
  }
  for (const recipe of modules.recipes || []) {
    Object.keys(recipe.ingredients || {}).forEach((key) =>
      add(consumed, key, `Ingrédient de ${recipe.name || recipe.key}`),
    );
    add(
      produced,
      recipe.output_item_key,
      `Produit par ${recipe.name || recipe.key}`,
    );
  }
  for (const delivery of modules.deliveries || []) {
    const target = delivery.target_building_key || delivery.building;
    if (target) {
      if (!destinations.has(target)) destinations.set(target, []);
      destinations.get(target).push(delivery.item_key || delivery.resource);
    }
  }
  const itemCards = (title, map) =>
    `<section><h4>${title}</h4><div class="derived-relation-grid">${
      [...map]
        .map(([key, reasons]) => {
          const item = itemDisplay(key);
          return `<article><span>${item.emoji}</span><div><b>${escapeHtml(item.name)}</b>${reasons.map((reason) => `<small>${escapeHtml(reason)}</small>`).join("")}</div><button type="button" data-open-derived="item" data-key="${escapeHtml(key)}">Voir ↗</button></article>`;
        })
        .join("") || '<p class="simple-empty">Aucune relation.</p>'
    }</div></section>`;
  return `<section class="derived-relations"><div class="section-copy"><span class="step-dot">⌁</span><div><small>RELATIONS DÉRIVÉES</small><h3>Relations calculées depuis le gameplay</h3><p>Aucune relation supplémentaire n’est stockée.</p></div></div>${itemCards("OBJETS UTILISÉS", used)}${itemCards("OBJETS PRODUITS", produced)}${itemCards("OBJETS CONSOMMÉS", consumed)}<section><h4>DESTINATIONS DE LIVRAISON</h4><div class="derived-relation-grid">${
    [...destinations]
      .map(([key, items]) => {
        const building = state.catalogs.building.find(
          (item) => item.entity_key === key,
        );
        return `<article><span>${escapeHtml(building?.payload.emoji || "🏰")}</span><div><b>${escapeHtml(building?.payload.name || key)}</b><small>${items.length} ressource(s) livrée(s)</small></div><button type="button" data-open-derived="building" data-key="${escapeHtml(key)}">Voir ↗</button></article>`;
      })
      .join("") || '<p class="simple-empty">Aucune destination.</p>'
  }</div></section></section>`;
}
...
function buildingOverviewMarkup(payload, buildingKey, modules) {
  const professions = modules.professions || [],
    ...
    productions =
      (modules.products || []).length + (modules.recipes || []).length,
    actions =
      (payload.actions || []).length + (modules.activities || []).length,
  ...
  const ambienceGroupKey = modules.audio?.default_group_key || "",
    ...
    actions = (payload.actions || [])
      .map((action, index) => ({
        action,
        index,
        sounds: (action.effects || []).filter(
          (effect) => effect.type === "play_audio",
        ),
      }))
...
function openSimpleSfxEditor(index) {
  const element = $$("#actions > .action-builder")[index];
  if (!element) return;
  const effects = readEffects(element.querySelector(".action-effects")),
    audioEffects = effects
      .map((effect, i) => ({ effect, i }))
      .filter((item) => item.effect.type === "play_audio"),
    actionName = fieldValue("action_name", element) || `Action ${index + 1}`,
    selected = audioEffects[0]?.effect.audio_key || "";
  const dialog = simpleDialog(
    "simple-sfx-dialog",
    "AUDIO › SONS DU GAMEPLAY",
    `🔊 ${escapeHtml(actionName)}`,
    `<div class="simple-zone-form"><label>Son<select name="audio">${audioOptions(
      selected,
      "sfx",
    )
      .map(
    ...
    (data, current) => {
      const root = element.querySelector(".action-effects"),
        existing = root.querySelectorAll(".effect");
      if (audioEffects.length) {
        const target = existing[audioEffects[0].i],
          typeField = target.querySelector('[data-field="effect_type"]');
        typeField.value = "play_audio";
        typeField.dispatchEvent(new Event("change"));
        target.querySelector('[data-field="effect_audio_key"]').value =
          data.get("audio");
      } else if (data.get("audio"))
        addEffect(root, { type: "play_audio", audio_key: data.get("audio") });
      current.close();
      markEditorDirty();
      refreshSimpleAudio();
    },
  );
  dialog.querySelector("[data-remove-simple-sfx]").onclick = () => {
    if (audioEffects.length)
      element
        .querySelectorAll(".action-effects > .effect")
        [audioEffects[0].i]?.remove();
    dialog.close();
    markEditorDirty();
    refreshSimpleAudio();
  };
  dialog.querySelector("[data-sfx-advanced]").onclick = () => {
    dialog.close();
    $('[data-building-tab="advanced"]').click();
    element.open = true;
    element.scrollIntoView({ behavior: "smooth", block: "start" });
  };
}

function refreshSimpleAudio() {
  const panel = $('[data-building-panel="sound"]'),
    key = $("#key")?.value || "";
  if (!panel) return;
  panel.querySelector(".simple-audio-panel")?.remove();
  panel.insertAdjacentHTML(
    "afterbegin",
    simpleAudioMarkup(buildPayload(), key, buildPayload().modules || {}),
  );
  bindSimpleAudio();
}
function bindSimpleAudio() {
  ...
  $("[data-change-ambience]")?.addEventListener("click", () => {
    ...
    dialog.querySelector("[data-ambience-search]").oninput = (event) => {
      const q = event.target.value.toLowerCase();
      [...dialog.querySelector("select").options].forEach(
        (option, i) =>
          (option.hidden =
            i > 0 && !option.textContent.toLowerCase().includes(q)),
      );
    };
    dialog.querySelector("[data-open-audio-bank]").onclick = () => {
      dialog.close();
      closeEditor();
      navigateTo("audio");
    };
  ...
  $$("[data-simple-sfx]").forEach(
    (button) =>
      (button.onclick = () =>
        openSimpleSfxEditor(Number(button.dataset.simpleSfx))),
  );
  $("[data-open-audio-advanced]")?.addEventListener("click", () => {
    $('[data-building-tab="advanced"]').click();
  });
  $(
    '[data-building-panel="sound"] [data-open-related="bot"]',
  )?.addEventListener("click", () => {
    closeEditor();
    navigateTo("bot");
  });
...
function mechanicIcon(name = "") {
  ...
  if (/chass|hunt/.test(value)) return "🏹";
  if (/forge|forger|smith/.test(value)) return "⚒️";
  if (/cuis|boul|cook|baker/.test(value)) return "🍳";
  if (/mine|minier/.test(value)) return "⛏️";
  return "🪓";
}
function effectResourceSummary(effect) {
  const resource =
      effect.resource ||
      effect.item ||
      effect.item_key ||
      ...
      effect.event ||
      "Effet avancé",
    display = itemDisplay(resource);
  return `${display.missing ? "⚙️" : display.emoji} ${display.missing ? resource : display.name}`;
}
function gameplayProjection() {
  const professions = readProfessionModules(),
    activities = readActivityModules(),
    deliveries = readDeliveryModules().map((delivery) => ({
      ...delivery,
      resource: delivery.item_key || delivery.resource,
    })),
    products = readProductModules(),
    recipes = readRecipeModules();
  const activityKeys = new Set(
    activities.flatMap((activity) => [activity.key, `claim_${activity.key}`]),
  );
  const customActions = $$("#actions > .action-builder")
    .map((element, index) => ({
      element,
      index,
      key: fieldValue("action_key", element),
      name: fieldValue("action_name", element) || `Action ${index + 1}`,
      emoji: fieldValue("action_emoji", element) || "⚡",
    }))
    .filter((action) => {
      if (
        activityKeys.has(action.key) ||
        /^(join|leave|claim|deliver)(_|$)/.test(action.key)
      )
        return false;
      const effectTypes = readEffects(
        action.element.querySelector(".action-effects"),
...
function openSimpleZoneEditor(index, discardOnCancel = false) {
  const activityElement = $$("#activity-modules > .activity-module")[index];
  if (!activityElement) return;
  let dialog = $("#simple-zone-dialog");
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "simple-zone-dialog";
    document.body.append(dialog);
  }
  const activity = readActivityModules()[index],
    professionOptions = readProfessionModules()
      .map(
        (item) =>
          `<option value="${escapeHtml(item.key)}" ${item.key === activity.profession ? "selected" : ""}>${escapeHtml(item.name || item.key)}</option>`,
      )
      .join(""),
    totalWeight = activity.outcomes.reduce(
  ...
  dialog.innerHTML = `<form method="dialog"><div class="dialog-head"><div><small>MODE SIMPLE · ZONE</small><h2>${escapeHtml(activity.emoji || "🌿")} ${escapeHtml(activity.name || activity.key)}</h2></div><button value="cancel">×</button></div><div class="simple-zone-form"><label>Nom visible<input name="name" value="${escapeHtml(activity.name || "")}" required></label><div class="form-grid"><label>Métier<select name="profession">${professionOptions}</select></label><label>Durée en secondes<input name="duration" type="number" min="0" value="${activity.duration_seconds || 0}"></label><label>Coût énergétique<input name="energy" type="number" min="0" value="${activity.energy_cost || 0}"></label><label>Niveau requis<input name="level" type="number" min="0" value="${activity.required_level || 0}"></label><label>Outil requis<select name="tool">${catalogOptions(
    ...
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === activity.tool ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join(
      "",
    )}</select></label><label>Usure de l’outil<input name="durability" type="number" min="0" value="${activity.durability_cost || 0}"></label></div><section class="simple-results"><div><h3>Résultats possibles</h3><small>Les pourcentages sont informatifs ; le moteur conserve les poids.</small></div>${
    activity.outcomes
      .map((outcome, outcomeIndex) => {
        const simple =
          outcome.effects.length === 1 &&
          ["reward", "cost", "stock_reward"].includes(outcome.effects[0]?.type);
        return `<article><span>${effectResourceSummary(outcome.effects[0] || {})}</span><b>Poids ${outcome.weight}</b><small>≈ ${totalWeight ? Math.round((Number(outcome.weight || 0) * 100) / totalWeight) : 0} %</small><em>${simple ? "Résultat simple" : `⚙ ${outcome.effects.length} effets`}</em><div><button type="button" data-edit-simple-result="${outcomeIndex}">${simple ? "Modifier" : "Voir"}</button><button type="button" class="danger-link" data-delete-simple-result="${outcomeIndex}">Supprimer</button></div></article>`;
      })
      .join("") || '<p class="simple-empty">Aucun résultat configuré.</p>'
  }<button type="button" class="secondary" id="add-simple-result">＋ Ajouter un résultat</button><button type="button" class="secondary" id="zone-advanced-results">Configurer les résultats avancés</button></section></div><div class="actions"><button value="cancel">Annuler</button><button value="save" class="primary">Appliquer</button></div></form>`;
  dialog.showModal();
  const syncSimpleZoneFields = () => {
    const data = new FormData(dialog.querySelector("form"));
    activityElement.querySelector('[data-field="module_activity_name"]').value =
      data.get("name");
    activityElement.querySelector(
      '[data-field="module_activity_profession"]',
    ).value = data.get("profession");
    activityElement.querySelector(
      '[data-field="module_activity_duration"]',
    ).value = data.get("duration");
    activityElement.querySelector(
      '[data-field="module_activity_energy"]',
    ).value = data.get("energy");
    activityElement.querySelector(
      '[data-field="module_activity_level"]',
    ).value = data.get("level");
    activityElement.querySelector('[data-field="module_activity_tool"]').value =
      data.get("tool");
    activityElement.querySelector(
      '[data-field="module_activity_durability"]',
    ).value = data.get("durability");
    markEditorDirty();
  };
  const cancelZoneCreation = (event) => {
    event?.preventDefault();
    dialog.close();
    if (discardOnCancel) {
      activityElement.remove();
      refreshSimpleGameplay();
    }
  };
  dialog
    .querySelectorAll('[value="cancel"]')
    .forEach((button) => (button.onclick = cancelZoneCreation));
  dialog.oncancel = cancelZoneCreation;
  $("#zone-advanced-results").onclick = () => {
    syncSimpleZoneFields();
    dialog.close();
    switchGameplayMode("advanced");
    activityElement.open = true;
    activityElement.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  dialog.querySelectorAll("[data-edit-simple-result]").forEach(
    (button) =>
      (button.onclick = () => {
        syncSimpleZoneFields();
        openSimpleResultEditor(
          index,
          Number(button.dataset.editSimpleResult),
          () => openSimpleZoneEditor(index),
        );
      }),
  );
  dialog.querySelectorAll("[data-delete-simple-result]").forEach(
    (button) =>
      (button.onclick = () => {
        if (!confirm("Supprimer ce résultat de cette zone ?")) return;
        activityElement
          .querySelectorAll(".outcome-modules > .outcome-module")
          [Number(button.dataset.deleteSimpleResult)]?.remove();
        openSimpleZoneEditor(index);
        markEditorDirty();
      }),
  );
  $("#add-simple-result").onclick = () => {
    syncSimpleZoneFields();
    addOutcomeModule(activityElement.querySelector(".outcome-modules"), {
      key: "",
      weight: 1,
      effects: [{ type: "reward", resource: "", amount: [1, 1] }],
    });
    openSimpleResultEditor(
      index,
      activityElement.querySelectorAll(".outcome-modules > .outcome-module")
        .length - 1,
      () => openSimpleZoneEditor(index),
    );
  };
...
function openSimpleResultEditor(activityIndex, outcomeIndex, onClose) {
  const activityElement = $$("#activity-modules > .activity-module")[
      activityIndex
    ],
    outcomeElement = activityElement?.querySelectorAll(
      ".outcome-modules > .outcome-module",
    )[outcomeIndex];
  ...
  const outcome = readActivityModules()[activityIndex].outcomes[outcomeIndex],
    ...
    simple =
      outcome.effects.length === 1 &&
      ["reward", "cost", "stock_reward"].includes(effect.type);
  if (!simple) {
    $("#simple-zone-dialog")?.close();
    switchGameplayMode("advanced");
    activityElement.open = true;
    outcomeElement.open = true;
    activityElement.scrollIntoView({ behavior: "smooth", block: "start" });
    return;
  }
  ...
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "simple-result-dialog";
    document.body.append(dialog);
  }
  const amount = effect.amount ?? 1,
    min = Array.isArray(amount) ? amount[0] : amount,
    max = Array.isArray(amount) ? amount[1] : amount;
  $("#simple-zone-dialog")?.close();
  dialog.innerHTML = `<form method="dialog"><div class="dialog-head"><div><small>MODE SIMPLE · RÉSULTAT</small><h2>${effectResourceSummary(effect)}</h2></div><button value="cancel">×</button></div><div class="simple-zone-form"><label>Type<select name="type"><option value="reward" ${effect.type === "reward" ? "selected" : ""}>Donner au joueur</option><option value="stock_reward" ${effect.type === "stock_reward" ? "selected" : ""}>Ajouter au stock</option><option value="cost" ${effect.type === "cost" ? "selected" : ""}>Retirer au joueur</option></select></label><label>Objet ou ressource<select name="resource">${catalogOptions(
    "item",
    effect.resource || effect.item || "",
  )
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === (effect.resource || effect.item) ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join(
      "",
  ...
  dialog.querySelector("form").onsubmit = (event) => {
    ...
    event.preventDefault();
    const data = new FormData(event.currentTarget),
      effectElement = outcomeElement.querySelector(
        ".outcome-effects > .outcome-effect",
      ),
      typeField = effectElement.querySelector(
        '[data-field="outcome_effect_type"]',
      );
    typeField.value = data.get("type");
    typeField.dispatchEvent(new Event("change"));
    const refreshed = outcomeElement.querySelector(
      ".outcome-effects > .outcome-effect",
    );
    refreshed.querySelector('[data-field="outcome_effect_resource"]').value =
      data.get("resource");
    refreshed.querySelector('[data-field="outcome_effect_min"]').value =
      data.get("minimum");
    refreshed.querySelector('[data-field="outcome_effect_max"]').value =
      data.get("maximum");
    outcomeElement.querySelector('[data-field="module_outcome_weight"]').value =
      data.get("weight");
    dialog.close();
    markEditorDirty();
    onClose?.();
...
function openSimpleResultEditor(activityIndex, outcomeIndex, onClose) {
  ...
  const outcomeElement = activityElement?.querySelectorAll(
    ".outcome-modules > .outcome-module",
  )[outcomeIndex];
  if (!outcomeElement) return;
  const outcome = readActivityModules()[activityIndex].outcomes[outcomeIndex];
  const editableTypes = ["reward", "cost", "stock_reward"];
  const effectIndex = outcome.effects.findIndex((effect) =>
    editableTypes.includes(effect.type),
  );
  if (effectIndex < 0) {
    $("#simple-zone-dialog")?.close();
    switchGameplayMode("advanced");
  ...
  dialog.querySelector("form").onsubmit = (event) => {
    ...
    event.preventDefault();
    const data = new FormData(event.currentTarget),
      effectElement = outcomeElement.querySelectorAll(
        ".outcome-effects > .outcome-effect",
      )[effectIndex];
    const typeField = effectElement.querySelector(
      '[data-field="outcome_effect_type"]',
    );
    typeField.value = data.get("type");
    typeField.dispatchEvent(new Event("change"));
    const refreshed = outcomeElement.querySelectorAll(
      ".outcome-effects > .outcome-effect",
    )[effectIndex];
    refreshed.querySelector('[data-field="outcome_effect_resource"]').value =
      data.get("resource");
...
function openSimpleProfessionEditor(index) {
  const element = $$("#profession-modules > .module-card")[index],
    profession = readProfessionModules()[index];
  if (!element) return;
  let dialog = $("#simple-profession-dialog");
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "simple-profession-dialog";
    document.body.append(dialog);
  }
  const zones = readActivityModules().filter(
    (item) => item.profession === profession.key,
  );
  dialog.innerHTML = `<form method="dialog"><div class="dialog-head"><div><small>MODE SIMPLE · MÉTIER</small><h2>${mechanicIcon(profession.name)} ${escapeHtml(profession.name)}</h2></div><button value="cancel">×</button></div><div class="simple-zone-form"><label>Nom du métier<input name="name" value="${escapeHtml(profession.name)}" required></label><label>Outil principal<select name="tool">${catalogOptions(
    ...
    .map(
      ([key, label]) =>
        `<option value="${escapeHtml(key)}" ${key === profession.required_item ? "selected" : ""}>${escapeHtml(label)}</option>`,
    )
    .join(
      "",
  ...
  dialog.querySelector("form").onsubmit = (event) => {
    ...
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    element.querySelector('[data-field="module_profession_name"]').value =
      data.get("name");
    element.querySelector('[data-field="module_profession_item"]').value =
      data.get("tool");
    element.querySelector('[data-field="module_profession_grant"]').checked =
      data.get("grant") === "on";
    dialog.close();
    refreshSimpleGameplay();
    markEditorDirty();
...
function openSimpleActionEditor(index) {
  const element = $$("#actions > .action-builder")[index];
  if (!element) return;
  let dialog = $("#simple-action-dialog");
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "simple-action-dialog";
    document.body.append(dialog);
  }
  const name = fieldValue("action_name", element),
    emoji = fieldValue("action_emoji", element),
    effects = readEffects(element.querySelector(".action-effects"));
  dialog.innerHTML = `<form method="dialog"><div class="dialog-head"><div><small>MODE SIMPLE · ACTION</small><h2>${escapeHtml(emoji || "⚡")} ${escapeHtml(name)}</h2></div><button value="cancel">×</button></div><div class="simple-zone-form"><label>Nom affiché<input name="name" value="${escapeHtml(name)}" required></label><label>Icône<input name="emoji" value="${escapeHtml(emoji)}"></label><label class="check"><input name="enabled" type="checkbox" ${fieldValue("action_enabled", element) ? "checked" : ""}><span>Disponible pour les joueurs</span></label><section class="simple-results"><div><h3>Résultats</h3><small>${effects.length} effet(s). L’identifiant technique reste inchangé.</small></div>${
    effects
      .slice(0, 5)
      .map(
        (effect) =>
          `<article><span>${escapeHtml(effectResourceSummary(effect))}</span><small>${escapeHtml(effect.type)}</small></article>`,
      )
      .join("") || '<p class="simple-empty">Aucun résultat.</p>'
  }<button type="button" class="secondary" id="simple-action-advanced">Configurer les effets avancés</button></section></div><div class="actions"><button value="cancel">Annuler</button><button value="save" class="primary">Appliquer</button></div></form>`;
  dialog.showModal();
  $("#simple-action-advanced").onclick = () => {
    dialog.close();
    switchGameplayMode("advanced");
    element.open = true;
    element.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  dialog.querySelector("form").onsubmit = (event) => {
    ...
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    element.querySelector('[data-field="action_name"]').value =
      data.get("name");
    element.querySelector('[data-field="action_emoji"]').value =
      data.get("emoji");
    element.querySelector('[data-field="action_enabled"]').checked =
      data.get("enabled") === "on";
    dialog.close();
    refreshSimpleGameplay();
    markEditorDirty();
...
function simpleDialog(id, eyebrow, title, content, onApply) {
  ...
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = id;
    dialog.className = "simple-building-dialog";
    document.body.append(dialog);
  }
  ...
  dialog
    .querySelectorAll("[data-local-cancel]")
    .forEach((button) => (button.onclick = () => dialog.close()));
  dialog.oncancel = () => dialog.close();
  dialog.querySelector("form").onsubmit = (event) => {
    event.preventDefault();
    if (event.submitter?.value !== "save") return;
    onApply(new FormData(event.currentTarget), dialog);
  };
  dialog.showModal();
  return dialog;
}
...
function bindContextualItemCreation(dialog) {
  dialog
    .querySelectorAll(
      'select[name="output"],select[name="item"],select[name="ingredient_item"]',
    )
    .forEach((selectField) => {
      if (selectField.nextElementSibling?.matches("[data-create-context-item]"))
        return;
      const button = document.createElement("button");
      button.type = "button";
      button.className = "secondary";
      button.dataset.createContextItem = "";
      button.textContent = "＋ Créer un objet";
      selectField.after(button);
      button.onclick = () => openContextualItemCreator(selectField);
    });
}
...
function openContextualItemCreator(targetSelect) {
  return simpleDialog(
    ...
    async (data, current) => {
      ...
      const response = await fetch(
        `/api/content/item/${encodeURIComponent(key)}`,
        {
          method: "POST",
          headers,
      ...
      if (!response.ok) {
        alert((await response.json()).detail || "Création impossible.");
        return;
      }
      const saved = await response.json();
      state.catalogs.item.push({ ...saved, payload });
      const option = document.createElement("option");
      option.value = key;
      option.textContent = `${payload.emoji} ${payload.name}`;
...
function openContextualLocationCreator(targetSelect) {
  return simpleDialog(
    ...
    `<div class="simple-zone-form"><label>Nom<input name="name" required></label><label>Type<select name="type"><option value="city">Ville</option><option value="village">Village</option><option value="forest">Forêt</option><option value="wilderness">Zone sauvage</option><option value="place">Lieu</option><option value="gate">Porte</option></select></label><label>Appartient à<select name="parent">${locationOptions(
      ...
      .join(
        "",
      )}</select></label><label>Description<textarea name="description" rows="3"></textarea></label><label>Emoji<input name="emoji" value="📍"></label></div>`,
    async (data, current) => {
      const name = String(data.get("name") || "").trim(),
        key = technicalKey(name, "lieu"),
        payload = {
          name,
          emoji: data.get("emoji") || "📍",
          description: data.get("description") || "",
      ...
      const response = await fetch(
        `/api/content/location/${encodeURIComponent(key)}`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({ payload, expected_version: null }),
      ...
      const saved = await response.json();
      state.catalogs.location.push({ ...saved, payload });
      const option = document.createElement("option");
      option.value = key;
      option.textContent = `${payload.emoji} ${payload.name}`;
      option.selected = true;
...
function openSimpleRecipeEditor(index, created = false) {
  const element = $$("#recipe-modules > .recipe-module")[index];
  if (!element) return;
  const recipe = readRecipeModules()[index],
    original = JSON.parse(element.dataset.original || "{}"),
    advancedCount = [
      original.hooks,
      original.conditions,
      original.outputs,
    ].filter(Boolean).length;
  const ingredientRows = Object.entries(recipe.ingredients || {})
    .map(
      ([key, amount]) =>
        `<div class="simple-ingredient-row"><label>Objet<select name="ingredient_item">${catalogOptions(
          "item",
          key,
        )
          .map(
            ([id, label]) =>
              `<option value="${escapeHtml(id)}" ${id === key ? "selected" : ""}>${escapeHtml(label)}</option>`,
          )
          .join(
            "",
          )}</select></label><label>Quantité<input name="ingredient_amount" type="number" min="1" value="${amount}"></label><button type="button" class="danger-link" data-remove-simple-row>Supprimer</button></div>`,
    )
    .join("");
  const dialog = simpleDialog(
    "simple-recipe-dialog",
    "FONCTIONNEMENT › RECETTES",
    `🍲 ${escapeHtml(recipe.name || "Nouvelle recette")}`,
    `<div class="simple-zone-form"><label>Nom visible<input name="name" value="${escapeHtml(recipe.name || "")}" required></label><label>Produit obtenu<select name="output">${catalogOptions(
      "item",
      recipe.output_item_key || "",
    )
      .map(
        ([id, label]) =>
          `<option value="${escapeHtml(id)}" ${id === recipe.output_item_key ? "selected" : ""}>${escapeHtml(label)}</option>`,
      )
      .join(
        "",
      )}</select></label><div class="form-grid"><label>Quantité produite<input name="output_quantity" type="number" min="1" value="${recipe.output_quantity || 1}"></label><label>Destination<select name="destination"><option value="building_stock" ${recipe.output_destination === "building_stock" ? "selected" : ""}>Stock du bâtiment</option><option value="player" ${recipe.output_destination === "player" ? "selected" : ""}>Inventaire du joueur</option></select></label><label>Durée en secondes<input name="duration" type="number" min="0" value="${recipe.duration_seconds || 0}"></label><label>Niveau requis<input name="level" type="number" min="1" value="${recipe.required_level || 1}"></label><label>Coût énergétique<input name="energy" type="number" min="0" value="${recipe.energy_cost || 0}"></label><label>Métier<select name="profession">${professionOptions(
      recipe.profession || "",
    )
      .map(
        ([id, label]) =>
          `<option value="${escapeHtml(id)}" ${id === recipe.profession ? "selected" : ""}>${escapeHtml(label)}</option>`,
      )
      .join(
        "",
      )}</select></label></div><section class="simple-results"><div><h3>Ingrédients</h3><small>Objets retirés du stock lors de la fabrication.</small></div><div data-simple-ingredients>${ingredientRows}</div><button type="button" class="secondary" data-add-simple-ingredient>＋ Ajouter un ingrédient</button></section>${advancedCount ? `<p class="field-note">⚙ ${advancedCount} configuration(s) avancée(s) seront conservées.</p>` : ""}<button type="button" class="danger-link" data-delete-simple-recipe>Supprimer la recette</button></div>`,
    (data, current) => {
      element.querySelector('[data-field="recipe_name"]').value =
        data.get("name");
      element.querySelector('[data-field="recipe_output"]').value =
        data.get("output");
      element.querySelector('[data-field="recipe_output_quantity"]').value =
        data.get("output_quantity");
      element.querySelector('[data-field="recipe_destination"]').value =
        data.get("destination");
      element.querySelector('[data-field="recipe_duration"]').value =
        data.get("duration");
      element.querySelector('[data-field="recipe_level"]').value =
        data.get("level");
      element.querySelector('[data-field="recipe_energy"]').value =
        data.get("energy");
      element.querySelector('[data-field="recipe_profession"]').value =
        data.get("profession");
      const root = element.querySelector(".recipe-ingredients");
      root.innerHTML = "";
      const items = data.getAll("ingredient_item"),
        amounts = data.getAll("ingredient_amount");
      items.forEach((key, i) => {
        if (key) addRecipeIngredient(root, key, amounts[i] || 1);
      });
      current.close();
      markEditorDirty();
      refreshSimpleGameplay();
  ...
  const addRow = () => {
    dialog.querySelector("[data-simple-ingredients]").insertAdjacentHTML(
      "beforeend",
      `<div class="simple-ingredient-row"><label>Objet<select name="ingredient_item">${catalogOptions(
        "item",
        "",
        ...
        .join(
          "",
        )}</select></label><label>Quantité<input name="ingredient_amount" type="number" min="1" value="1"></label><button type="button" class="danger-link" data-remove-simple-row>Supprimer</button></div>`,
    );
    bindRows();
    bindContextualItemCreation(dialog);
  };
  const bindRows = () =>
    dialog
      .querySelectorAll("[data-remove-simple-row]")
      .forEach(
        (button) =>
          (button.onclick = () =>
            button.closest(".simple-ingredient-row").remove()),
      );
  bindRows();
  bindContextualItemCreation(dialog);
  dialog.querySelector("[data-add-simple-ingredient]").onclick = addRow;
  dialog.querySelector("[data-delete-simple-recipe]").onclick = () => {
    if (confirm("Supprimer cette recette du brouillon ?")) {
      element.remove();
      dialog.close();
      markEditorDirty();
      refreshSimpleGameplay();
...
function openSimpleDeliveryEditor(index, created = false) {
  const element = $$("#delivery-modules > .delivery-module")[index];
  if (!element) return;
  const delivery = readDeliveryModules()[index],
    original = JSON.parse(element.dataset.original || "{}"),
    advancedCount =
      (original.conditions ? 1 : 0) +
      (original.events ? 1 : 0) +
      (Number(original.unit_price || 0) > 0 ? 1 : 0);
  const dialog = simpleDialog(
    "simple-delivery-dialog",
    "FONCTIONNEMENT › LIVRAISONS",
    "📦 Livraison",
    `<div class="simple-zone-form"><label>Objet transféré<select name="item">${catalogOptions(
      "item",
      delivery.item_key || "",
    )
      .map(
        ([id, label]) =>
          `<option value="${escapeHtml(id)}" ${id === delivery.item_key ? "selected" : ""}>${escapeHtml(label)}</option>`,
      )
      .join(
        "",
      )}</select></label><div class="delivery-flow-preview"><span>Inventaire joueur</span><b>→ ${escapeHtml(itemDisplay(delivery.item_key).name)} →</b><span>Stock du bâtiment</span></div><div class="form-grid"><label>Depuis<select name="source"><option value="player_inventory">Inventaire du joueur</option></select></label><label>Vers quel bâtiment<select name="building">${catalogOptions(
      "building",
      delivery.target_building_key || "",
    )
      .map(
        ([id, label]) =>
          `<option value="${escapeHtml(id)}" ${id === delivery.target_building_key ? "selected" : ""}>${escapeHtml(label)}</option>`,
      )
      .join(
        "",
      )}</select></label><label>Stock destinataire<select name="destination"><option value="building_stock">Stock du bâtiment</option><option value="player_inventory" ${delivery.destination === "player_inventory" ? "selected" : ""}>Inventaire joueur</option></select></label><label>Quantité minimum<input name="minimum" type="number" min="1" value="${delivery.minimum_quantity || 1}"></label><label>Quantité maximum<input name="maximum" type="number" min="0" value="${delivery.maximum_quantity || 0}"></label></div>${advancedCount ? `<p class="field-note">⚙ ${advancedCount} réglage(s) avancé(s) seront conservés.</p>` : ""}<button type="button" class="danger-link" data-delete-simple-delivery>Supprimer la livraison</button></div>`,
    (data, current) => {
      element.querySelector('[data-field="delivery_resource"]').value =
        data.get("item");
      element.querySelector('[data-field="delivery_source"]').value =
        data.get("source");
      element.querySelector('[data-field="delivery_destination"]').value =
        data.get("destination");
      element.querySelector('[data-field="delivery_building"]').value =
        data.get("building");
      element.querySelector('[data-field="delivery_min"]').value =
        data.get("minimum");
      element.querySelector('[data-field="delivery_max"]').value =
        data.get("maximum");
      current.close();
      markEditorDirty();
      refreshSimpleGameplay();
  ...
  bindContextualItemCreation(dialog);
  dialog.querySelector("[data-delete-simple-delivery]").onclick = () => {
    if (confirm("Supprimer cette livraison du brouillon ?")) {
      element.remove();
      dialog.close();
      markEditorDirty();
      refreshSimpleGameplay();
...
function openSimpleProductEditor(index, created = false) {
  const element = $$("#product-modules > .product-module")[index];
  if (!element) return;
  const product = readProductModules()[index];
  const dialog = simpleDialog(
    "simple-product-dialog",
    "FONCTIONNEMENT › PRODUCTIONS",
    "🛒 Produit",
    `<div class="simple-zone-form"><label>Objet<select name="item">${catalogOptions(
      "item",
      product.item_key || "",
    )
      .map(
        ([id, label]) =>
          `<option value="${escapeHtml(id)}" ${id === product.item_key ? "selected" : ""}>${escapeHtml(label)}</option>`,
      )
      .join(
        "",
      )}</select></label><label>Nom visible<input name="name" value="${escapeHtml(product.name || "")}"></label><div class="form-grid"><label>Prix<input name="price" type="number" min="0" value="${product.price || 0}"></label><label>Stock initial<input name="stock" type="number" min="0" value="${product.initial_stock || 0}"></label><label>Maximum par achat<input name="maximum" type="number" min="1" value="${product.maximum_per_purchase || 1}"></label></div><button type="button" class="danger-link" data-delete-simple-product>Supprimer le produit</button></div>`,
    (data, current) => {
      element.querySelector('[data-field="product_item"]').value =
        data.get("item");
      element.querySelector('[data-field="product_name"]').value =
        data.get("name");
      element.querySelector('[data-field="product_price"]').value =
        data.get("price");
      element.querySelector('[data-field="product_stock"]').value =
        data.get("stock");
      element.querySelector('[data-field="product_max"]').value =
        data.get("maximum");
      current.close();
      markEditorDirty();
      refreshSimpleGameplay();
  ...
  bindContextualItemCreation(dialog);
  dialog.querySelector("[data-delete-simple-product]").onclick = () => {
    element.remove();
    dialog.close();
    markEditorDirty();
    refreshSimpleGameplay();
...
function switchGameplayMode(mode) {
  const simple = $("#simple-gameplay-root"),
    advanced = $("#advanced-gameplay-root");
  if (!simple || !advanced) return;
  if (mode === "simple") refreshSimpleGameplay();
  simple.hidden = mode !== "simple";
  advanced.hidden = mode !== "advanced";
  $$("[data-gameplay-toggle]").forEach((button) =>
    button.classList.toggle("active", button.dataset.gameplayToggle === mode),
  );
}
function refreshSimpleGameplay() {
  const root = $("#simple-gameplay-root");
  if (!root) return;
  root.innerHTML = gameplayProjection();
  bindSimpleGameplay();
}
function bindSimpleGameplay() {
  const actionList = $(".simple-action-list");
  if (actionList) {
    const addButton = document.createElement("button");
    addButton.type = "button";
    addButton.id = "add-simple-action";
    addButton.className = "secondary";
    addButton.textContent = "＋ Ajouter une action personnalisée";
    actionList.before(addButton);
    addButton.onclick = () => {
      addAction({
      ...
      refreshSimpleGameplay();
      const index = $$("#actions > .action-builder").length - 1,
        element = $$("#actions > .action-builder")[index];
      openSimpleActionEditor(index);
      const dialog = $("#simple-action-dialog");
      dialog?.querySelectorAll('[value="cancel"]').forEach(
        (button) =>
          (button.onclick = () => {
            element?.remove();
            dialog.close();
  ...
  $$("[data-simple-zone]").forEach(
    (button) =>
      (button.onclick = () =>
        openSimpleZoneEditor(Number(button.dataset.simpleZone))),
  );
  $$("[data-simple-profession]").forEach(
    (button) =>
      (button.onclick = () =>
        openSimpleProfessionEditor(Number(button.dataset.simpleProfession))),
  );
  $$("[data-simple-action]").forEach(
    (button) =>
      (button.onclick = () => {
        switchGameplayMode("advanced");
        const card = $$("#actions > .action-builder")[
          Number(button.dataset.simpleAction)
        ];
        if (card) {
          card.open = true;
  ...
  $$("[data-open-advanced-section]").forEach(
    (button) =>
      (button.onclick = () => {
        switchGameplayMode("advanced");
        $("#" + button.dataset.openAdvancedSection)?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }),
  );
  $$("[data-add-simple-zone]").forEach(
    (button) =>
      (button.onclick = () => {
        addActivityModule({
          profession: button.dataset.addSimpleZone,
          outcomes: [],
        });
        refreshSimpleGameplay();
        openSimpleZoneEditor(
          $$("#activity-modules > .module-card").length - 1,
          true,
        );
      }),
  );
  $$("[data-gameplay-mode]").forEach(
    (button) =>
      (button.onclick = () => switchGameplayMode(button.dataset.gameplayMode)),
  );
  $("#add-simple-mechanic")?.addEventListener("click", () => {
    addProfessionModule({ name: "Nouveau métier" });
    refreshActivityProfessionOptions();
    refreshSimpleGameplay();
    const index = $$("#profession-modules > .module-card").length - 1,
      element = $$("#profession-modules > .module-card")[index];
    openSimpleProfessionEditor(index);
    const dialog = $("#simple-profession-dialog");
    dialog?.querySelectorAll('[value="cancel"]').forEach(
      (button) =>
        (button.onclick = () => {
          element?.remove();
  ...
  const flowButtons = $$(".simple-exchange-card button"),
    deliveries = readDeliveryModules(),
    products = readProductModules(),
    recipes = readRecipeModules();
  flowButtons.forEach(
    (button, index) =>
      (button.onclick = () => {
        if (index < deliveries.length) return openSimpleDeliveryEditor(index);
        if (index < deliveries.length + products.length)
          return openSimpleProductEditor(index - deliveries.length);
        return openSimpleRecipeEditor(
          index - deliveries.length - products.length,
        );
      }),
  );
  const flowGrid = $(".simple-exchange-grid");
  if (flowGrid)
    flowGrid.insertAdjacentHTML(
      "afterend",
      '<div class="simple-flow-actions"><button type="button" class="secondary" id="add-simple-recipe">＋ Ajouter une recette</button><button type="button" class="secondary" id="add-simple-delivery">＋ Ajouter une livraison</button><button type="button" class="secondary" id="add-simple-product">＋ Ajouter un produit</button></div>',
    );
  if (flowGrid && deliveries.length) {
    const groups = new Map();
    deliveries.forEach((delivery, index) => {
      const key = delivery.target_building_key || delivery.building || "";
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push({ delivery, index });
    });
    const grouped = document.createElement("div");
    grouped.className = "grouped-deliveries";
    grouped.innerHTML = [...groups]
      .map(([key, rows]) => {
        const building = state.catalogs.building.find(
          (item) => item.entity_key === key,
        );
        return `<article><header><div><small>DESTINATION</small><h4>${escapeHtml(building?.payload.name || key || "Bâtiment actuel")}</h4><p>${rows.length} ressource(s)</p></div><button type="button" data-add-delivery-target="${escapeHtml(key)}">＋ Ajouter une ressource</button></header>${rows
          .map(({ delivery, index }) => {
            const item = itemDisplay(delivery.item_key || delivery.resource);
            return `<button type="button" data-edit-grouped-delivery="${index}"><span>${item.emoji}</span><b>${escapeHtml(item.name)}</b><small>${delivery.minimum_quantity || 1}–${delivery.maximum_quantity || "∞"}</small><i>Configurer ›</i></button>`;
          })
          .join("")}</article>`;
      })
      .join("");
    flowGrid.before(grouped);
    [...flowGrid.children]
      .slice(0, deliveries.length)
      .forEach((card) => (card.hidden = true));
    $$("[data-edit-grouped-delivery]").forEach(
      (button) =>
        (button.onclick = () =>
          openSimpleDeliveryEditor(Number(button.dataset.editGroupedDelivery))),
    );
    $$("[data-add-delivery-target]").forEach(
      (button) =>
        (button.onclick = () => {
          addDeliveryModule({
            source: "player_inventory",
            destination: "building_stock",
            target_building_key: button.dataset.addDeliveryTarget,
            minimum_quantity: 1,
            maximum_quantity: 10,
          });
          refreshSimpleGameplay();
          openSimpleDeliveryEditor(
            $$("#delivery-modules > .delivery-module").length - 1,
            true,
          );
        }),
    );
  }
  $("#add-simple-recipe")?.addEventListener("click", () => {
    addRecipeModule({
      active: true,
      ingredients: {},
      duration_seconds: 30,
    ...
    refreshSimpleGameplay();
    openSimpleRecipeEditor(
      $$("#recipe-modules > .recipe-module").length - 1,
      true,
    );
  });
  $("#add-simple-delivery")?.addEventListener("click", () => {
    addDeliveryModule({
      source: "player_inventory",
      destination: "building_stock",
      minimum_quantity: 1,
  ...
  $("#add-simple-product")?.addEventListener("click", () => {
    addProductModule({
      active: true,
      initial_stock: 0,
      maximum_per_purchase: 1,
    });
    refreshSimpleGameplay();
    openSimpleProductEditor(
      $$("#product-modules > .product-module").length - 1,
      true,
    );
  });
  $$("[data-simple-action]").forEach(
    (button) =>
      (button.onclick = () =>
        openSimpleActionEditor(Number(button.dataset.simpleAction))),
  );
}

function setupSimpleGameplay(root) {
  const panel = root.querySelector('[data-building-panel="mechanics"]'),
    children = [...panel.children],
    toolbar = document.createElement("div"),
    simple = document.createElement("div"),
    advanced = document.createElement("div");
  toolbar.className = "gameplay-mode-switch";
  toolbar.innerHTML =
    '<div><small>ÉDITION DU GAMEPLAY</small><b>Choisissez votre niveau de détail</b></div><span><button type="button" class="active" data-gameplay-toggle="simple">Mode simple</button><button type="button" data-gameplay-toggle="advanced">Mode avancé</button></span>';
  simple.id = "simple-gameplay-root";
  advanced.id = "advanced-gameplay-root";
  children.forEach((child) => advanced.append(child));
  panel.append(toolbar, simple, advanced);
  refreshSimpleGameplay();
  advanced.hidden = true;
  $$("[data-gameplay-toggle]").forEach(
    (button) =>
      (button.onclick = () =>
        switchGameplayMode(button.dataset.gameplayToggle)),
  );
}

function bindEntitySearches(root = document) {
  root.querySelectorAll("[data-entity-search]").forEach(
    (search) =>
      (search.oninput = () => {
        const selectField = search.nextElementSibling;
        if (!(selectField instanceof HTMLSelectElement)) return;
        const query = search.value.trim().toLowerCase();
        [...selectField.options].forEach(
          (option, index) =>
            (option.hidden =
              index > 0 &&
              query &&
...
function itemMenuPicker(component) {
  const items = state.catalogs.item || [],
    selected = new Set(
      (component.options || []).map(
        (option) =>
          option.interaction?.item_key || option.item_key || option.key,
      ),
    );
  const categories = [
    ...new Set(items.map((item) => String(item.payload.category || "Autres"))),
  ].sort((a, b) => a.localeCompare(b, "fr"));
  return `<section class="item-menu-picker"><div class="section-head"><div><b>Ajouter depuis les objets</b><small>Recherche et sélection multiple</small></div><button type="button" class="primary add-catalog-items">Ajouter la sélection</button></div><div class="item-menu-tools"><input type="search" data-item-menu-search placeholder="Rechercher une bière, un repas, un outil…"><select data-item-menu-category><option value="">Toutes les catégories</option>${categories.map((category) => `<option value="${escapeHtml(category)}">${escapeHtml(category)}</option>`).join("")}</select><select data-item-menu-sort><option value="name">Nom A–Z</option><option value="category">Catégorie</option><option value="price">Prix croissant</option></select></div><div class="item-menu-results">${
    items
      .map((item) => {
        const p = item.payload,
          category = String(p.category || "Autres"),
          name = String(p.name || item.entity_key),
          search = `${name} ${item.entity_key} ${category}`.toLowerCase();
        return `<label class="item-menu-result" data-name="${escapeHtml(name.toLowerCase())}" data-search="${escapeHtml(search)}" data-category="${escapeHtml(category)}" data-price="${Number(p.price || 0)}"><input type="checkbox" value="${escapeHtml(item.entity_key)}" ${selected.has(item.entity_key) ? "disabled" : ""}><span>${escapeHtml(p.emoji || "📦")}</span><b>${escapeHtml(name)}</b><small>${escapeHtml(category)} · ${Number(p.price || 0)} écus</small></label>`;
      })
      .join("") || '<p class="field-note">Aucun objet disponible.</p>'
  }</div><p class="field-note">Discord limite un menu à 25 options. Un objet déjà ajouté est grisé. Pour un achat, configure aussi son prix et son stock dans la section Produits.</p></section>`;
}
...
function bindPropertyPanel() {
      ...
      (field.onchange = () => {
        ...
        if (component && optionMatch) {
          const option = component.options[Number(optionMatch[1])],
            property = optionMatch[2];
          option.interaction ||= {
            type: "navigate",
            page: state.interfaceDraft.start_page,
          };
          if (property === "interaction_type")
            option.interaction =
              value === "navigate"
                ? { type: "navigate", page: state.interfaceDraft.start_page }
                : value === "purchase"
                  ? { type: "purchase", item_key: option.item_key || "" }
                  : {
                      type: "action",
                      building:
                        state.type === "building"
          ...
          else if (property === "target_page")
            option.interaction = { type: "navigate", page: value };
          else if (property === "target_building")
            option.interaction = {
              type: "action",
              building: value,
              action: "",
            };
          else if (property === "target_action")
            option.interaction.action = value;
          else if (property === "purchase_item") {
            option.item_key = value;
            option.interaction = { type: "purchase", item_key: value };
          } else if (property === "cooldown_seconds")
            option.interaction.cooldown_seconds = Number(value) || 0;
          else if (property === "access_type") {
            if (value === "none") delete option.access_when;
            else
              option.access_when = {
                profession_level: {
                  profession:
                    state.buildingBase?.modules?.professions?.[0]?.key ||
                    "profession",
                  minimum: 1,
                },
              };
          } else if (property === "access_profession") {
            option.access_when ||= { profession_level: {} };
            option.access_when.profession_level.profession = value;
          } else if (property === "access_minimum") {
            option.access_when ||= { profession_level: {} };
            option.access_when.profession_level.minimum = Number(value) || 1;
          } else option[property] = value;
          renderVisualStudio();
          return;
        }
        const sequenceMatch = key.match(/^sequence_(\d+)_(.+)$/);
        if (component && sequenceMatch) {
          const step = component.props.steps[Number(sequenceMatch[1])],
            property = sequenceMatch[2];
          if (property === "condition") {
            if (value === "none") delete step.visible_when;
            else {
              const firstProfession =
                state.buildingBase?.modules?.professions?.[0]?.key ||
                "profession";
              step.visible_when = {
                profession_level: { profession: firstProfession, minimum: 1 },
              };
            }
          } else if (property === "profession") {
            step.visible_when ||= { profession_level: {} };
            step.visible_when.profession_level.profession = value;
          } else if (property === "minimum") {
            step.visible_when ||= { profession_level: {} };
            step.visible_when.profession_level.minimum = Number(value) || 1;
          } else if (property === "delay_seconds")
            step.delay_seconds = Math.max(0, Number(value) || 0);
          else step[property] = value;
          renderVisualStudio();
          return;
        }
        if (key === "interface_building")
          state.interfaceDraft.target_building_key = value;
        else if (key === "theme_color") {
          state.interfaceDraft.theme ||= {};
          state.interfaceDraft.theme.color = value;
        } else if (key === "theme_density") {
          state.interfaceDraft.theme ||= {};
          state.interfaceDraft.theme.density = value;
        } else if (key === "page_name") page.name = value;
        else if (key === "page_key") {
          const old = page.key,
            newKey = technicalKey(value, "page");
          page.key = newKey;
          if (state.interfaceDraft.start_page === old)
            state.interfaceDraft.start_page = newKey;
          state.interfaceDraft.pages.forEach((item) =>
            item.components.forEach((child) => {
              if (
                child.interaction?.type === "navigate" &&
                child.interaction.page === old
              )
                child.interaction.page = newKey;
              (child.options || []).forEach((option) => {
                if (
                  option.interaction?.type === "navigate" &&
                  option.interaction.page === old
                )
                  option.interaction.page = newKey;
              });
            }),
          );
          state.selectedPage = newKey;
        } else if (component && key === "interaction_type") {
          component.interaction =
            value === "navigate"
              ? { type: "navigate", page: state.interfaceDraft.start_page }
              : value === "action"
                ...
                : { type: value };
        } else if (component && key === "target_page")
          component.interaction = { type: "navigate", page: value };
        else if (component && key === "target_building")
          component.interaction = {
            type: "action",
            building: value,
            action: "",
          };
        else if (component && key === "target_action")
          component.interaction.action = value;
        else if (component && key === "world_destination")
          component.interaction.destination = value;
        else if (
          component &&
          ["cooldown_seconds", "global_cooldown_seconds"].includes(key)
        )
          component.interaction[key] = Math.max(0, Number(value) || 0);
        else if (component && key === "inline")
          component.props.inline = value === "true";
        else if (component && key === "access_type") {
          if (value === "none") delete component.access_when;
          else
            component.access_when = {
              profession_level: {
                profession:
                  state.buildingBase?.modules?.professions?.[0]?.key ||
                  "profession",
        ...
        } else if (component && key === "access_profession") {
          component.access_when ||= { profession_level: {} };
          component.access_when.profession_level.profession = value;
        } else if (component && key === "access_minimum") {
          component.access_when ||= { profession_level: {} };
          component.access_when.profession_level.minimum = Number(value) || 1;
        } else if (component) component.props[key] = value;
        renderVisualStudio();
      }),
  );
  const start = panel.querySelector('[data-field="page_start"]');
  if (start)
    start.onchange = () => {
      if (start.checked) state.interfaceDraft.start_page = page.key;
      renderVisualStudio();
    };
  panel.querySelector(".delete-component")?.addEventListener("click", () => {
    page.components = page.components.filter(
      (item) => item.id !== state.selectedComponent,
    );
    state.selectedComponent = null;
    renderVisualStudio();
  });
  panel.querySelector(".add-select-option")?.addEventListener("click", () => {
    if (component.options.length >= 25) return;
    const index = component.options.length + 1;
    component.options.push({
      key: `option_${index}`,
      label: `Option ${index}`,
      emoji: "",
      description: "",
      interaction: { type: "navigate", page: state.interfaceDraft.start_page },
    });
    renderVisualStudio();
  });
  const itemResults = panel.querySelector(".item-menu-results"),
    itemSearch = panel.querySelector("[data-item-menu-search]"),
    itemCategory = panel.querySelector("[data-item-menu-category]"),
    itemSort = panel.querySelector("[data-item-menu-sort]");
  const refreshItemPicker = () => {
    if (!itemResults) return;
    const query = (itemSearch?.value || "").trim().toLowerCase(),
      category = itemCategory?.value || "",
      rows = [...itemResults.querySelectorAll(".item-menu-result")];
    rows.sort((a, b) =>
      itemSort?.value === "price"
        ? Number(a.dataset.price) - Number(b.dataset.price)
        : itemSort?.value === "category"
          ? `${a.dataset.category} ${a.dataset.name}`.localeCompare(
              `${b.dataset.category} ${b.dataset.name}`,
              "fr",
            )
  ...
  if (itemSearch) itemSearch.oninput = refreshItemPicker;
  if (itemCategory) itemCategory.onchange = refreshItemPicker;
  if (itemSort) itemSort.onchange = refreshItemPicker;
  panel.querySelector(".add-catalog-items")?.addEventListener("click", () => {
    const keys = [
        ...panel.querySelectorAll(".item-menu-result input:checked"),
      ].map((input) => input.value),
      remaining = Math.max(0, 25 - component.options.length);
    if (!keys.length) {
      alert("Sélectionnez au moins un objet.");
      return;
    }
    if (keys.length > remaining) {
      alert(`Il reste ${remaining} place(s) dans ce menu Discord.`);
      return;
    }
    for (const key of keys) {
      const entity = state.catalogs.item.find(
        (item) => item.entity_key === key,
      );
      if (!entity) continue;
      const p = entity.payload;
      component.options.push({
        ...
        label: p.name || key,
        emoji: p.emoji || "📦",
        description: `${p.category || "Objet"}${p.price != null ? ` · ${p.price} écus` : ""}`,
        interaction: { type: "purchase", item_key: key },
      });
    }
    renderVisualStudio();
  });
  panel.querySelectorAll("[data-remove-option]").forEach(
    (button) =>
      (button.onclick = () => {
        component.options.splice(Number(button.dataset.removeOption), 1);
        renderVisualStudio();
      }),
  );
  panel.querySelector(".add-sequence-step")?.addEventListener("click", () => {
    component.props.steps ||= [];
    component.props.steps.push({ text: "Nouveau texte", delay_seconds: 1 });
    renderVisualStudio();
  });
  panel.querySelectorAll("[data-remove-step]").forEach(
    (button) =>
      (button.onclick = () => {
        component.props.steps.splice(Number(button.dataset.removeStep), 1);
        renderVisualStudio();
      }),
  );
  panel.querySelector(".remove-page")?.addEventListener("click", () => {
    const fallback = state.interfaceDraft.pages.find(
      (item) => item.key !== page.key,
    );
    state.interfaceDraft.pages = state.interfaceDraft.pages.filter(
      (item) => item.key !== page.key,
    );
    state.interfaceDraft.pages.forEach((item) =>
      item.components.forEach((child) => {
        if (
          child.interaction?.type === "navigate" &&
          child.interaction.page === page.key
        )
          child.interaction.page = fallback.key;
        (child.options || []).forEach((option) => {
          if (
            option.interaction?.type === "navigate" &&
            option.interaction.page === page.key
          )
            option.interaction.page = fallback.key;
        });
      }),
    );
    if (state.interfaceDraft.start_page === page.key)
      state.interfaceDraft.start_page = fallback.key;
    state.selectedPage = fallback.key;
    state.selectedComponent = null;
    renderVisualStudio();
  });
}
...
function soundTrackChecklist(type, selected = []) {
  const entries = state.catalogs.audio.filter(
    (entity) =>
      (entity.payload.audio_type || entity.payload.channel || "sfx") === type,
  );
...
function addSoundGroup(group = {}) {
  ...
  element.className = "module-card sound-group";
  element.open = !group.key;
  element.dataset.original = JSON.stringify(group);
  ...
  $("#sound-groups").append(element);
  element.querySelector(".remove").onclick = () => {
    element.remove();
    refreshSoundGroupSelectors();
  };
  element.querySelector('[data-field="sound_group_name"]').oninput = (e) => {
    if (
      !element.querySelector('[data-field="sound_group_key"]').dataset.touched
    )
      element.querySelector('[data-field="sound_group_key"]').value =
        technicalKey(e.target.value, "ambiance");
    element.querySelector("summary strong").textContent =
      `🎼 ${e.target.value || "Nouveau groupe"}`;
    refreshSoundGroupSelectors();
  };
  element.querySelector('[data-field="sound_group_key"]').oninput = (e) => {
    e.target.dataset.touched = "true";
    refreshSoundGroupSelectors();
  };
}

function readSoundGroups() {
  return $$("#sound-groups > .sound-group").map((element, index) => {
    const tracks = {};
    element
      .querySelectorAll("[data-sound-type]")
      .forEach(
        (lane) =>
          (tracks[lane.dataset.soundType] = [
            ...lane.querySelectorAll("input:checked"),
          ].map((input) => input.value)),
      );
    return {
      key: fieldValue("sound_group_key", element) || `ambiance_${index + 1}`,
      name: fieldValue("sound_group_name", element) || `Ambiance ${index + 1}`,
      volume: fieldValue("sound_group_volume", element) ?? 1,
      tracks,
    };
  });
}
function currentSoundGroupOptions(value = "") {
  const groups = readSoundGroups(),
    global = (state.catalogs.audio_group || []).map((entity) => ({
    ...
    all = [
      ...groups,
      ...global.filter(
        (item) => !groups.some((group) => group.key === item.key),
      ),
    ];
  return [
    ["", "Choisir un groupe…"],
    ...all.map((group) => [group.key, group.name]),
    ...(value && !all.some((group) => group.key === value)
      ? [[value, `⚠ ${value}`]]
      : []),
  ];
}
function addEventAudioLayer(root, layer = {}) {
  const row = document.createElement("article");
  row.className = "event-audio-layer";
  row.innerHTML = `<button type="button" class="remove">×</button>${select("Groupe d’ambiance", "event_audio_group", layer.group_key || "", [["", "Choisir un groupe…"], ...(state.catalogs.audio_group || []).map((entity) => [entity.entity_key, `🌲 ${entity.payload.name}`])])}<div><b>Bâtiments concernés</b><div class="audio-building-picker">${state.catalogs.building.map((building) => `<label class="check"><input type="checkbox" data-event-audio-building="${escapeHtml(building.entity_key)}" ${(layer.building_keys || []).includes(building.entity_key) ? "checked" : ""}><span>${escapeHtml(building.payload.emoji || "🏰")} ${escapeHtml(building.payload.name)}</span></label>`).join("")}</div><small class="field-note">Aucun bâtiment coché = tout le royaume.</small></div>`;
  root.append(row);
  row.querySelector(".remove").onclick = () => row.remove();
}
function readEventAudioLayers() {
  return $$("#event-audio-layers > .event-audio-layer")
    .map((row) => ({
      group_key: fieldValue("event_audio_group", row),
      building_keys: [
        ...row.querySelectorAll("[data-event-audio-building]:checked"),
      ].map((input) => input.dataset.eventAudioBuilding),
    }))
    .filter((layer) => layer.group_key);
}
function refreshSoundGroupSelectors() {
  $$("[data-sound-group-selector]").forEach((field) => {
    const value = field.value;
    field.innerHTML = currentSoundGroupOptions(value)
      .map(
        ([key, label]) =>
...
function addSoundRoute(route = {}) {
  const row = document.createElement("div");
  row.className = "builder sound-route";
  row.innerHTML = `<button type="button" class="remove">×</button><div class="form-grid">${select("Événement", "sound_route_event", route.event || "", catalogOptions("event", route.event || ""))}${select("Groupe à activer", "sound_route_group", route.group_key || "", currentSoundGroupOptions(route.group_key || "")).replace('data-field="sound_route_group"', 'data-field="sound_route_group" data-sound-group-selector')}</div>`;
  $("#sound-routes").append(row);
  row.querySelector(".remove").onclick = () => row.remove();
}
function readSoundRoutes() {
  return $$("#sound-routes > .sound-route")
    .map((row) => ({
      event: fieldValue("sound_route_event", row),
      group_key: fieldValue("sound_route_group", row),
    }))
    .filter((route) => route.event && route.group_key);
}
...
function renderBuildingFields(payload, preset = null) {
  ...
  const presetInfo =
    preset ||
    KingdomBuildingPresets.find((item) => item.key === payload.building_kind);
  ...
  const moduleCount = [
    ...
    (total, key) =>
      total + (Array.isArray(modules[key]) ? modules[key].length : 0),
    0,
  );
  const buildingKey =
    $("#key").value ||
    technicalKey($("#name").value || payload.name || "batiment", "batiment");
  const linked = payload.interface_key
    ? state.catalogs.interface.find(
        (item) => item.entity_key === payload.interface_key,
      )?.payload
    : null;
  initializeInterfaceDraft(
    payload.interface ||
      linked ||
      blankInterface(
        buildingKey,
        payload.name || "Nouveau bâtiment",
        payload.emoji || "🏰",
        payload.color || "7a1f1f",
      ),
    buildingKey,
  );
  const access = payload.access || {};
  $(".wizard-panel").classList.add("visual-mode", "building-mode");
  $("#context-help").hidden = false;
  setHelp("building_mechanics");
  ...
  root
    ...
    .insertAdjacentHTML("beforeend", derivedBuildingRelationsMarkup(modules));
  root
    .querySelector('[data-building-panel="overview"]')
    .insertAdjacentHTML(
      "afterbegin",
  ...
  $("#create-building-location").onclick = () =>
    openContextualLocationCreator($('[data-field="building_location"]'));
  const sound = modules.audio || {},
    assignedBots = state.catalogs.bot.filter(
      (entity) =>
        entity.payload.bot_type === "voice" &&
        entity.payload.building_key === buildingKey,
    );
  root.insertAdjacentHTML(
    "beforeend",
  ...
  root.insertAdjacentHTML(
    "beforeend",
    '<div data-building-panel="advanced" class="building-advanced-panel" hidden><div class="empty-admin">Les réglages techniques et Discord avancés sont regroupés ici.</div></div>',
  );
  (sound.groups || []).forEach(addSoundGroup);
  (sound.event_routes || []).forEach(addSoundRoute);
  refreshSoundGroupSelectors();
  $("#add-sound-group").onclick = () => {
    addSoundGroup({ tracks: { music: [], ambience: [], sfx: [], voice: [] } });
    refreshSoundGroupSelectors();
  };
  $("#add-sound-route").onclick = () => addSoundRoute({});
  root
  ...
  root
    .querySelectorAll('[data-building-panel="mechanics"] > details.advanced')
    .forEach((details) => advancedPanel.append(details));
  advancedPanel.insertAdjacentHTML(
    "afterbegin",
    `<section class="form-section"><div class="section-copy"><span class="step-dot">◇</span><div><h3>Nature de l'entité interactive</h3><p>Le contrat historique s'appelle « bâtiment », mais peut représenter une planète, un véhicule, une institution, une salle ou une zone abstraite.</p></div></div><div class="form-grid">${select(
      "Nature",
      "entity_kind",
      payload.entity_kind || "place",
      [
        ["place", "Lieu"],
        ["room", "Salle"],
        ["institution", "Institution"],
        ["vehicle", "Véhicule / navire"],
        ["planet", "Planète"],
        ["zone", "Zone"],
        ["abstract", "Espace abstrait"],
        ["custom", "Personnalisé"],
      ],
    )}${input("Catégorie libre", "entity_category", payload.category || "")}${input("Tags (séparés par des virgules)", "entity_tags", (payload.tags || []).join(", "))}${select("Entité parente", "entity_parent", payload.parent_key || "", catalogOptions("building", payload.parent_key || ""))}</div></section>`,
  );
  ...
  $("#create-related-profession").onclick = () => {
    addProfessionModule({ name: "Nouveau métier" });
    refreshActivityProfessionOptions();
    refreshSimpleGameplay();
    $('[data-building-tab="mechanics"]').click();
    const cards = $$("#profession-modules > .module-card");
    cards.at(-1)?.scrollIntoView({ behavior: "smooth", block: "center" });
  };
  $('[data-field="relation_primary_profession"]').onchange = (event) => {
    const selected = professionCatalog().find(
      (item) => item.key === event.target.value,
    );
    if (
      !selected ||
      readProfessionModules().some((item) => item.key === selected.key)
    )
      return;
    addProfessionModule(clone(selected.definition));
    refreshActivityProfessionOptions();
  };
  $$("[data-open-related]").forEach(
    (button) =>
      (button.onclick = () => {
        closeEditor();
        navigateTo(button.dataset.openRelated);
      }),
  );
  $$("[data-open-derived]").forEach(
    (button) =>
      (button.onclick = () => {
        closeEditor();
        navigateTo(button.dataset.openDerived);
      }),
  );
  bindEntitySearches(root);
  $("#dissociate-building-bot").onclick = () => {
    if (!confirm("Dissocier ce bot du bâtiment sans supprimer le bot ?"))
      return;
    $('[data-field="relation_bot_key"]').value = "";
    markEditorDirty();
  };
  $("#remove-building-ambience").onclick = () => {
    $('[data-field="relation_ambience_key"]').value = "";
    markEditorDirty();
  };
  $$("[data-open-building-tab]").forEach(
    (button) =>
      (button.onclick = () =>
        root
          .querySelector(
            `[data-building-tab="${button.dataset.openBuildingTab}"]`,
          )
          ?.click()),
  );
  $("#add-activity").onclick = () => addActivityModule({ outcomes: [] });
  $("#add-delivery").onclick = () =>
    addDeliveryModule({
      source: "player_inventory",
      destination: "building_stock",
      minimum_quantity: 1,
      unit_price: 0,
      payment_resource: "money",
    });
  $("#add-product").onclick = () =>
    addProductModule({
      active: true,
  ...
  const simpleSoundPanel = root.querySelector('[data-building-panel="sound"]');
  simpleSoundPanel.querySelectorAll(":scope > section").forEach((section) => {
    section.hidden = true;
    section.dataset.advancedAudio = "true";
  });
  simpleSoundPanel.insertAdjacentHTML(
    "afterbegin",
    simpleAudioMarkup(payload, buildingKey, modules),
  );
  bindSimpleAudio();
  const simpleVisualPanel = root.querySelector(
    '[data-building-panel="visual"]',
  );
  simpleVisualPanel.querySelector(".visual-studio").hidden = true;
  simpleVisualPanel.insertAdjacentHTML("afterbegin", simpleDiscordMarkup());
  bindSimpleDiscord();
  bindVisualStudio();
  renderVisualStudio();
  setupSimpleGameplay(root);
  $$("[data-building-tab]").forEach(
    (button) =>
      (button.onclick = () => {
        $$("[data-building-tab]").forEach((item) =>
          item.classList.toggle("active", item === button),
        );
        $$("[data-building-panel]").forEach(
          (panel) =>
            (panel.hidden =
              panel.dataset.buildingPanel !== button.dataset.buildingTab),
        );
        if (button.dataset.buildingTab === "visual") renderVisualStudio();
        KingdomTutorials.notify(
          "building_tab_changed",
          button.dataset.buildingTab,
        );
      }),
  );
...
function installBuildingWorkbench(payload, buildingKey, modules) {
  const layout = $("#editor .editor-layout"),
    ...
    help = $("#context-help"),
    overview = $('[data-building-panel="overview"]'),
    advanced = $('[data-building-panel="advanced"]');
  ...
  navigator.className = "building-workbench-nav";
  navigator.setAttribute("aria-label", "Bâtiments du monde");
  const buildings = (state.catalogs.building || [])
    .slice()
    .sort((a, b) =>
      String(a.payload.name || a.entity_key).localeCompare(
        String(b.payload.name || b.entity_key),
        "fr",
      ),
    );
  ...
  const actionCount = (payload.actions || []).length,
    professionCount = (modules.professions || []).length,
    activityCount = (modules.activities || []).length,
    pageCount = (state.interfaceDraft?.pages || []).length;
  const warnings = [];
  if (!payload.location_key) warnings.push("Aucun lieu n'est associé.");
  ...
  // Le formulaire est la surface principale. Les deux panneaux secondaires
  // restent accessibles à la demande sans créer trois zones de défilement.
    ...
    (button) =>
      (button.onclick = async () => {
        ...
        if (
          state.editorDirty &&
          !confirm(
            "Des modifications ne sont pas enregistrées. Changer de bâtiment ?",
          )
        )
          return;
        const entity = state.catalogs.building.find(
          (item) => item.entity_key === button.dataset.workbenchBuilding,
        );
        if (entity) await openEditor(entity);
      }),
  );
  navigator.querySelector(".building-workbench-new").onclick = () => {
    if (
      state.editorDirty &&
      !confirm(
        "Des modifications ne sont pas enregistrées. Créer un autre bâtiment ?",
      )
    )
      return;
    closeEditor(true);
...
function itemMatchesFilter(entity, filter = "any") {
  ...
  const value =
    `${entity.payload.category || ""} ${entity.payload.type || ""}`.toLowerCase();
  if (filter === "tool") return /(tool|outil|equipment|équipement)/.test(value);
  if (filter === "resource")
    return /(resource|ressource|ingredient|ingrédient)/.test(value);
  if (filter === "consumable")
    return (
      entity.payload.consumable === true ||
      /(drink|food|boisson|repas|consommable)/.test(value)
    );
  return value.includes(filter.toLowerCase());
}
function itemReferenceOptions(
  currentValue = "",
  filter = "any",
  building = "",
  extraItems = [],
  ...
  const entries = state.catalogs.item
    .filter(
      (entity) =>
        itemMatchesFilter(entity, filter) &&
        (!building ||
          state.catalogs.itemEnriched
            .find((item) => item.id === entity.entity_key)
            ?.buildings.some((link) => link.key === building)),
    )
    .map((entity) => [
      entity.entity_key,
      `${entity.payload.emoji || "📦"} ${entity.payload.name}`,
    ]);
  for (const item of extraItems)
    if (
      item?.item_key &&
      !system.concat(entries).some(([key]) => key === item.item_key)
    )
      entries.unshift([
        item.item_key,
        `⚠️ Objet inconnu — ${item.item_key} (référence manquante)`,
      ]);
  if (
    currentValue &&
    !system.concat(entries).some(([key]) => key === currentValue)
  ) {
    const missing = itemDisplay(currentValue);
    entries.unshift([
      currentValue,
      `${missing.emoji} ${missing.name} — ${currentValue} (référence manquante)`,
    ]);
  }
...
function itemSelector(
  ...
  return `<label class="item-selector">${label}<input class="item-selector-search" data-item-search-for="${name}" placeholder="🔎 Rechercher dans les objets…"><select name="${name}" data-field="${name}" data-item-filter="${filter}" data-item-building="${escapeHtml(building)}">${itemReferenceOptions(
    value,
    filter,
    building,
    extraItems,
  ...
  root.querySelectorAll("[data-item-search-for]").forEach((search) => {
    search.oninput = () => {
      ...
      const selectField = container.querySelector(
        `[data-field="${CSS.escape(search.dataset.itemSearchFor)}"]`,
      );
...
function restoreContentToolbar() {
  const toolbar = $("#content-workspace .toolbar");
  if (!$("#search"))
    toolbar.innerHTML =
      '<input id="search" placeholder="Rechercher…"><span>Les changements publiés sont chargés par tous les modules.</span>';
  $("#search").oninput = () =>
    state.type === "bot" ? renderDiscordConnections() : renderCards();
  if (state.type !== "location") $("#cards").className = "cards";
}

function itemDisplay(id) {
  if (["money", "energy"].includes(id))
    return {
      id,
      name: id === "money" ? "Monnaie" : "Énergie",
      emoji: id === "money" ? "💰" : "⚡",
      category: "system",
    };
  ...
  return entity
    ? {
        id,
        name: entity.payload.name,
        emoji: entity.payload.emoji || "📦",
        category: entity.payload.category || entity.payload.type || "other",
      }
    : {
        id,
        name: "Objet inconnu",
...
async function loadItemCatalog() {
  // Une navigation depuis la carte peut laisser ses classes de mise en page sur
  // le conteneur partagé. Le catalogue reprend toujours sa propre grille.
  $("#cards").className = "cards item-card-grid";
  const filters = state.itemFilters,
    query = new URLSearchParams(filters),
    response = await fetch(`/api/admin/items?${query}`, { headers });
  if (!response.ok) {
    alert("Catalogue d’objets indisponible.");
    return;
  }
  const data = await response.json();
  ...
  $$("[data-clear-item]").forEach(
    (b) =>
      (b.onclick = () => {
        if (b.dataset.clearItem === "all")
        ...
        else filters[b.dataset.clearItem] = "";
        loadItemCatalog();
      }),
  );
}
...
function professionOptions(current = "") {
  const options = $$("#profession-modules > .profession-module")
    .map((element) => {
...
function addProfessionModule(profession = {}) {
  const element = document.createElement("details");
  element.className = "builder profession-module module-card";
  element.open = !profession.key;
  element.dataset.original = JSON.stringify(profession);
  ...
  $("#profession-modules").append(element);
  element.querySelector(".remove").onclick = () => {
    element.remove();
    refreshActivityProfessionOptions();
  };
  element.querySelector('[data-field="module_profession_name"]').oninput = (
    event,
  ) => {
    if (
      !element.querySelector('[data-field="module_profession_key"]').dataset
        .touched
    )
      element.querySelector('[data-field="module_profession_key"]').value =
        technicalKey(event.target.value, "metier");
    element.querySelector("summary strong").textContent =
      `📜 ${event.target.value || "Nouveau métier"}`;
    refreshActivityProfessionOptions();
  };
  element.querySelector('[data-field="module_profession_key"]').oninput = (
    event,
  ) => {
    event.target.dataset.touched = "true";
    element.querySelector("summary small").textContent =
      event.target.value || "à configurer";
    refreshActivityProfessionOptions();
  };
}

function refreshActivityProfessionOptions() {
  $$("#activity-modules .activity-module").forEach((element) => {
    const field = element.querySelector(
        '[data-field="module_activity_profession"]',
      ),
      value = field.value;
    field.innerHTML = professionOptions(value)
      .map(
        ([key, label]) =>
          `<option value="${escapeHtml(key)}" ${key === value ? "selected" : ""}>${escapeHtml(label)}</option>`,
      )
...
function addOutcomeModule(container, outcome = {}) {
  const element = document.createElement("details");
  element.className = "builder outcome-module";
  element.open = !outcome.key;
  element.dataset.original = JSON.stringify(outcome);
  ...
  (outcome.effects || legacyOutcomeEffects(outcome)).forEach((effect) =>
    addOutcomeEffect(element.querySelector(".outcome-effects"), effect),
  );
  element.querySelector(".add-outcome-effect").onclick = () =>
    addOutcomeEffect(element.querySelector(".outcome-effects"), {});
  element.querySelector(".remove").onclick = () => element.remove();
}

function legacyOutcomeEffects(outcome) {
  return Object.entries(outcome.rewards || {}).map(([resource, amount]) => ({
    type: "reward",
    resource,
    amount,
  }));
...
function addOutcomeEffect(container, effect = {}) {
  ...
  element.className = "builder outcome-effect";
  element.dataset.original = JSON.stringify(effect);
  container.append(element);
  const outcomeTypes = [
    ["reward", "Donner au joueur"],
    ["stock_reward", "Ajouter au stock d’un bâtiment"],
    ["cost", "Retirer au joueur"],
    ["play_audio", "Jouer un son"],
    ["set_audio_group", "Changer l’ambiance"],
    ["message", "Afficher un message"],
    ["profession", "Donner de l’expérience"],
    ["emit", "Envoyer un événement"],
    ["state", "Modifier un état"],
  ];
  const render = () => {
    const type =
      element.querySelector('[data-field="outcome_effect_type"]')?.value ||
      effect.type ||
      "reward";
    let fields = "";
    if (["reward", "cost", "stock_reward"].includes(type)) {
      const amount = effect.amount ?? 1,
        min = Array.isArray(amount) ? amount[0] : amount,
        max = Array.isArray(amount) ? amount[1] : amount;
      fields = `${itemSelector("Ressource", "outcome_effect_resource", effect.resource || effect.item || "", "resource")}${input("Minimum", "outcome_effect_min", min, "number")}${input("Maximum", "outcome_effect_max", max, "number")}${type === "stock_reward" ? select("Stock du bâtiment", "outcome_effect_building", effect.building || "", catalogOptions("building", effect.building || "")) : ""}`;
    } else if (type === "play_audio")
      fields = select(
        "Son joué",
        "outcome_effect_audio_key",
        effect.audio_key || "",
        audioOptions(effect.audio_key || ""),
      );
    else if (type === "set_audio_group")
      fields = select(
        "Nouvelle ambiance",
        "outcome_effect_group_key",
        effect.group_key || "",
        currentSoundGroupOptions(effect.group_key || ""),
      );
    else if (type === "message")
      fields = input("Message", "outcome_effect_text", effect.text || "");
    else if (type === "profession")
      fields = `${select("Métier", "outcome_effect_profession", effect.profession || "", professionOptions(effect.profession || ""))}${input("Expérience", "outcome_effect_experience", effect.experience || 0, "number")}${input("XP par niveau", "outcome_effect_xp_level", effect.experience_per_level || 100, "number")}`;
    else if (type === "emit")
      fields = `${select("Événement", "outcome_effect_event", effect.event || "", catalogOptions("event", effect.event || ""))}`;
    else
      fields = `${input("État à modifier", "outcome_effect_state", effect.key || "")}${select(
        "Opération",
        "outcome_effect_operation",
        effect.operation || "set",
        [
          ["set", "Définir"],
          ["increment", "Ajouter"],
        ],
      )}${input("Valeur", "outcome_effect_value", effect.value ?? 1, "number")}`;
    element.innerHTML = `<button type="button" class="remove">×</button><div class="outcome-effect-grid">${select("Type", "outcome_effect_type", type, outcomeTypes)}${fields}</div>`;
    element.querySelector('[data-field="outcome_effect_type"]').onchange = (
      event,
    ) => {
      effect = { type: event.target.value };
      render();
    };
    element.querySelector(".remove").onclick = () => element.remove();
  };
...
function addAction(action = {}) {
  ...
  const conditionGroup = action.conditions?.any ? "any" : "all";
  ...
  )}</div><div class="condition-editors"></div><div class="section-head"><div><b>Résultats</b><small class="field-note">Pour attribuer un métier : Ajouter → Gérer un métier → Attribuer.</small></div><button type="button" class="secondary add-inner">＋ Ajouter</button></div><div class="action-effects"></div><details><summary>⚡ Événements</summary><div class="form-grid">${select("Au lancement", "action_hook_start", hookEvent("on_start"), catalogOptions("event", hookEvent("on_start")))}${select("Après réussite", "action_hook_success", hookEvent("on_success"), catalogOptions("event", hookEvent("on_success")))}${select("En cas d’échec", "action_hook_failure", hookEvent("on_failure"), catalogOptions("event", hookEvent("on_failure")))}${select("À la récupération", "action_hook_claim", hookEvent("on_claim"), catalogOptions("event", hookEvent("on_claim")))}</div></details><details class="advanced"><summary>Identifiant technique</summary><div class="advanced-content">${input("Identifiant", "action_key", action.key || "")}</div></details></div>`;
  container.append(element);
  element.dataset.originalAction = JSON.stringify(action);
  (action.effects || []).forEach((effect) =>
    addEffect(element.querySelector(".action-effects"), effect),
  );
  const conditionRoot = action.conditions || {};
  const conditions =
    conditionRoot.all ||
    conditionRoot.any ||
    (Object.keys(conditionRoot).length ? [conditionRoot] : []);
  conditions.forEach((condition) =>
    addConditionEditor(element.querySelector(".condition-editors"), condition),
  );
  element.querySelector(".add-condition").onclick = () =>
    addConditionEditor(element.querySelector(".condition-editors"), {});
  element.querySelector(".add-inner").onclick = () => {
    addEffect(element.querySelector(".action-effects"), {});
    setHelp("effects");
  };
  ...
  const emojiField = element.querySelector('[data-field="action_emoji"]'),
    summary = element.querySelector("[data-action-summary]"),
    keySummary = element.querySelector("[data-action-key-summary]");
  const refreshSummary = () => {
    summary.textContent = `${emojiField.value || "⚙️"} ${nameField.value || "Nouvelle action"}`;
    keySummary.textContent = keyField.value || "à configurer";
  };
  nameField.addEventListener("input", () => {
    if (!keyField.dataset.touched)
      keyField.value = technicalKey(nameField.value, "action");
    refreshSummary();
  });
  emojiField.addEventListener("input", refreshSummary);
  keyField.addEventListener("input", () => {
    keyField.dataset.touched = "true";
    refreshSummary();
  });
}
...
function addEffect(container, effect = {}) {
  ...
  const types = [
    ["message", "Afficher un message"],
    ["reward", "Donner une ressource"],
    ["cost", "Retirer une ressource"],
    ["profession", "Gérer un métier / XP"],
    ["schedule", "Lancer une activité temporisée"],
    ["claim_scheduled", "Récupérer une activité terminée"],
    ...
    ["random_bundle", "Résultat groupé avancé"],
    ["random_message", "Message aléatoire avancé"],
    ["upgrade", "Amélioration avancée"],
  ];
  types.splice(
    3,
    0,
  ...
  const render = () => {
    ...
    let fields = "",
      afterRender = () => {};
    if (type === "message")
      fields = `<label class="effect-wide">Message affiché au joueur<textarea data-field="effect_text" rows="3" placeholder="Décris clairement ce qui vient de se passer.">${escapeHtml(effect.text || "")}</textarea></label><p class="effect-help">Un message n’a pas de quantité : seul ce texte sera affiché.</p>`;
    else if (["reward", "cost"].includes(type))
      fields = `${itemSelector(type === "reward" ? "Ressource donnée" : "Ressource retirée", "effect_resource", effect.resource || "", "any")}${input("Quantité", "effect_amount", effect.amount ?? 1, "number", "min=0")}`;
    else if (type === "emit")
      fields = `${select("Événement déclenché", "effect_event", effect.event || "", catalogOptions("event", effect.event || ""))}<p class="effect-help">KingdomEvent prévient les autres modules, notamment KingdomVoice.</p>`;
    else if (type === "profession") {
      const operation = effect.operation || "experience";
      fields = `${select(
        "Ce résultat doit…",
        "effect_profession_operation",
        operation,
        [
          ["join", "Attribuer / rejoindre le métier"],
          ["leave", "Faire quitter le métier"],
          ["experience", "Donner de l’expérience métier"],
        ],
      )}${select("Métier concerné", "effect_profession", effect.profession || "", professionOptions(effect.profession || ""))}${operation === "experience" ? input("Points d’expérience gagnés", "effect_profession_experience", effect.experience || effect.amount || 0, "number", "min=0") : ""}<p class="effect-help">L’expérience augmente automatiquement le niveau selon le seuil configuré dans le bâtiment.</p>`;
      afterRender = () => {
        element.querySelector(
          '[data-field="effect_profession_operation"]',
        ).onchange = (event) => {
          effect = {
            ...effect,
            type,
            operation: event.target.value,
            profession: fieldValue("effect_profession", element),
          };
          render();
        };
      };
      ...
      fields = `${itemSelector("Outil réparé", "effect_tool", effect.tool || "", "tool")}${input("Durabilité maximale", "effect_max_durability", effect.max_durability || 1, "number", "min=1")}${input("Prix par point", "effect_price_per_point", effect.price_per_point || 1, "number", "min=0")}`;
    else if (type === "claim_scheduled")
      fields = `${input("Identifiant de l’activité à récupérer", "effect_action", effect.action || "")}<p class="effect-help">Utilise exactement le même identifiant que dans « Lancer une activité temporisée ».</p>`;
    else if (type === "schedule") {
      fields = `<div class="effect-schedule-guide"><b>⏳ Déroulement</b><span>1. Le joueur paie les coûts placés avant cet effet.</span><span>2. Le moteur attend la durée configurée.</span><span>3. Le bouton de récupération remet les résultats ci-dessous.</span></div><div class="form-grid">${input("Identifiant de l’activité", "effect_action", effect.action || "activite", "text")}${input("Durée (secondes)", "effect_duration", effect.duration_seconds || 0, "number", "min=0")}${select(
        "Limite appliquée",
        "effect_limit_scope",
        effect.limit_scope || "player_action",
        [
          ["player", "Toutes les activités du joueur"],
          ["player_building", "Dans ce bâtiment"],
          ["player_action", "Cette activité seulement"],
          ["category", "Cette catégorie"],
        ],
      )}${input("Maximum simultané", "effect_max_active", effect.max_active || 1, "number", "min=1")}${input("Catégorie facultative", "effect_category", effect.category || "")}</div><div class="section-head"><div><b>Récompenses remises à la récupération</b><small class="field-note">Ajoute ici ressources, XP métier, messages ou événements.</small></div><button type="button" class="secondary add-scheduled-effect">＋ Ajouter un résultat final</button></div><div class="scheduled-effects"></div>`;
      afterRender = () => {
        const nested = element.querySelector(".scheduled-effects");
        (effect.effects || []).forEach((child) => addEffect(nested, child));
        element.querySelector(".add-scheduled-effect").onclick = () =>
          addEffect(nested, {});
      };
    } else
      fields = `<p class="effect-help">Ce résultat avancé conserve sa configuration actuelle. Pour une nouvelle activité, préfère les zones et résultats pondérés de la section Métiers.</p>`;
    ...
    element.querySelector('[data-field="effect_type"]').onchange = (event) => {
      effect = { type: event.target.value };
      element.dataset.originalEffect = JSON.stringify(effect);
      render();
    };
    ...
    afterRender();
    if (type === "play_audio")
      element.querySelector(".effect-specific-fields").innerHTML =
        `${select("Son joué dans le bâtiment", "effect_audio_key", effect.audio_key || "", audioOptions(effect.audio_key || ""))}<p class="effect-help">Le bot vocal attribué au bâtiment joue ce son, puis reprend l’ambiance générale.</p>`;
    if (type === "set_audio_group")
      element.querySelector(".effect-specific-fields").innerHTML =
        `${select("Nouvelle ambiance", "effect_group_key", effect.group_key || "", currentSoundGroupOptions(effect.group_key || ""))}<p class="effect-help">Le changement reste actif jusqu’au prochain groupe.</p>`;
  };
  render();
}

function readEffects(container) {
  return [...container.querySelectorAll(":scope > .effect-builder")].map(
    (element) => {
      const type = fieldValue("effect_type", element);
      if (type === "profession") {
        const operation =
            fieldValue("effect_profession_operation", element) || "experience",
          profession = fieldValue("effect_profession", element);
        return operation === "experience"
          ? {
              ...
              profession,
              experience:
                fieldValue("effect_profession_experience", element) || 0,
          ...
          : {
              ...
              profession,
              exclusive: true,
              block_when_pending: true,
            };
      }
      if (type === "play_audio")
        return { type, audio_key: fieldValue("effect_audio_key", element) };
      if (type === "set_audio_group")
        return { type, group_key: fieldValue("effect_group_key", element) };
      if (type === "schedule")
        return {
          ...JSON.parse(element.dataset.originalEffect || "{}"),
          type,
          action: fieldValue("effect_action", element),
          duration_seconds: fieldValue("effect_duration", element),
          limit_scope: fieldValue("effect_limit_scope", element),
          max_active: fieldValue("effect_max_active", element),
          category: fieldValue("effect_category", element) || "",
          effects: readEffects(element.querySelector(".scheduled-effects")),
        };
      ...
      if (["stock_cost", "stock_reward"].includes(type))
        return {
          type,
          item: fieldValue("effect_stock_item", element),
          amount: fieldValue("effect_amount", element),
          building: fieldValue("effect_stock_building", element),
        };
      if (type === "durability")
        return {
          type,
          tool: fieldValue("effect_tool", element),
          amount: fieldValue("effect_amount", element),
        };
      if (type === "repair")
        return {
          type,
          tool: fieldValue("effect_tool", element),
          max_durability: fieldValue("effect_max_durability", element),
          price_per_point: fieldValue("effect_price_per_point", element),
        };
      if (
        [
          "random_reward",
        ...
        return { ...JSON.parse(element.dataset.originalEffect || "{}"), type };
      if (type === "message")
        return { type, text: fieldValue("effect_text", element) };
      if (type === "emit")
        return {
          type,
          event: fieldValue("effect_event", element),
          payload: {},
        };
      return {
        type,
        resource: fieldValue("effect_resource", element),
        amount: fieldValue("effect_amount", element),
      };
    },
  );
...
function addConditionEditor(container, source = {}) {
  let negated = !!source.not,
    condition = clone(source.not || source);
  const element = document.createElement("div");
  element.className = "builder condition-editor";
  container.append(element);
  const render = () => {
    const type = condition.type || "resource";
    let reference = "";
    if (type === "resource")
      ...
      reference = select(
        ...
        professionOptions(condition.profession || ""),
      );
    else if (["tool_present", "tool_level", "tool_durability"].includes(type))
      reference = itemSelector(
        "Outil",
        "condition_ref",
        condition.tool || "",
      ...
      reference = input("État ou jauge", "condition_ref", condition.key || "");
    const scopes = ["no_pending_activity", "activity_limit_available"].includes(
      type,
    )
      ? select(
          "Portée",
          "condition_scope",
          condition.scope || "player_action",
          [
            ["player", "Joueur"],
            ["player_building", "Joueur + bâtiment"],
            ["player_action", "Joueur + action"],
            ["category", "Catégorie"],
          ],
        )
      : "";
    element.innerHTML = `<button type="button" class="remove">×</button><div class="condition-grid">${select(
      "Type",
      "condition_type",
      type,
      [
        ["resource", "Ressource minimale"],
        ["item_present", "Objet possédé"],
        ["item_absent", "Objet absent"],
        ["profession_active", "Métier actif"],
        ["no_active_profession", "Aucun métier actif"],
        ["profession_level", "Niveau de métier"],
        ["tool_present", "Outil possédé"],
        ["tool_level", "Niveau d’outil"],
        ["tool_durability", "Durabilité minimale"],
        ["voice_presence", "Présence vocale"],
        ["discord_role", "Rôle Discord"],
        ["no_pending_activity", "Aucune activité active"],
    ...
    )}${input("Valeur", "condition_value", condition.value ?? 1, "number")}${scopes}${check("Inverser (NOT)", "condition_not", negated)}</div><div class="reorder"><button type="button" class="secondary move-up">↑</button><button type="button" class="secondary move-down">↓</button></div>`;
    element.querySelector('[data-field="condition_type"]').onchange = (
      event,
    ) => {
    ...
    element.querySelector(".remove").onclick = () => element.remove();
    element.querySelector(".move-up").onclick = () =>
      element.previousElementSibling &&
      container.insertBefore(element, element.previousElementSibling);
    element.querySelector(".move-down").onclick = () =>
...
function readConditions(actionElement) {
  const conditions = [
    ...actionElement.querySelectorAll(".condition-editors > .condition-editor"),
  ].map((element) => {
    const type = fieldValue("condition_type", element),
      reference = fieldValue("condition_ref", element);
    const condition = {
      type,
      operator: fieldValue("condition_operator", element),
    ...
    if (type === "resource") condition.resource = reference;
      ...
      condition.profession = reference;
    else if (["tool_present", "tool_level", "tool_durability"].includes(type))
      condition.tool = reference;
    else if (type === "discord_role") condition.role = reference;
    else if (type === "state") condition.key = reference;
    if (fieldValue("condition_scope", element))
      condition.scope = fieldValue("condition_scope", element);
    return fieldValue("condition_not", element)
      ? { not: condition }
      : condition;
  });
  return conditions.length
    ? { [fieldValue("condition_group", actionElement) || "all"]: conditions }
    : undefined;
}

function readHooks(element, prefix = "action_hook_") {
  return Object.fromEntries(
    [
      ...
      .map(([hook, suffix]) => [
        hook,
        fieldValue(prefix + suffix, element)
          ? [{ event: fieldValue(prefix + suffix, element), payload: {} }]
          : null,
      ])
      .filter(([, value]) => value),
  );
...
function addDeliveryModule(rule = {}) {
  const element = document.createElement("div");
  element.className = "builder delivery-module";
  element.dataset.original = JSON.stringify(rule);
  const events = rule.events || {};
  element.innerHTML = `<button type="button" class="remove">×</button><div class="form-grid">${select("Ressource", "delivery_resource", rule.item_key || rule.resource || "", catalogOptions("item", rule.item_key || rule.resource || ""))}${select("Source", "delivery_source", rule.source || "player_inventory", [["player_inventory", "Inventaire du joueur"]])}${select(
    "Destination",
    "delivery_destination",
    rule.destination || "building_stock",
    [
      ["building_stock", "Stock d’un bâtiment"],
      ["player_inventory", "Inventaire joueur"],
    ],
  )}${select("Bâtiment destinataire", "delivery_building", rule.target_building_key || rule.building || "", catalogOptions("building", rule.target_building_key || rule.building || ""))}${input("Quantité minimale", "delivery_min", rule.minimum_quantity ?? 1, "number", "min=1")}${input("Quantité maximale (0 = aucune)", "delivery_max", rule.maximum_quantity ?? 0, "number", "min=0")}${input("Prix par unité", "delivery_price", rule.unit_price ?? 0, "number", "min=0")}${select("Monnaie de paiement", "delivery_currency", rule.payment_resource || rule.currency || "money", catalogOptions("item", rule.payment_resource || rule.currency || "money"))}${select("Événement au début", "delivery_event_start", events.on_start?.event || "", catalogOptions("event", events.on_start?.event || ""))}${select("Événement après livraison", "delivery_event_success", events.on_success?.event || "", catalogOptions("event", events.on_success?.event || ""))}${select("Événement en cas d’échec", "delivery_event_failure", events.on_failure?.event || "", catalogOptions("event", events.on_failure?.event || ""))}</div>`;
  element.insertAdjacentHTML(
    "beforeend",
    `<div class="section-head"><b>Conditions</b><button type="button" class="secondary add-delivery-condition">＋ Ajouter</button></div>${select(
      "Combinaison",
      "condition_group",
      rule.conditions?.any ? "any" : "all",
      [
        ["all", "Toutes les conditions"],
        ["any", "Au moins une condition"],
      ],
    )}<div class="condition-editors"></div>`,
  );
  $("#delivery-modules").append(element);
  const conditionRoot = rule.conditions || {},
    conditions =
      conditionRoot.all ||
      conditionRoot.any ||
      (Object.keys(conditionRoot).length ? [conditionRoot] : []);
  ...
  element.querySelector(".add-delivery-condition").onclick = () =>
    addConditionEditor(element.querySelector(".condition-editors"), {});
  element.querySelector(".remove").onclick = () => element.remove();
}

function readDeliveryModules() {
  return $$("#delivery-modules > .delivery-module").map((element) => {
    const maximum = fieldValue("delivery_max", element),
      events = {},
      conditions = readConditions(element);
    [
      ["on_start", "delivery_event_start"],
      ["on_success", "delivery_event_success"],
      ["on_failure", "delivery_event_failure"],
    ].forEach(([key, field]) => {
      if (fieldValue(field, element))
        events[key] = { event: fieldValue(field, element), payload: {} };
    });
    return {
      ...JSON.parse(element.dataset.original || "{}"),
      item_key: fieldValue("delivery_resource", element),
      source: fieldValue("delivery_source", element),
      destination: fieldValue("delivery_destination", element),
      target_building_key: fieldValue("delivery_building", element),
      minimum_quantity: fieldValue("delivery_min", element),
      maximum_quantity: maximum > 0 ? maximum : null,
      unit_price: fieldValue("delivery_price", element),
      payment_resource: fieldValue("delivery_currency", element) || "money",
      events,
      conditions,
    };
  });
...
function addProductModule(product = {}) {
  const element = document.createElement("details");
  element.className = "module-card product-module";
  element.open = !product.item_key;
  element.innerHTML = `<summary><strong>${escapeHtml(product.emoji || "🛒")} ${escapeHtml(product.name || itemDisplay(product.item_key).name || "Nouveau produit")}</strong><small>${Number(product.price || 0)} écus</small></summary><div class="module-content"><button type="button" class="remove">×</button><div class="form-grid">${itemSelector("Objet vendu", "product_item", product.item_key || "", "consumable")}${input("Nom affiché", "product_name", product.name || "")}${input("Emoji", "product_emoji", product.emoji || "🛒")}${input("Prix", "product_price", product.price || 0, "number", "min=0")}${input("Stock initial", "product_stock", product.initial_stock || 0, "number", "min=0")}${input("Maximum par commande", "product_max", product.maximum_per_purchase || 1, "number", "min=1")}</div><div class="checks">${check("Disponible au comptoir", "product_active", product.active !== false)}</div></div>`;
  $("#product-modules").append(element);
  bindItemSelectors(element);
  element.querySelector(".remove").onclick = () => element.remove();
}
function readProductModules() {
  return $$("#product-modules > .product-module").map((element) => ({
    item_key: fieldValue("product_item", element),
    name:
      fieldValue("product_name", element) ||
      itemDisplay(fieldValue("product_item", element)).name,
    emoji: fieldValue("product_emoji", element) || "🛒",
    price: fieldValue("product_price", element),
    initial_stock: fieldValue("product_stock", element),
    maximum_per_purchase: fieldValue("product_max", element),
    active: fieldValue("product_active", element),
  }));
}

function addRecipeIngredient(root, key = "", amount = 1) {
  const row = document.createElement("div");
  row.className = "form-grid recipe-ingredient";
  row.innerHTML = `${itemSelector("Ingrédient", "recipe_ingredient_item", key, "resource")}${input("Quantité", "recipe_ingredient_amount", amount, "number", "min=1")}<button type="button" class="remove">×</button>`;
  root.append(row);
...
function addRecipeModule(recipe = {}) {
  ...
  element.querySelector(".add-recipe-ingredient").onclick = () =>
    addRecipeIngredient(element.querySelector(".recipe-ingredients"));
  element.querySelector(":scope > .module-content > .remove").onclick = () =>
    element.remove();
}
function readRecipeModules() {
  return $$("#recipe-modules > .recipe-module").map((element, index) => {
    const original = JSON.parse(element.dataset.original || "{}");
    const ingredients = Object.fromEntries(
      $$(".recipe-ingredient", element)
        .filter((row) => fieldValue("recipe_ingredient_item", row))
        .map((row) => [
          fieldValue("recipe_ingredient_item", row),
...
function addRumorModule(rumor = {}) {
  const element = document.createElement("details");
  element.className = "module-card rumor-module";
  element.open = !rumor.key;
  element.innerHTML = `<summary><strong>🗣️ ${escapeHtml(rumor.key || "Nouveau récit")}</strong><small>poids ${Number(rumor.weight || 1)}</small></summary><div class="module-content"><button type="button" class="remove">×</button><div class="form-grid">${input("Identifiant", "rumor_key", rumor.key || "")}${input("Poids", "rumor_weight", rumor.weight || 1, "number", "min=0.01 step=0.01")}${input("Événement / audio facultatif", "rumor_event", rumor.event || rumor.audio || "")}</div><label>Texte raconté<textarea data-field="rumor_text" rows="4">${escapeHtml(rumor.text || "")}</textarea></label><div class="checks">${check("Récit actif", "rumor_enabled", rumor.enabled !== false)}</div></div>`;
  $("#rumor-modules").append(element);
  element.querySelector(".remove").onclick = () => element.remove();
}
function readRumorModules() {
  return $$("#rumor-modules > .rumor-module").map((element, index) => ({
    key: fieldValue("rumor_key", element) || `story_${index + 1}`,
    text: fieldValue("rumor_text", element),
    weight: fieldValue("rumor_weight", element) || 1,
    event: fieldValue("rumor_event", element) || undefined,
    enabled: fieldValue("rumor_enabled", element),
  }));
}

function addGameChoice(root, choice = {}) {
  const row = document.createElement("div");
  row.className = "module-card game-choice";
  row.innerHTML = `<button type="button" class="remove">×</button><div class="form-grid">${input("Identifiant du choix", "game_choice_key", choice.key || "")}${input("Libellé", "game_choice_name", choice.name || "")}${input("Issues gagnantes (séparées par des virgules)", "game_choice_wins", (choice.winning_outcomes || choice.winning_faces || []).join(", "))}${input("Multiplicateur", "game_choice_multiplier", choice.multiplier || 1, "number", "min=0 step=0.1")}</div>`;
  root.append(row);
  row.querySelector(".remove").onclick = () => row.remove();
}
function addGameModule(game = {}) {
  const element = document.createElement("details");
  element.className = "module-card game-module";
  element.open = !game.key;
  element.dataset.original = JSON.stringify(game);
  element.innerHTML = `<summary><strong>🎲 ${escapeHtml(game.name || game.key || "Nouveau jeu")}</strong><small>${Number(game.sides || 6)} issues</small></summary><div class="module-content"><button type="button" class="remove">×</button><div class="form-grid">${input("Identifiant", "game_key", game.key || "")}${input("Nom", "game_name", game.name || "")}${input("Mise", "game_stake", game.stake || 0, "number", "min=0")}${itemSelector("Monnaie de mise", "game_currency", game.stake_resource || "money")}${input("Nombre d'issues", "game_sides", game.sides || 6, "number", "min=2")}</div><div class="section-head"><b>Choix proposés au joueur</b><button type="button" class="secondary add-game-choice">＋ Ajouter un choix</button></div><div class="game-choices"></div></div>`;
  $("#game-modules").append(element);
  bindItemSelectors(element);
  (game.choices || game.bets || []).forEach((choice) =>
    addGameChoice(element.querySelector(".game-choices"), choice),
  );
  element.querySelector(".add-game-choice").onclick = () =>
    addGameChoice(element.querySelector(".game-choices"));
  element.querySelector(":scope > .module-content > .remove").onclick = () =>
    element.remove();
}
function readGameModules() {
  return Object.fromEntries(
    $$("#game-modules > .game-module").map((element, index) => {
      const original = JSON.parse(element.dataset.original || "{}");
      const key = fieldValue("game_key", element) || `game_${index + 1}`;
      const choices = $$(".game-choice", element).map((row, choiceIndex) => ({
        key: fieldValue("game_choice_key", row) || `choice_${choiceIndex + 1}`,
        name: fieldValue("game_choice_name", row),
        winning_outcomes: String(fieldValue("game_choice_wins", row) || "")
          .split(",")
          .map((value) => Number(value.trim()))
          .filter(Number.isFinite),
        multiplier: fieldValue("game_choice_multiplier", row) || 1,
      }));
      return [
        key,
        {
          ...original,
          key,
          name: fieldValue("game_name", element) || key,
          stake: fieldValue("game_stake", element),
          stake_resource: fieldValue("game_currency", element) || "money",
          sides: fieldValue("game_sides", element) || 6,
          choices,
        },
      ];
    }),
...
function readProfessionModules() {
  return $$("#profession-modules > .profession-module").map(
    (element, index) => ({
      ...JSON.parse(element.dataset.original || "{}"),
      key:
        fieldValue("module_profession_key", element) ||
        technicalKey(
          fieldValue("module_profession_name", element),
          `metier_${index + 1}`,
        ),
      name: fieldValue("module_profession_name", element),
      emoji: fieldValue("module_profession_emoji", element),
      ...
      tool_level: fieldValue("module_profession_tool_level", element),
      initial_durability: fieldValue(
        "module_profession_initial_durability",
        element,
      ),
      max_durability: fieldValue("module_profession_max_durability", element),
    }),
  );
}

function readOutcomeEffects(container) {
  return [...container.querySelectorAll(":scope > .outcome-effect")].map(
    (element) => {
      const type = fieldValue("outcome_effect_type", element);
      if (type === "play_audio")
        return {
          type,
          audio_key: fieldValue("outcome_effect_audio_key", element),
        };
      if (type === "set_audio_group")
        return {
          type,
          group_key: fieldValue("outcome_effect_group_key", element),
        };
      if (["reward", "cost", "stock_reward"].includes(type)) {
        const minimum = fieldValue("outcome_effect_min", element),
          maximum = fieldValue("outcome_effect_max", element);
        const amount = minimum === maximum ? minimum : [minimum, maximum];
        if (type === "stock_reward")
          return {
            type,
            item: fieldValue("outcome_effect_resource", element),
            building: fieldValue("outcome_effect_building", element),
            amount,
          };
        return {
          type,
          resource: fieldValue("outcome_effect_resource", element),
          amount,
        };
      }
      if (type === "message")
        return { type, text: fieldValue("outcome_effect_text", element) };
      if (type === "profession")
        return {
          type,
          profession: fieldValue("outcome_effect_profession", element),
          experience: fieldValue("outcome_effect_experience", element),
          experience_per_level: fieldValue("outcome_effect_xp_level", element),
        };
      if (type === "emit")
        return {
          type,
          event: fieldValue("outcome_effect_event", element),
          payload: JSON.parse(element.dataset.original || "{}").payload || {},
        };
      return {
        type: "state",
        key: fieldValue("outcome_effect_state", element),
        operation: fieldValue("outcome_effect_operation", element),
        value: fieldValue("outcome_effect_value", element),
      };
    },
  );
}
...
function readActivityModules() {
  return $$("#activity-modules > .activity-module").map((element, index) => ({
    ...JSON.parse(element.dataset.original || "{}"),
    key:
      fieldValue("module_activity_key", element) ||
      technicalKey(
        fieldValue("module_activity_name", element),
        `zone_${index + 1}`,
      ),
    name: fieldValue("module_activity_name", element),
    emoji: fieldValue("module_activity_emoji", element),
    description: fieldValue("module_activity_description", element),
    profession: fieldValue("module_activity_profession", element),
    tool: fieldValue("module_activity_tool", element) || undefined,
    required_level: fieldValue("module_activity_level", element),
    duration_seconds: fieldValue("module_activity_duration", element),
    energy_cost: fieldValue("module_activity_energy", element),
    durability_cost: fieldValue("module_activity_durability", element),
    minimum_durability: fieldValue("module_activity_min_durability", element),
    tool_max_durability: fieldValue("module_activity_max_durability", element),
    activity_limit: {
      scope: fieldValue("module_activity_scope", element),
      max_active: fieldValue("module_activity_max_active", element),
      category: fieldValue("module_activity_category", element),
    },
    hooks: Object.fromEntries(
      [
        ["on_start", "module_hook_start"],
        ["on_success", "module_hook_success"],
        ["on_failure", "module_hook_failure"],
        ["on_claim", "module_hook_claim"],
      ]
        .map(([hook, field]) => [
          hook,
          fieldValue(field, element)
            ? [{ event: fieldValue(field, element), payload: {} }]
            : null,
        ])
        .filter(([, value]) => value),
    ),
    outcomes: [
      ...element.querySelectorAll(
        ":scope > .module-content > .outcome-modules > .outcome-module",
      ),
    ].map((outcome, outcomeIndex) => ({
      key:
        fieldValue("module_outcome_key", outcome) ||
        `result_${outcomeIndex + 1}`,
      weight: Number(fieldValue("module_outcome_weight", outcome)),
...
function buildPayload() {
  const payload =
    state.type === "building"
      ? clone(state.buildingBase || {})
      : state.type === "interface"
        ? clone(state.interfaceDraft || {})
        : ["audio", "bot", "environment"].includes(state.type)
          ? clone(state.editing?.payload || {})
          : {};
  ...
  if (state.type === "building") {
    let modules = {};
    try {
      modules = JSON.parse(fieldValue("modules_json") || "{}");
    } catch (_) {
      throw Error("La configuration modulaire contient un JSON invalide.");
    }
    ...
    modules.rumors = {
      ...(modules.rumors || {}),
      catalogue: readRumorModules(),
      player_cooldown_seconds: fieldValue("rumor_player_cooldown"),
      global_cooldown_seconds: fieldValue("rumor_global_cooldown"),
    };
    modules.games = readGameModules();
    modules.delivery_mode = fieldValue("delivery_all")
      ? "all_available"
      : "selected_quantity";
    const soundGroups = readSoundGroups(),
      simpleAmbience = fieldValue("relation_ambience_key") || "";
    if (simpleAmbience) {
      let globalGroup = soundGroups.find(
        (group) => group.key === "global_ambience",
      );
      if (!globalGroup) {
        globalGroup = {
          key: "global_ambience",
          ...
          volume: 1,
          tracks: { music: [], ambience: [], sfx: [], voice: [] },
        };
        soundGroups.unshift(globalGroup);
      }
      globalGroup.tracks ||= {};
      globalGroup.tracks.ambience = [simpleAmbience];
    } else {
      const globalGroup = soundGroups.find(
        (group) => group.key === "global_ambience",
      );
      if (globalGroup) {
        globalGroup.tracks ||= {};
        globalGroup.tracks.ambience = [];
      }
    }
    ...
    modules.audio = {
      ...
      default_group_key: simpleAmbience
        ...
        : selectedAudioGroup === "global_ambience"
          ...
          : selectedAudioGroup,
      groups: soundGroups,
      event_routes: readSoundRoutes(),
    };
    const buildingKey = $("#key").value.trim();
    const previousTarget = state.interfaceDraft?.target_building_key;
    const interfaceDefinition = clone(
      state.interfaceDraft ||
        blankInterface(buildingKey, payload.name, payload.emoji, payload.color),
    );
    if (!state.editing || state.duplicate)
      interfaceDefinition.name = `Interface - ${payload.name}`;
    else interfaceDefinition.name ||= `Interface - ${payload.name}`;
    interfaceDefinition.target_building_key = buildingKey;
    interfaceDefinition.theme ||= {};
    interfaceDefinition.theme.color =
      fieldValue("color") || interfaceDefinition.theme.color || "7a1f1f";
    interfaceDefinition.pages.forEach((page) =>
      page.components.forEach((component) => {
        if (
          component.interaction?.type === "action" &&
          (!component.interaction.building ||
            component.interaction.building === previousTarget)
        )
          component.interaction.building = buildingKey;
        (component.options || []).forEach((option, index) => {
          option.key = technicalKey(
            option.key || option.label || `option_${index + 1}`,
            `option_${index + 1}`,
          );
          if (
            option.interaction?.type === "action" &&
            (!option.interaction.building ||
              option.interaction.building === previousTarget)
          )
            option.interaction.building = buildingKey;
        });
      }),
    );
    Object.assign(payload, {
      building_kind:
        ...
        state.editing?.payload?.building_kind ||
        "custom",
      location_key: fieldValue("building_location") || "",
      color: fieldValue("color") || "7c5cff",
      npc_name: fieldValue("npc_name") || "",
      action_mode: fieldValue("action_mode") || "manual",
      modules,
      relations: {
        ...(payload.relations || {}),
        primary_profession_key:
          fieldValue("relation_primary_profession") ||
          modules.professions[0]?.key ||
          "",
        ambience_audio_key: fieldValue("relation_ambience_key") || "",
      },
      interface: interfaceDefinition,
      access: {
        visible: fieldValue("building_visible") !== false,
        required_roles: (fieldValue("required_roles") || "")
          .split(",")
          .map((value) => value.trim())
          .filter(Boolean),
        temporary_text: fieldValue("temporary_text") !== false,
      },
      actions: $$("#actions > .action-builder").map((element, index) => {
        const action = {
          ...JSON.parse(element.dataset.originalAction || "{}"),
          key:
            fieldValue("action_key", element) ||
            technicalKey(
              fieldValue("action_name", element),
              `action_${index + 1}`,
            ),
          name: fieldValue("action_name", element),
          emoji: fieldValue("action_emoji", element),
          enabled: fieldValue("action_enabled", element),
          effects: readEffects(element.querySelector(".action-effects")),
          hooks: readHooks(element),
        };
        const conditions = readConditions(element);
        if (conditions) action.conditions = conditions;
        else delete action.conditions;
        return action;
      }),
    });
  }
  if (state.type === "building")
    Object.assign(payload, {
      entity_kind: fieldValue("entity_kind") || payload.entity_kind || "place",
      category: fieldValue("entity_category") || "",
      tags: (fieldValue("entity_tags") || "")
        .split(",")
        .map((value) => value.trim())
        .filter(Boolean),
      parent_key: fieldValue("entity_parent") || "",
    });
  if (state.type === "item")
    Object.assign(payload, {
      category: fieldValue("category"),
      type: fieldValue("type"),
      rarity: fieldValue("rarity"),
      price: fieldValue("price"),
      stack_limit: fieldValue("stack_limit"),
      stackable: fieldValue("stackable"),
      consumable: fieldValue("consumable"),
      sellable: fieldValue("sellable"),
      building_relations: $$("[data-item-building]:checked").map((field) => ({
        building_key: field.dataset.itemBuilding,
        relation: $(
          `[data-item-relation="${CSS.escape(field.dataset.itemBuilding)}"]`,
        ).value,
      })),
    });
  if (state.type === "event") {
  ...
  if (state.type === "npc")
    Object.assign(payload, {
      ...
      location_key: fieldValue("npc_location") || "",
      building_key: fieldValue("npc_building") || "",
      voice_profile_key: payload.voice_profile_key || "",
      voice_presence_key: payload.voice_presence_key || "",
      state: fieldValue("npc_state") || "disponible",
      behavior: fieldValue("npc_behavior") || "contextuel",
      tags: (fieldValue("npc_tags") || "")
        .split(",")
        .map((value) => value.trim())
        .filter(Boolean),
      reactions: readNpcReactions(),
      dialogues: payload.dialogues || [],
    });
  if (state.type === "profession")
    Object.assign(payload, {
      emoji: fieldValue("profession_emoji") || "⚒️",
      required_item: fieldValue("profession_item") || "",
      initial_level: fieldValue("profession_level") || 1,
      experience_per_level: fieldValue("profession_xp") || 100,
    });
  if (state.type === "environment")
    Object.assign(payload, {
      mode: fieldValue("environment_mode") || "manual",
      clock_mode: fieldValue("clock_mode") || "accelerated",
      day: fieldValue("environment_day") || 1,
      hour: fieldValue("environment_hour") ?? 12,
      minute: fieldValue("environment_minute") ?? 0,
      speed: fieldValue("environment_speed") ?? 1,
      weather_interval_seconds: fieldValue("weather_interval") || 3600,
      weather_options: readWeatherOptions(),
      weather: {
        ...(payload.weather || {}),
        key: technicalKey(fieldValue("weather_name") || "beau", "meteo"),
        name: fieldValue("weather_name") || "Beau",
        emoji: fieldValue("weather_emoji") || "☀️",
        modifiers: readWorldModifiers(),
      },
    });
  if (state.type === "location")
    Object.assign(payload, {
      location_type: fieldValue("location_type") || "place",
      parent_key: fieldValue("location_parent") || "",
      tags: (fieldValue("location_tags") || "")
        .split(",")
        .map((value) => value.trim())
        .filter(Boolean),
      map: {
        x: fieldValue("location_map_x") || 0,
        y: fieldValue("location_map_y") || 0,
      },
      exploration_enabled: fieldValue("location_exploration"),
      activities: readLocationActivities(),
      connections: readLocationConnections(),
    });
  if (state.type === "bot")
    Object.assign(payload, {
      bot_type: fieldValue("bot_type"),
      application_id_env: fieldValue("application_id_env"),
      token_env: fieldValue("token_env"),
      guild_id: fieldValue("guild_id"),
      presence: fieldValue("presence"),
      server_nickname: fieldValue("server_nickname") || "",
      server_bio: fieldValue("server_bio") || "",
      avatar_path: fieldValue("avatar_path") || "",
      enabled: fieldValue("enabled"),
      auto_join: fieldValue("auto_join"),
      voice_channel_id: fieldValue("voice_channel_id") || "0",
      voice_channel_env: fieldValue("voice_channel_env"),
      building_key: fieldValue("building_key"),
      leave_delay: fieldValue("leave_delay"),
      welcome_folder: fieldValue("welcome_folder") ?? payload.welcome_folder,
      music_folder: fieldValue("music_folder") ?? payload.music_folder,
      ambience_folder: fieldValue("ambience_folder") ?? payload.ambience_folder,
      phrase_folder: fieldValue("phrase_folder") ?? payload.phrase_folder,
      volume: {
        voice: fieldValue("volume_voice"),
        music: fieldValue("volume_music"),
        ambience: fieldValue("volume_ambience"),
        sfx: fieldValue("volume_sfx"),
      },
    });
  if (state.type === "audio")
    Object.assign(payload, {
      audio_type: fieldValue("audio_type"),
      channel: fieldValue("audio_type"),
      speaker_bot_key: fieldValue("speaker_bot_key") || "",
      tags: (fieldValue("audio_tags") || "")
        .split(",")
        .map((value) => value.trim())
        .filter(Boolean),
...
async function publishItem(key, version) {
  ...
  const response = await fetch(
    `/api/content/${state.type}/${key}/${version}/publish`,
    { method: "POST", headers, body: "{}" },
  );
  if (!response.ok) {
    setSaveState("error", "Publication échouée");
    alert((await response.json()).detail);
    return;
  }
...
function markEditorDirty() {
  if ($("#editor").hidden || state.referencePreview) return;
  state.editorDirty = true;
  setSaveState("saving", "Modifications non enregistrées");
}
function closeEditor(force = false) {
  if (
    state.editorDirty &&
    !force &&
    !confirm("Des modifications ne sont pas enregistrées. Fermer quand même ?")
  )
    return false;
  const returnType = state.referencePreview ? state.referenceReturnType : null;
  resetEditor();
  state.referencePreview = false;
  state.referenceReturnType = null;
  if (returnType) state.type = returnType;
  $("#editor").hidden = true;
...
function metricCard(label, value, detail = "") {
  return `<article class="admin-metric"><span>${label}</span><strong>${escapeHtml(value)}</strong>${detail ? `<small>${escapeHtml(detail)}</small>` : ""}</article>`;
}
function inventoryChips(inventory) {
  ...
  return entries.length
    ? `<div class="inventory-chips">${entries.map((item) => `<span class="inventory-chip ${item.missing ? "missing-reference" : ""}" title="ID : ${escapeHtml(item.id)}">${escapeHtml(item.emoji || "📦")} ${escapeHtml(item.name || "Objet inconnu")} × ${item.quantity}${item.missing ? " · ⚠ Référence manquante" : ""}</span>`).join("")}</div>`
    : '<span class="empty-admin">Vide</span>';
}
function adminTable(headersList, rows) {
  return `<div class="admin-table-wrap"><table class="admin-table"><thead><tr>${headersList.map((item) => `<th>${item}</th>`).join("")}</tr></thead><tbody>${rows.join("")}</tbody></table></div>`;
}
...
function renderClientAdministration(data) {
  const metrics = data.metrics,
    capabilities = data.services
      .map(
        (service) =>
          `<article class="service-card ${service.running ? "is-running" : "is-stopped"}"><header><span class="service-icon">${escapeHtml(service.emoji || "◉")}</span><div><small>DISPONIBILITÉ</small><strong>${escapeHtml(service.name)}</strong></div><span class="status-pill ${service.running ? "running" : ""}"><i></i>${service.running ? "DISPONIBLE" : "INDISPONIBLE"}</span></header><p>${service.running ? "Aucune intervention nécessaire." : "Une alerte a été remontée à la supervision."}</p></article>`,
      )
      .join("");
  const recent =
    data.activity
      ...
      .map(
        (item) =>
          `<article class="audit-row"><b>${escapeHtml(item.player_name || item.discord_id)}</b> · ${escapeHtml(item.action_name || item.action_key)}<small>${escapeHtml(item.building_name || item.building_key)} · ${formatDate(item.created_at)}</small></article>`,
      )
      .join("") || '<p class="empty-admin">Aucune activité récente.</p>';
  $("#admin-view").innerHTML =
    `<div class="admin-grid supervision-page"><section class="supervision-hero"><div><small>SUPERVISION DU MONDE</small><h2>Santé et alertes</h2><p>Les informations techniques internes restent gérées par Payen Studio.</p></div><div class="supervision-health"><strong>${metrics.running_services}/${data.services.length}</strong><span>capacités disponibles</span></div></section><section class="admin-metrics">${metricCard("DISPONIBILITÉ", `${metrics.running_services}/${data.services.length}`)}${metricCard("CONTENUS PUBLIÉS", metrics.published_buildings)}${metricCard("JOUEURS", metrics.players)}${metricCard("ACTIVITÉS EN COURS", metrics.pending_jobs)}${metricCard("ÉVÉNEMENTS ACTIFS", metrics.active_events)}</section><section class="admin-section"><div class="admin-section-head"><div><small>ÉTAT DU MONDE</small><h2>Fonctions disponibles</h2></div><span class="live-refresh"><i></i> Actualisation automatique</span></div><div class="service-grid">${capabilities}</div></section><section class="admin-section"><div class="admin-section-head"><div><small>ACTIVITÉ</small><h2>Dernières opérations du monde</h2></div></div>${recent}</section></div>`;
}

function renderAdministration(data) {
  if (data.item_catalog)
    state.catalogs.item = data.item_catalog.items.map((item) => ({
      entity_key: item.id,
      payload: item.payload,
      status: item.status,
      version: item.version,
  ...
  const services = data.services
    .map((service) => {
      const description =
        service.key === "core"
          ? "Moteur Discord, interfaces et mécanique de jeu."
          : service.key === "voice"
            ? "Bots vocaux, ambiances et effets sonores."
            : "Studio no-code, API et supervision du royaume.";
      const processLabel = service.pid ? `PID ${service.pid}` : "Non lancé";
      const controls = service.controllable
        ? `<div class="service-actions"><button data-service="${service.key}" data-operation="start" ${service.running ? "disabled" : ""}>▶ Démarrer</button><button class="service-restart" data-service="${service.key}" data-operation="restart" ${!service.running ? "disabled" : ""}>↻ Redémarrer</button><button class="service-stop" data-service="${service.key}" data-operation="stop" ${!service.running ? "disabled" : ""}>■ Arrêter</button></div>`
        : `<div class="service-host-note">● Service hôte · arrêté avec le serveur de test</div>`;
      return `<article class="service-card ${service.running ? "is-running" : "is-stopped"}"><header><span class="service-icon">${service.emoji}</span><div><small>${escapeHtml(service.key.toUpperCase())}</small><strong>${escapeHtml(service.name)}</strong></div><span class="status-pill ${service.running ? "running" : ""}"><i></i>${service.running ? "EN LIGNE" : "ARRÊTÉ"}</span></header><div class="service-visual"><span><small>PROCESSUS</small><b>${processLabel}</b></span><span><small>DÉMARRÉ LE</small><b>${service.started_at ? formatDate(service.started_at) : "—"}</b></span></div><p>${description}</p>${controls}</article>`;
    })
    .join("");
  const buildings = adminTable(
    ["Bâtiment", "Publication", "Interface", "Actions", "Tâches", "Stock"],
    data.buildings.map(
      (building) =>
        `<tr><td><b>${building.emoji} ${escapeHtml(building.name)}</b><br><small>${building.key} · v${building.version}</small></td><td><span class="status-pill ${building.status}">${building.status.toUpperCase()}</span></td><td>${building.pages} page(s)</td><td>${building.actions}</td><td>${building.pending}</td><td><details><summary>${building.stock_total} unité(s)</summary>${inventoryChips(building.stock)}</details></td></tr>`,
    ),
  );
  const players = adminTable(
    [
      "Joueur Discord",
      "Économie",
      "Métiers",
    ...
    data.players.map(
      (player) =>
        `<tr><td><b>${escapeHtml(player.discord_id)}</b></td><td>💰 ${player.money}<br>⚡ ${player.energy}</td><td>${player.professions.length ? player.professions.map((job) => `${escapeHtml(job.key)} niv. ${job.level} (${job.experience} XP)`).join("<br>") : "—"}</td><td>${inventoryChips(player.inventory)}</td><td>${formatDate(player.updated_at)}</td></tr>`,
    ),
  );
  const activity = adminTable(
    ["Date", "Joueur", "Bâtiment", "Action"],
    data.activity.map(
      (item) =>
        `<tr><td>${formatDate(item.created_at)}</td><td>${escapeHtml(item.discord_id)}</td><td>${escapeHtml(item.building_key)}</td><td>${escapeHtml(item.action_key)}</td></tr>`,
    ),
  );
  $("#admin-view").innerHTML =
    `<div class="admin-grid supervision-page"><section class="supervision-hero" data-supervision-overview><div><small>SUPERVISION DU ROYAUME</small><h2>Services & journaux</h2><p>Contrôlez les moteurs Discord et repérez immédiatement les incidents.</p></div><div class="supervision-health"><strong>${metrics.running_services}/${data.services.length}</strong><span>services en ligne</span></div></section><section class="admin-metrics" data-supervision-overview>${metricCard("SERVICES", `${metrics.running_services}/${data.services.length}`, "en ligne")}${metricCard("BÂTIMENTS", metrics.published_buildings, `${metrics.buildings} définitions`)}${metricCard("JOUEURS", metrics.players)}${metricCard("TÂCHES EN COURS", metrics.pending_jobs)}${metricCard("BASE", `${Math.max(1, Math.round(data.database.size_bytes / 1024))} Ko`)}</section><div id="service-operation-status" class="service-operation-status" hidden></div><section class="admin-section services-section" data-supervision-panel="services"><div class="admin-section-head"><div><small>INFRASTRUCTURE</small><h2>État des services</h2></div><span class="live-refresh"><i></i> Actualisation toutes les 5 secondes</span></div><div class="service-grid">${services}</div></section><section class="admin-section" data-supervision-panel="buildings"><div class="admin-section-head"><h2>État des bâtiments</h2><small>Publication, interfaces, actions et stocks</small></div>${buildings}</section><div class="admin-split" data-supervision-panel="players"><section class="admin-section"><div class="admin-section-head"><h2>Joueurs et inventaires</h2><small>${data.players.length} joueur(s)</small></div>${players}</section><section class="admin-section"><div class="admin-section-head"><h2>Activité récente</h2><small>30 dernières actions</small></div>${activity}</section></div></div>`;
}
...
async function fetchOverview(message = "Chargement…") {
  showSystemView();
  $("#admin-view").innerHTML =
    `<div class="empty-admin"><span class="refresh-spin">⟳</span> ${message}</div>`;
  const response = await fetch("/api/admin/overview", { headers });
  if (!response.ok) {
    $("#admin-view").innerHTML =
      `<div class="empty-admin">Données indisponibles : ${escapeHtml((await response.json()).detail || response.status)}</div>`;
    return null;
  }
  return response.json();
}

// Snapshot analytique : toutes les valeurs proviennent de /api/admin/overview.
// Aucun historique n'est fabriqué : les graphiques comparent uniquement des valeurs présentes.
function dashboardAnalytics(data, activeToday) {
  const m = data.metrics || {};
  const counts = [
    ["Joueurs", m.players], ["Actifs 24 h", activeToday],
    ["Bâtiments", m.buildings], ["Publiés", m.published_buildings],
    ["Événements", m.active_events], ["Tâches", m.pending_jobs],
  ].map(([label, raw]) => [label, Number(raw)]).filter(([, value]) => Number.isFinite(value) && value >= 0);
  const max = Math.max(1, ...counts.map(([, value]) => value));
  const bars = counts.map(([label, value]) => `<div class="kw-analytics-bar"><span>${escapeHtml(label)}</span><div role="meter" aria-label="${escapeHtml(label)}" aria-valuemin="0" aria-valuemax="${max}" aria-valuenow="${value}"><i style="width:${Math.max(0, Math.min(100, value / max * 100))}%"></i></div><strong>${value.toLocaleString("fr-FR")}</strong></div>`).join("");
  const players = Array.isArray(data.players) ? data.players : [];
  const recent = [...players].sort((a, b) => (Date.parse(b.updated_at || "") || 0) - (Date.parse(a.updated_at || "") || 0)).slice(0, 8);
  const rows = recent.map(player => {
    const name = player.display_name || player.name || player.username || player.discord_id || "Joueur sans nom";
    const date = Date.parse(player.updated_at || "");
    const updated = Number.isFinite(date) ? new Date(date).toLocaleString("fr-FR") : "Non renseignée";
    return `<tr><td>${escapeHtml(String(name))}</td><td>${escapeHtml(updated)}</td></tr>`;
  }).join("");
  return `<section class="royal-panel kw-analytics" data-analytics="snapshot"><div class="royal-panel-title"><div><small>DONNÉES DU ROYAUME</small><h2>Indicateurs en un regard</h2></div><span class="kw-source-label">Vue instantanée · API de supervision</span></div><p class="kw-analytics-note">Comparaison des volumes actuels. Ce graphique ne représente pas une évolution dans le temps.</p><div class="kw-analytics-bars">${bars}</div><div class="kw-analytics-footer"><button type="button" data-go="supervision">Ouvrir la supervision détaillée →</button></div></section><section class="royal-panel kw-analytics" data-analytics="players"><div class="royal-panel-title"><div><small>JOUEURS</small><h2>Dernières mises à jour</h2></div><span class="kw-source-label">${players.length} profil(s)</span></div><div class="kw-table-scroll"><table class="kw-data-table"><thead><tr><th scope="col">Joueur</th><th scope="col">Dernière mise à jour</th></tr></thead><tbody>${rows || '<tr><td colspan="2">Aucun joueur disponible.</td></tr>'}</tbody></table></div><p class="kw-analytics-note">Dernière mise à jour du profil, pas nécessairement une connexion Discord.</p><button type="button" data-go="players">Gérer les joueurs →</button></section>`;
}
...
async function loadDashboard() {
  ...
  const metrics = data.metrics,
    activeToday = data.players.filter(
      (player) => Date.now() - new Date(player.updated_at).getTime() < 86400000,
    ).length;
  const serviceMetric = (key) =>
    data.services.find((service) => service.key === key) ||
    data.services.find((service) => service.name.toLowerCase().includes(key));
  const rank = (items, kind) =>
    items
      .map((player, index) => {
        const xp = (player.professions || []).reduce(
          (sum, job) => sum + job.experience,
          0,
        );
        const detail =
          kind === "wealth"
            ? `${player.money} écus`
            : `${player.professions.length} métier(s) · ${xp} XP`;
        return `<li><span class="rank">${index + 1}</span>${player.avatar_url ? `<img src="${escapeHtml(player.avatar_url)}" alt="">` : `<i>${escapeHtml((player.display_name || "?")[0])}</i>`}<b>${escapeHtml(player.display_name || player.discord_id)}</b><strong>${detail}</strong></li>`;
      })
      .join("") || `<li class="empty-admin">Aucun habitant classé.</li>`;
  const projects = (data.projects || [])
    .map(
      (project) =>
        `<section class="royal-project"><div class="project-head"><div><small>CHANTIER COLLECTIF</small><h2>${project.emoji} ${escapeHtml(project.name)}</h2><p>${escapeHtml(project.description)}</p></div><strong>${project.progress} %</strong></div><progress max="100" value="${project.progress}"></progress><div class="project-stages">${project.stages.map((stage, index) => `<article class="${stage.complete ? "complete" : ""}"><h3>${stage.complete ? "✓" : "◆"} Étape ${index + 1} · ${escapeHtml(stage.name)}</h3>${stage.requirements.map((item) => `<p><span>${item.emoji} ${escapeHtml(item.name)}</span><b>${item.current} / ${item.required}</b></p>`).join("")}</article>`).join("")}</div></section>`,
    )
    .join("");
  const activity =
    data.activity
  ...
  const activeEvents =
    (data.events || [])
      ...
      .map(
        (event) =>
          `<article class="dashboard-event"><span>${escapeHtml(event.emoji)}</span><div><b>${escapeHtml(event.name)}</b><small>${escapeHtml(event.trigger === "manual" ? "Déclenchement manuel" : event.trigger)}</small></div><i>${escapeHtml(event.status)}</i></article>`,
      )
      .join("") ||
    `<p class="empty-admin">Aucun événement actif pour le moment.</p>`;
  const alerts = [
    ...data.services
      ...
      .map((service) => ({
        ...
        detail: "Cette capacité peut nécessiter une intervention",
        kind: "danger",
      })),
    ...(metrics.pending_jobs
      ...
      : [
          {
            icon: "✓",
            title: "Aucune activité bloquée",
            detail: "Les actions temporisées sont disponibles",
            kind: "success",
          },
        ]),
  ].slice(0, 5);
...
async function loadSupervision(background = false) {
  ...
  if (!background) {
    clearInterval(state.adminTimer);
    state.adminTimer = setInterval(() => {
      if (state.type === "supervision") loadSupervision(true);
    }, 5000);
  }
}
...
function renderSupervision(data) {
  if (data.client_safe) {
    renderClientAdministration(data);
    return;
  }
  ...
  const logs = Object.entries(data.logs || {})
    .map(([key, value]) => {
      const errors = value.errors || [],
        ...
        content =
          [...errors, ...output].join("\n") ||
          "Aucun journal disponible. Démarrez le service pour activer la capture.";
      return `<details class="log-panel ${errors.length ? "has-errors" : "is-healthy"}" data-log-key="${escapeHtml(key)}" open><summary><span class="log-service-icon">${errors.length ? "!" : "✓"}</span><span><b>${escapeHtml(key.toUpperCase())}</b><small>${errors.length ? `${errors.length} ligne(s) d’erreur` : `${output.length} ligne(s) · aucun incident détecté`}</small></span><i>⌄</i></summary><div class="log-toolbar"><span>${errors.length ? "Attention requise" : "Journal opérationnel"}</span><button type="button" class="secondary copy-log" data-copy-log="${escapeHtml(key)}">▣ Copier</button></div><pre data-log-content="${escapeHtml(key)}">${escapeHtml(content)}</pre></details>`;
    })
    .join("");
  grid.insertAdjacentHTML(
    "beforeend",
    `<section class="admin-section logs-section" data-supervision-panel="logs"><div class="admin-section-head"><div><small>DIAGNOSTIC</small><h2>Journaux des services</h2></div><div><small>Actualisation suspendue pendant la lecture</small> <button type="button" class="secondary" id="refresh-logs">↻ Actualiser</button></div></div><div class="logs-grid">${logs}</div></section>`,
  );
  $$("[data-supervision-overview]").forEach(
    (panel) => (panel.hidden = state.supervisionTab !== "overview"),
  );
  $$("[data-supervision-panel]").forEach(
    (panel) =>
      (panel.hidden = panel.dataset.supervisionPanel !== state.supervisionTab),
  );
  grid.children[0].insertAdjacentHTML(
    "afterend",
    `<nav class="section-tabs" aria-label="Sections de supervision"><button data-supervision-tab="overview" class="${state.supervisionTab === "overview" ? "active" : ""}">Vue générale</button><button data-supervision-tab="services" class="${state.supervisionTab === "services" ? "active" : ""}">Services</button><button data-supervision-tab="buildings" class="${state.supervisionTab === "buildings" ? "active" : ""}">Bâtiments</button><button data-supervision-tab="players" class="${state.supervisionTab === "players" ? "active" : ""}">Joueurs & activité</button><button data-supervision-tab="logs" class="${state.supervisionTab === "logs" ? "active" : ""}">Journaux</button></nav>`,
  );
  $$("[data-supervision-tab]").forEach(
    (button) =>
      (button.onclick = () => {
        state.supervisionTab = button.dataset.supervisionTab;
        $$("[data-supervision-tab]").forEach((item) =>
          item.classList.toggle("active", item === button),
        );
        $$("[data-supervision-overview]").forEach(
          (panel) => (panel.hidden = state.supervisionTab !== "overview"),
        ...
        $$("[data-supervision-panel]").forEach(
          (panel) =>
            (panel.hidden =
              panel.dataset.supervisionPanel !== state.supervisionTab),
        );
        if (state.supervisionTab === "logs") {
          clearInterval(state.adminTimer);
          state.adminTimer = null;
        } else if (!state.adminTimer)
          state.adminTimer = setInterval(() => {
            if (state.type === "supervision") loadSupervision(true);
          }, 5000);
      }),
    ...
    (button) =>
      (button.onclick = async () => {
        ...
        if (
          operation === "stop" &&
          !confirm(
            `Arrêter ${service} ? Les fonctions associées seront indisponibles jusqu’au prochain démarrage.`,
          )
        )
          return;
        clearInterval(state.adminTimer);
        state.adminTimer = null;
        const status = $("#service-operation-status");
        status.hidden = false;
        status.className = "service-operation-status busy";
        status.textContent = `${operation === "restart" ? "Redémarrage" : operation === "stop" ? "Arrêt" : "Démarrage"} de ${service} en cours…`;
        $$("[data-service]").forEach((item) => (item.disabled = true));
        try {
          const response = await fetch(
            `/api/admin/services/${service}/${operation}`,
            { method: "POST", headers, body: "{}" },
          );
          const result = await response.json();
          if (!response.ok)
            throw Error(result.detail || "Opération impossible.");
          status.className = "service-operation-status success";
          status.textContent = `${result.name} est maintenant ${result.running ? "en ligne" : "arrêté"}.`;
          await new Promise((resolve) => setTimeout(resolve, 500));
          await loadSupervision(true);
        } catch (error) {
          status.className = "service-operation-status error";
          status.textContent = error.message;
        } finally {
          if (
            state.type === "supervision" &&
            state.supervisionTab !== "logs" &&
            !state.adminTimer
          )
            state.adminTimer = setInterval(() => loadSupervision(true), 5000);
        }
      }),
  );
  ...
  $$(".copy-log").forEach(
    (button) =>
      (button.onclick = async () => {
        const content =
          $(`[data-log-content="${button.dataset.copyLog}"]`)?.textContent ||
          "";
        await navigator.clipboard.writeText(content);
        button.textContent = "Copié !";
        setTimeout(() => (button.textContent = "Copier ce journal"), 1200);
      }),
  );
}

const settingField = (label, path, value, type = "text") =>
  `<label>${label}<input data-setting="${path}" type="${type}" value="${escapeHtml(value ?? "")}"></label>`;
const settingArea = (label, path, value) =>
  `<label>${label}<textarea data-setting="${path}" rows="8">${escapeHtml(value || "")}</textarea></label>`;
const settingCheck = (label, path, value) =>
  `<label class="check"><input data-setting="${path}" type="checkbox" ${value ? "checked" : ""}><span>${label}</span></label>`;

async function loadSettings() {
  showSystemView();
  $("#admin-view").innerHTML =
    `<div class="empty-admin"><span class="refresh-spin">⟳</span> Chargement des paramètres…</div>`;
  const [response, provisionResponse] = await Promise.all([
    fetch("/api/server/settings", { headers }),
    fetch("/api/admin/discord/provision/status", {
      headers,
      cache: "no-store",
    }),
  ]);
  if (!response.ok) {
    $("#admin-view").innerHTML =
      `<div class="empty-admin">Paramètres indisponibles.</div>`;
    return;
  }
  state.settingsEntity = await response.json();
  state.discordProvisionStatus = provisionResponse.ok
    ? await provisionResponse.json()
    : { status: "never", installed: false };
  renderSettings(state.settingsEntity.payload, state.discordProvisionStatus);
}

function renderSettings(
  settings,
  provision = { status: "never", installed: false },
) {
  ...
  const provisionButton = $("#install-discord-server");
  provisionButton.className = "primary";
  provisionButton.textContent = provision.installed
    ? "Synchroniser maintenant"
    : "Installer KingdomEngine sur Discord";
  $("#save-settings").className = "secondary";
  $("#save-settings").textContent = "Enregistrer les paramètres";
  onboardingEnabled
    ?.closest("label")
    ?.insertAdjacentHTML(
      "afterend",
      settingField(
        "Dotation remise à chaque nouveau joueur",
        "onboarding.starting_money",
        onboarding.starting_money ?? 100,
        "number",
      ) +
        settingField(
          "Nom de la monnaie affichée",
          "onboarding.currency_label",
          onboarding.currency_label || "unités",
        ) +
        settingField(
          "Nom de la validation",
          "onboarding.action_name",
          onboarding.action_name || "validation initiale",
        ),
    );
...
async function requestServerProvision() {
  const button = $("#install-discord-server"),
    status = $("#discord-provision-status");
  button.disabled = true;
  status.textContent = "Demande envoyée à KingdomCore…";
  const response = await fetch("/api/admin/discord/provision", {
      method: "POST",
      headers,
  ...
  if (!response.ok) {
    button.disabled = false;
    status.textContent = result.detail || "Installation impossible.";
    return;
  }
  status.textContent = "Installation en cours sur Discord…";
  const started = Date.now(),
    poll = setInterval(async () => {
      const check = await fetch("/api/admin/discord/provision/status", {
        headers,
        cache: "no-store",
      });
      if (!check.ok) return;
      ...
      if (current.status === "done") {
        clearInterval(poll);
        status.textContent = `✓ Serveur synchronisé · ${current.report || "salons et rôles à jour"}`;
        button.textContent = "Synchroniser maintenant";
        button.disabled = false;
      } else if (current.status === "failed") {
        clearInterval(poll);
        status.textContent = `Échec : ${current.error}`;
        button.disabled = false;
      } else if (Date.now() - started > 120000) {
        clearInterval(poll);
        status.textContent =
          "KingdomCore ne répond pas. Vérifiez qu’il est démarré.";
        button.disabled = false;
      }
    }, 1500);
}
...
async function persistBuildingBotRelation(buildingKey, botKey) {
  const selected = state.catalogs.bot.find(
      (item) => item.entity_key === botKey,
    ),
    currentlyAssigned = state.catalogs.bot.filter(
      (item) =>
        item.payload.building_key === buildingKey && item.entity_key !== botKey,
    );
  if (
    selected?.payload.building_key &&
    selected.payload.building_key !== buildingKey
  ) {
    const former =
      state.catalogs.building.find(
        (item) => item.entity_key === selected.payload.building_key,
      )?.payload.name || selected.payload.building_key;
    if (
      !confirm(
        `${selected.payload.name} est actuellement associé à ${former}. Voulez-vous le transférer vers ce bâtiment ?`,
      )
    )
      throw Error("Transfert du bot annulé.");
  }
  const update = async (entity, target) => {
    const payload = {
      ...clone(entity.payload),
      building_key: target,
      voice_channel_id: entity.payload.voice_channel_id || "0",
    };
    const response = await fetch(
      `/api/content/bot/${encodeURIComponent(entity.entity_key)}`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
          payload,
          expected_version: entity.version,
          author: "studio-building-relation",
        }),
      },
    );
    if (!response.ok)
      throw Error(
        `Le bâtiment a été enregistré, mais la relation du bot n’a pas pu être modifiée : ${(await response.json()).detail}`,
      );
    const saved = await response.json();
    if (entity.status === "published") {
      const publication = await fetch(
        `/api/content/bot/${encodeURIComponent(entity.entity_key)}/${saved.version}/publish`,
        { method: "POST", headers, body: "{}" },
      );
      if (!publication.ok)
        throw Error("La nouvelle association du bot n’a pas pu être publiée.");
    }
  };
  if (selected && selected.payload.building_key !== buildingKey)
    await update(selected, buildingKey);
  for (const entity of currentlyAssigned) await update(entity, "");
}

async function loadDiscordChannelAudit() {
  const button = $("#audit-discord-channels"),
    target = $("#discord-channel-audit");
  button.disabled = true;
  button.textContent = "Analyse en cours…";
  const response = await fetch("/api/admin/discord/channels/audit", {
      headers,
      cache: "no-store",
    }),
    data = await response.json();
  button.disabled = false;
  button.textContent = "Analyser à nouveau";
  if (!response.ok) {
    target.innerHTML = `<p class="warning-box">${escapeHtml(data.detail || "Analyse impossible")}</p>`;
    return;
  }
  if (!data.configured) {
    target.innerHTML = `<p class="warning-box">${escapeHtml(data.message)}</p>`;
    return;
  }
  const affected = data.buildings.filter(
    (item) => item.duplicates.length || item.stale_mapping,
  );
  ...
  const cleanup = $("#cleanup-discord-channels");
  if (cleanup) {
    cleanup.dataset.allowEmpty = String(data.stale_mapping_count > 0);
    cleanup.onclick = cleanupDiscordChannels;
  }
}

async function cleanupDiscordChannels() {
  const channel_ids = $$("[data-duplicate-channel]:checked").map(
      (item) => item.dataset.duplicateChannel,
    ),
    allowEmpty = $("#cleanup-discord-channels")?.dataset.allowEmpty === "true";
  if (!channel_ids.length && !allowEmpty) {
    alert("Sélectionnez au moins un doublon.");
    return;
  }
  if (
    !confirm(
      `Supprimer définitivement ${channel_ids.length} salon(s) Discord sélectionné(s) ? Les salons non reconnus restent protégés.`,
    )
  )
    return;
  const response = await fetch("/api/admin/discord/channels/cleanup", {
      method: "POST",
      headers,
      body: JSON.stringify({ confirmed: true, channel_ids }),
    }),
    data = await response.json();
  if (!response.ok) {
    alert(data.detail);
  ...
  alert(`${data.deleted_count} doublon(s) supprimé(s). ${data.message}`);
  await loadDiscordChannelAudit();
}
...
async function saveSettings() {
  const button = $("#save-settings"),
    payload = clone(state.settingsEntity.payload);
  ...
  const response = await fetch("/api/server/settings", {
    ...
    body: JSON.stringify({
      payload,
      expected_version: state.settingsEntity.version,
    }),
  });
  const data = await response.json();
  button.disabled = false;
...
function bindNavigationShortcuts() {
  $$("[data-go]").forEach(
    (button) => (button.onclick = () => navigateTo(button.dataset.go)),
  );
}
function playerSystemView() {
  showSystemView();
  $("#new").hidden = true;
}
const playerName = (p) => p.display_name || `Joueur ${p.discord_id}`;
const optionList = (entries, key = "key", label = "name") =>
  entries
    .map(
      (x) =>
        `<option value="${escapeHtml(x[key])}">${escapeHtml(`${x.emoji || ""} ${x[label]}`.trim())}</option>`,
    )
    .join("");

function relativeActivity(activity) {
  if (!activity) return "En attente";
  return `${activity.action_name || activity.action_key}${activity.remaining_seconds > 0 ? ` · fin dans ${activity.remaining_seconds} s` : " · prêt"}`;
}
function unifiedInventory(items) {
  return (
    items
      .map(
        (item) =>
          `<div class="live-inventory-item ${item.missing ? "missing-reference" : ""}"><span>${escapeHtml(item.emoji || "📦")}</span><div><b>${escapeHtml(item.name)}</b> <strong>×${item.quantity}</strong>${item.tool_state ? `<small>Durabilité ${item.tool_state.durability}/${item.tool_state.max_durability} · niv. ${item.tool_state.level}${item.tool_state.loot_bonus ? ` · bonus +${item.tool_state.loot_bonus}%` : ""}</small>` : ""}${item.missing ? `<small>⚠ Référence manquante · ${escapeHtml(item.item_key)}</small>` : ""}</div></div>`,
      )
      .join("") || "<em>Inventaire vide</em>"
  );
}
function playerLiveCard(p) {
  const active = p.professions.find((job) => job.active),
    activity = p.current_activity,
    expanded = state.expandedPlayers.includes(String(p.discord_id));
...
async function loadPlayers(background = false) {
  playerSystemView();
  const filters = state.playerFilters,
  ...
  if (!background)
    $("#admin-view").innerHTML =
      '<div class="empty-admin"><span class="refresh-spin">⟳</span> Chargement des habitants…</div>';
  ...
  $("#refresh-players").onclick = () => loadPlayers();
  $("#player-prev").onclick = () => {
    state.playerPage--;
    loadPlayers();
  };
  ...
  clearInterval(state.adminTimer);
  state.adminTimer = setInterval(() => {
    if (state.type === "players" && !document.hidden) loadPlayers(true);
  }, 5000);
}

function bindLivePlayerCards() {
  $$(".player-inline-admin").forEach(
    (details) =>
      (details.ontoggle = () => {
        const id = details.closest("[data-live-player]").dataset.livePlayer;
        state.expandedPlayers = details.open
          ? [...new Set([...state.expandedPlayers, id])]
          : state.expandedPlayers.filter((value) => value !== id);
      }),
  );
  $$("[data-card-mutation]").forEach(
    (button) =>
      (button.onclick = async () => {
        const id = button.closest("[data-live-player]").dataset.livePlayer,
          detail = await fetchPlayerDetail(id);
        if (detail)
          showPlayerMutation(
            detail,
            button.dataset.cardMutation,
            button.dataset.resource,
          );
      }),
  );
  $$("[data-player-advanced]").forEach(
    (button) =>
      (button.onclick = async () => {
        const id = button.closest("[data-live-player]").dataset.livePlayer,
          detail = await fetchPlayerDetail(id);
        if (detail) showPlayerAdvanced(detail);
      }),
  );
}
...
function historyRows(data) {
  const entries = [
    ...data.history.actions.map((x) => ({
      ...x,
      origin: "Moteur",
      summary: `${x.building_key} · ${x.action_key}`,
    })),
    ...data.history.deliveries.map((x) => {
      const item = itemDisplay(x.resource_key);
      return {
        ...x,
        origin: "Livraison",
        summary: `${x.quantity} ${item.name}${item.missing ? ` (${x.resource_key} · référence manquante)` : ""} · ${x.total_payment} ¤`,
      };
    }),
    ...data.history.administration.map((x) => ({
      ...x,
      origin: "Administration",
      summary: `${x.action} · ${x.target} — ${x.reason}`,
    })),
  ...
  return (
    entries
      .map(
        (x) =>
          `<tr><td>${formatDate(x.created_at)}</td><td>${escapeHtml(x.origin)}</td><td>${escapeHtml(x.summary)}</td></tr>`,
      )
      .join("") || '<tr><td colspan="3">Aucun historique conservé.</td></tr>'
  );
}
function playerPanel(data) {
  const p = data.player,
    active = data.professions.find((x) => x.active),
    pending = data.activities.find((x) => x.status === "pending");
...
async function openPlayer(id) {
  ...
  const data = await response.json(),
    p = data.player;
  $("#admin-view").innerHTML =
    `<div class="player-detail"><button id="back-players">← Retour aux joueurs</button><header class="player-hero"><div class="player-avatar">${p.avatar_url ? `<img src="${escapeHtml(p.avatar_url)}">` : "👤"}</div><div><small>FICHE JOUEUR</small><h2>${escapeHtml(playerName(p))}</h2><p>${escapeHtml(p.discord_id)}</p></div><button id="refresh-player">⟳ Actualiser</button></header><nav class="section-tabs player-tabs">${[
      ...
      .map(
        ([k, l]) =>
          `<button data-player-tab="${k}" class="${state.playerTab === k ? "active" : ""}">${l}</button>`,
      )
      .join("")}</nav><div id="player-panel">${playerPanel(data)}</div></div>`;
  $("#back-players").onclick = () => {
    state.playerId = null;
    loadPlayers();
  };
  $("#refresh-player").onclick = () => openPlayer(id);
  $$("[data-player-tab]").forEach(
    (b) =>
      (b.onclick = () => {
  ...
  $$("[data-mutation]").forEach(
    (b) =>
      (b.onclick = () =>
        showPlayerMutation(data, b.dataset.mutation, b.dataset.resource)),
  );
  $$("[data-activity]").forEach(
    (b) =>
      (b.onclick = () =>
        showPlayerMutation(data, "activity", null, {
          id: Number(b.dataset.activity),
          operation: b.dataset.operation,
        })),
  );
  $$("[data-cooldown]").forEach(
    (b) =>
      (b.onclick = () => {
        const [building, action] = b.dataset.cooldown.split("|");
        showPlayerMutation(data, "cooldown", null, {
...
function showPlayerMutation(data, type, resource = null, preset = {}) {
  let dialog = $("#player-admin-dialog");
  if (!dialog) {
    dialog = document.createElement("dialog");
    dialog.id = "player-admin-dialog";
    document.body.append(dialog);
  }
  const itemOptions = optionList(data.catalogs.items),
    professionOptions = optionList(data.catalogs.professions),
    tools = optionList(data.catalogs.items);
  let fields = "";
  if (type === "resource")
    fields = `<input type="hidden" name="resource" value="${resource}"><label>Valeur actuelle<input disabled value="${data.player[resource]}"></label><label>Opération<select name="operation"><option value="add">Ajouter</option><option value="remove">Retirer</option><option value="set">Définir</option></select></label><label>Montant<input name="amount" type="number" min="0" required></label>`;
  if (type === "inventory")
    fields = `${itemSelector(
      "Objet",
      "item_key",
      ...
      "any",
      "",
      data.inventory.filter((item) => item.missing),
    )}<label>Opération<select name="operation"><option value="add">Ajouter</option><option value="remove">Retirer</option><option value="set">Définir la quantité</option></select></label><label>Quantité<input name="amount" type="number" min="0" required></label><p class="field-note">Une référence manquante peut uniquement être retirée ou définie à zéro.</p>`;
  if (type === "profession")
    fields = `<label>Métier<select name="profession_key">${professionOptions}</select></label><label>Opération<select name="operation"><option value="join">Attribuer</option><option value="leave">Faire quitter</option><option value="add_xp">Ajouter de l’XP</option><option value="set_xp">Définir l’XP</option></select></label><label>Expérience<input name="experience" type="number" min="0" value="0"></label>`;
  if (type === "tool")
    fields = `${itemSelector("Outil", "tool_key", "", "tool")}<label>Opération<select name="operation"><option value="grant">Attribuer</option><option value="repair">Réparer</option><option value="update">Modifier</option><option value="remove">Retirer</option></select></label><div class="cols"><label>Durabilité<input name="durability" type="number" min="0" value="100"></label><label>Maximum<input name="max_durability" type="number" min="1" value="100"></label></div><div class="cols"><label>Niveau<input name="level" type="number" min="1" value="1"></label><label>Bonus<input name="loot_bonus" type="number" value="0"></label></div>`;
  if (type === "activity")
  ...
  dialog.querySelector("form").onsubmit = async (event) => {
    ...
    if (
      ...
      return;
    const response = await fetch(
      `/api/admin/players/${encodeURIComponent(data.player.discord_id)}/${path}`,
      { method: "POST", headers, body: JSON.stringify(body) },
    );
    if (!response.ok) {
      dialog.querySelector("#player-mutation-error").textContent = (
        await response.json()
      ).detail;
      return;
    }
    dialog.close();
    await openPlayer(data.player.discord_id);
  };
}

function navigateTo(type) {
  const button = $(`#nav [data-type="${type}"]`);
  if (button) {
    const submenu = button.closest("[data-nav-submenu]");
    if (submenu) openNavigationGroup(submenu.dataset.navSubmenu);
    button.click();
  }
}
...
function openNavigationGroup(group) {
  ...
  $$("[data-nav-submenu]").forEach((menu) => {
    menu.hidden = false;
    const trigger = $(`[data-nav-group="${menu.dataset.navSubmenu}"]`);
...
function activateNavigation(button) {
  ...
  $$("#nav [data-type]").forEach((item) =>
    item.classList.toggle("active", item === button),
  );
  $$("#nav [data-nav-group]").forEach((item) =>
    item.classList.toggle(
      "active",
      parent?.dataset.navSubmenu === item.dataset.navGroup,
    ),
  );
  $$("#mobile-dock [data-mobile-type]").forEach((item) =>
    item.classList.toggle(
      "active",
      item.dataset.mobileType === button.dataset.type,
    ),
  );
}
...
$("#cards").addEventListener("click", async (event) => {
  ...
  if (event.target.closest(".building-card-actions details"))
    event.stopPropagation();
  const preview = event.target.closest("[data-audio-preview]");
  if (preview) {
    event.stopPropagation();
    await previewAudio(preview.dataset.audioPreview);
    return;
  }
  ...
  if (invite) {
    event.stopPropagation();
    const popup = window.open("about:blank", "_blank");
    const response = await fetch(`/api/bots/${invite.dataset.invite}/invite`, {
      headers,
    });
    const data = await response.json();
    if (!response.ok) {
  ...
  const credentials = event.target.closest("[data-worker-credentials]");
  if (credentials) {
    event.stopPropagation();
    openWorkerCredentials(credentials.dataset.workerCredentials);
    return;
  }
  ...
  if (duplicate) {
    event.stopPropagation();
    const entity = state.items.find(
      (item) => item.entity_key === duplicate.dataset.duplicate,
    );
    if (entity) openEditor(entity, true);
    return;
  }
  const publish = event.target.closest("[data-publish]");
  if (publish) {
    event.stopPropagation();
    await publishItem(publish.dataset.publish, Number(publish.dataset.version));
    return;
  }
  ...
  if (remove) {
    event.stopPropagation();
    const entity = state.items.find(
      (item) => item.entity_key === remove.dataset.delete,
    );
    if (
      !entity ||
      !confirm(
        `Supprimer « ${entity.payload.name} » ?\n\nCette définition disparaîtra du Studio, mais son historique restera conservé.`,
      )
    )
      return;
    const response = await fetch(
      `/api/content/${state.type}/${remove.dataset.delete}`,
      { method: "DELETE", headers },
    );
    if (!response.ok) {
      alert((await response.json()).detail);
...
$("#cards").addEventListener("keydown", (event) => {
  if (["Enter", " "].includes(event.key)) {
    ...
    if (card) {
      event.preventDefault();
      const entity = state.items.find(
        (item) => item.entity_key === card.dataset.open,
      );
      if (entity) openEditor(entity);
    }
  }
...
$("#editor").addEventListener("focusin", (event) => {
  const key = event.target.dataset.help;
  if (key) setHelp(key);
});
$("#editor").addEventListener("mouseover", (event) => {
  const target = event.target.closest("[data-help]");
  if (target) setHelp(target.dataset.help);
});
$("#name").addEventListener("input", () => {
  if (!state.editing && !state.keyTouched)
    $("#key").value = technicalKey(
      $("#name").value,
      state.type === "building" ? "batiment" : state.type,
    );
  if (state.type === "building" && state.interfaceDraft) {
    const previous = state.interfaceDraft.target_building_key,
      current = $("#key").value;
    state.interfaceDraft.target_building_key = current;
    state.interfaceDraft.pages.forEach((page) =>
      page.components.forEach((component) => {
        if (
          component.interaction?.type === "action" &&
          component.interaction.building === previous
        )
          component.interaction.building = current;
        (component.options || []).forEach((option) => {
          if (
            option.interaction?.type === "action" &&
            option.interaction.building === previous
          )
            option.interaction.building = current;
        });
      }),
    );
  }
  ...
  try {
    ...
    if (response.status === 409 && state.editing) {
      const latestResponse = await fetch(
        `/api/content/${state.type}/${encodeURIComponent(key)}`,
        { headers, cache: "no-store" },
      );
      if (!latestResponse.ok) throw Error((await response.json()).detail);
      const latest = await latestResponse.json(),
        automatic = /^(migration|seed|import)/i.test(
      ...
      if (
        ...
        throw Error(
          "Enregistrement annulé : recharge le bâtiment pour voir la version la plus récente.",
        );
    ...
    KingdomTutorials.notify("content_saved", savedType);
    state.editorDirty = false;
    closeEditor(true);
    await loadCatalogs();
    await load();
...
$$("#mobile-dock [data-mobile-type]").forEach(
  (button) => (button.onclick = () => navigateTo(button.dataset.mobileType)),
);
$("#mobile-dock [data-mobile-menu]").onclick = () => setSidebarOpen(true);
$("#sidebar-logout").onclick = logoutAccount;
$("#sidebar-theme").onclick = () =>
  applyTheme(
...
$("#sidebar-collapse").onclick = () => {
  ...
  localStorage.setItem(
    "kingdomSidebarCollapsed",
    document.body.classList.contains("sidebar-collapsed") ? "1" : "0",
  );
};
if (localStorage.getItem("kingdomSidebarCollapsed") === "1")
  document.body.classList.add("sidebar-collapsed");
function setupPublicRegistration() {
  const loginForm = $("#login-form"),
    submit = loginForm.querySelector(".login-submit");
  submit.insertAdjacentHTML(
    "afterend",
    '<button type="button" class="login-register-button" id="open-registration">♙ CRÉER UN COMPTE</button>',
  );
  document.body.insertAdjacentHTML(
    "beforeend",
    `<div class="registration-overlay" id="registration-overlay" hidden><form class="registration-card" id="registration-form"><div class="registration-head"><div><small>NOUVEAU COMPTE</small><h2>Rejoindre KingdomWeb</h2><p>Créez votre identité. Un administrateur vous attribuera ensuite les royaumes à gérer.</p></div><button type="button" id="close-registration" aria-label="Fermer">×</button></div><label>Nom affiché<input name="display_name" minlength="2" maxlength="80" autocomplete="name" required placeholder="Votre nom"></label><label>Identifiant<input name="username" minlength="3" maxlength="50" pattern="[a-zA-Z0-9_.-]+" autocomplete="username" required placeholder="ex. atilla"></label><label>Adresse e-mail<input name="email" type="email" maxlength="254" autocomplete="email" placeholder="vous@exemple.fr"></label><div class="registration-passwords"><label>Mot de passe<input name="password" type="password" minlength="8" autocomplete="new-password" required></label><label>Confirmer<input name="password_confirmation" type="password" minlength="8" autocomplete="new-password" required></label></div><p id="registration-error" role="alert"></p><div class="registration-actions"><button type="button" id="cancel-registration">Annuler</button><button class="primary" type="submit">Créer mon compte</button></div></form></div>`,
  );
  ...
  $("#open-registration").onclick = () => {
    overlay.hidden = false;
    form.elements.display_name.focus();
  };
  $("#close-registration").onclick = close;
  $("#cancel-registration").onclick = close;
  ...
  form.onsubmit = async (event) => {
    ...
    try {
      const response = await fetch("/api/auth/register", {
        ...
        result = await response.json();
      if (!response.ok) {
        error.textContent = result.detail || "Création impossible.";
        return;
      }
      close();
      await initializeAccount();
    } catch (_) {
      error.textContent = "KingdomWeb est momentanément inaccessible.";
    } finally {
      button.disabled = false;
    }
  };
...
$("#login-form").onsubmit = async (event) => {
  ...
  try {
    ...
    try {
      ...
      data = { detail: "Le serveur n’a pas renvoyé de réponse exploitable." };
    }
    if (!response.ok) {
      error.textContent = data.detail || "Connexion impossible.";
      return;
    }
    await initializeAccount();
  } catch (_) {
...
$$("#nav [data-nav-group]").forEach(
  (button) =>
    (button.onclick = () => openNavigationGroup(button.dataset.navGroup)),
);
openNavigationGroup("");
async function selectNavigationPage(button) {
  const nextType = button?.dataset.type;
  if (!nextType) return;
  KingdomTutorials.pauseForNavigation(nextType);
  state.viewRequest++;
...
document.addEventListener("click", (event) => {
  const button = event.target.closest?.("#calendar-config-save");
  if (button) saveCalendarConfiguration(button);
});
